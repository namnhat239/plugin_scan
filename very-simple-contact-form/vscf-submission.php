...
if ($error == false) {
	...
	if ((!empty($vscf_atts['subject'])) && ($subject_setting != 'yes')) {
		$subject_for_content = $form_data['form_subject']."\r\n\r\n";
	} else {
		$subject_for_content = '';
	}
	// auto reply message
	...
	if ($privacy_setting == 'yes') {
		$privacy_consent = sprintf( __( 'Privacy consent: %s', 'very-simple-contact-form' ), $privacy_label );
	} else {
		$privacy_consent = '';
	}
	...
	if ($ip_address_setting == 'yes') {
		...
		$ip_address = sprintf( __( 'IP: %s', 'very-simple-contact-form' ), vscf_get_the_ip() );
	}
