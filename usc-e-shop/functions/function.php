...
function usces_delete_ordered_acting_data( $rand ) {
	...
	$query = $wpdb->prepare( "DELETE FROM $wpdb->prefix{usces_access} WHERE acc_type = %s AND acc_key = %s", 'acting_data', $rand );
	$res   = $wpdb->query( $query );
}
...
function usces_is_same_itemcode( $post_id, $item_code ){
	...
	$id_str = implode(',', $ids);
	$query = "SELECT ID FROM $wpdb->posts WHERE ID IN({$id_str}) AND post_status = 'publish' AND post_type = 'post'";
