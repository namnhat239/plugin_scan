		...
		if ( "on" == $( this ).val() ) {
			...
			case 'usces_continue':
				...
				if ( 'usces_orderlist' == $admin_page && ( isset( $_GET['order_action'] ) && ( 'edit' == $_GET['order_action'] || 'editpost' == $_GET['order_action'] || 'newpost' == $_GET['order_action'] ) ) ||
					'usces_continue' == $admin_page && ( isset( $_GET['continue_action'] ) && 'settlement_sbps_card' == $_GET['continue_action'] ) ) {
...
jQuery( document ).ready( function( $ ) {
	...
	public function settlement_update() {
		...
		$options['acting_settings']['sbps']['sales_dlseller'] = ( isset( $_POST['sales_dlseller'] ) ) ? $_POST['sales_dlseller'] : 'manual';
		$options['acting_settings']['sbps']['auto_settlement_mail'] = ( isset( $_POST['auto_settlement_mail'] ) ) ? $_POST['auto_settlement_mail'] : 'off';
		...
		$options['acting_settings']['sbps']['conv_limit'] = ( isset( $_POST['conv_limit'] ) ) ? $_POST['conv_limit'] : '';
	...
	<form action="" method="post" name="sbps_form" id="sbps_form">
		...
		<table class="settle_table">
			...
			?>
			<tr id="ex_cust_manage_sbps" class="explanation card_token_sbps"><td colspan="2">クレジットカード情報お預かりサービスを利用して、会員のカード情報をSBペイメントサービスに保存します。保存する場合は、基本設定の「会員システム」を「利用する」に設定してください。<?php echo $cust_manage_msg; ?></td></tr>
			...
			<tr class="card_link_token_sbps">
				...
				<td><label><input name="sales_dlseller" type="radio" id="sales_dlseller_sbps_manual" value="manual"<?php if( $acting_opts['sales_dlseller'] == 'manual' ) echo ' checked="checked"'; ?> /><span>指定売上（仮売上）</span></label><br />
					<label><input name="sales_dlseller" type="radio" id="sales_dlseller_sbps_auto" value="auto"<?php if( $acting_opts['sales_dlseller'] == 'auto' ) echo ' checked="checked"'; ?> /><span>自動売上（実売上）</span></label>
				</td>
			</tr>
			<tr id="ex_sales_dlseller_sbps" class="explanation card_link_token_sbps"><td colspan="2">自動継続課金（要 WCEX DLSeller）時の売上方式。</td></tr>
			<tr class="card_link_token_sbps">
				<th><a class="explanation-label" id="label_ex_auto_settlement_mail_sbps"><?php _e( 'Automatic Continuing Charging Completion Mail', 'usces' );//自動継続課金完了メール ?></a></th>
				<td><label><input name="auto_settlement_mail" type="radio" id="auto_settlement_mail_sbps_1" value="on"<?php if( $acting_opts['auto_settlement_mail'] == 'on' ) echo ' checked="checked"'; ?> /><span><?php _e( "Send", 'usces' ); ?></span></label><br />
					<label><input name="auto_settlement_mail" type="radio" id="auto_settlement_mail_sbps_0" value="off"<?php if( $acting_opts['auto_settlement_mail'] == 'off' ) echo ' checked="checked"'; ?> /><span><?php _e( "Don't send", 'usces' ); ?></span></label>
				</td>
			</tr>
			<tr id="ex_auto_settlement_mail_sbps" class="explanation card_link_token_sbps"><td colspan="2"><?php _e( 'Send billing completion mail to the member on which automatic continuing charging processing (required WCEX DLSeller) is executed.', 'usces' ); ?></td></tr>
			<?php endif; ?>
			<tr>
			...
			<tr class="card_link_sbps">
				<th>3Dセキュア</th>
				<td><label><input name="3d_secure" type="radio" id="threed_secure_sbps_1" value="on"<?php if ( isset( $acting_opts['3d_secure'] ) && $acting_opts['3d_secure'] == 'on' ) echo ' checked="checked"'; ?> /><span><?php _e( 'Use', 'usces' ); ?></span></label><br />
					<label><input name="3d_secure" type="radio" id="threed_secure_sbps_2" value="off"<?php if ( isset( $acting_opts['3d_secure'] ) && $acting_opts['3d_secure'] == 'off' ) echo ' checked="checked"'; ?> /><span><?php _e( 'Do not Use', 'usces' ); ?></span></label>
				</td>
			</tr>
		</table>
		<table class="settle_table">
			<tr>
				<th>コンビニ決済</th>
				<td><label><input name="conv_activate" type="radio" class="conv_activate_sbps" id="conv_activate_sbps_1" value="on"<?php if ( isset( $acting_opts['conv_activate'] ) && $acting_opts['conv_activate'] == 'on' ) echo ' checked="checked"'; ?> /><span><?php _e( 'Use', 'usces' ); ?></span></label><br />
					<label><input name="conv_activate" type="radio" class="conv_activate_sbps" id="conv_activate_sbps_2" value="off"<?php if ( isset( $acting_opts['conv_activate'] ) && $acting_opts['conv_activate'] == 'off' ) echo ' checked="checked"'; ?> /><span><?php _e( 'Do not Use', 'usces' ); ?></span></label>
				</td>
			</tr>
			<tr class="conv_sbps">
				<th><a class="explanation-label" id="label_ex_conv_limit_sbps"><?php _e( 'Payment due days', 'usces' ); ?></a></th>
				<td><input name="conv_limit" type="text" id="conv_limit" value="<?php echo $acting_opts['conv_limit']; ?>" class="small-text" /><?php _e( 'days', 'usces' ); ?>（1～59）</td>
			</tr>
...
クレジットカード決済以外の決済サービスでは、全て「リンク型」になります。</p>
		<p>尚、本番環境では、正規SSL証明書のみでのSSL通信となりますのでご注意ください。</p>
			...
			?>
		<p>※Welcart v1.9.34 以前からご利用で、「リンク型」から「API型」へ変更される場合は、SBペイメントサービスへの申請が必要となります。また、管理画面連携機能<?php echo $wcex; ?>を利用したい場合は、「3DES 暗号化キー」「3DES 初期化キー」「Basic 認証ID」「Basic 認証 Password」の設定が必要です。<br />
	...
	public function settlement_information( $data, $action_args ) {
		...
		if ( 'new' != $order_action && ! empty( $order_id ) ) {
			...
			if ( 'acting_sbps_card' == $payment['settlement'] || 'acting_sbps_paypay' == $payment['settlement'] ) {
				...
				if ( ! empty( $tracking_id ) ) {
					echo '<input type="button" class="button settlement-information" id="settlement-information-' . $tracking_id . '" data-tracking_id="' . $tracking_id . '" data-num="1" value="' . __( 'Settlement info', 'usces' ) . '">';
				}
			}
		}
	}
	...
	public function update_settlement( $html, $member ) {
		...
		if ( 'on' == $acting_opts['cust_manage'] || 'choice' == $acting_opts['cust_manage'] ) {
			...
			if ( isset( $cust_ref['result'] ) && 'OK' === $cust_ref['result'] ) {
				...
				$html .= '<li><a href="' . $register_settlement_url . '">' . __( "Credit card registration is here >>", 'usces' ) . '</a></li>';
			}
		}
		return $html;
	}
...
<div class="whitebox">
	...
	<form id="member-card-info" name="member_update_settlement" action="<?php echo $update_settlement_url; ?>" method="post" onKeyDown="if(event.keyCode == 13) {return false;}">
		<?php echo $html; ?>
		<div class="send">
	...
	<?php if ( $register ) : ?>
			<input type="button" id="card-register" class="card-update" data-update_mode="register" value="<?php _e( 'Register', 'usces' ); ?>" />
	<?php else : ?>
			<input type="button" id="card-update" class="card-update" data-update_mode="update" value="<?php _e( 'Update', 'usces' ); ?>" />
		<?php if ( ! usces_have_member_continue_order( $member['ID'] ) && ! usces_have_member_regular_order( $member['ID'] ) ) : ?>
			<input type="button" id="card-delete" class="card-delete" data-update_mode="delete" value="<?php _e( 'Delete', 'usces' ); ?>" />
		<?php endif; ?>
	<?php endif; ?>
...
<sps-api-request id="ST02-00201-101">
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
						...
						if ( isset( $response_sales['res_result'] ) && 'OK' === $response_sales['res_result'] ) {
							...
							$log = array( 'acting' => $this->acting_card, 'key' => $rand, 'result' => $res_err_code, 'data' => $data );
			...
			$log = array( 'acting' => $this->acting_card . '(member_process)', 'key' => $member_id, 'result' => $result, 'data' => $cust_ref );
			usces_save_order_acting_error( $log );
	...
	public function admin_notices_autodelivery( $msg ) {
		...
		if ( ( isset( $acting_opts['activate'] ) && 'on' == $acting_opts['activate'] ) &&
			...
			if ( ( ! $zeus_flag || 'off' == $zeus_batch ) || ( ! $welcart_flag || 'off' == $welcart_batch ) ) {
				$msg = '
				<div class="error">
				<p>' . __( "「クレジット決済設定」にて、SBペイメントサービスのクレジットカード情報保存を「保存する」に設定してください。", 'usces' ) . '</p>
				</div>';
			}
		}
		return $msg;
...
<sps-api-request id="ST02-00201-101">
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
						...
						if ( isset( $response_sales['res_result'] ) && 'OK' === $response_sales['res_result'] ) {
							...
							$log = array( 'acting' => $this->acting_card, 'key' => $rand, 'result' => $response_sales['res_err_code'], 'data' => $response_sales );
							usces_save_order_acting_error( $log );
						}
					...
					$this->save_acting_log( $response_credit, $this->acting_card, $acting_opts['sales'], $response_credit['res_result'], $order_id, $tracking_id );
					$settltment_errmsg = __( '[Regular purchase] Settlement was not completed.', 'autodelivery' );
					$log = array( 'acting' => $this->acting_card, 'key' => $rand, 'result' => $response_credit['res_err_code'], 'data' => $response_credit );
					usces_save_order_acting_error( $log );
					do_action( 'usces_action_register_auto_orderdata', $args, $response_credit );
				}
			} else {
				...
				$settltment_errmsg = __( '[Regular purchase] Settlement was not completed.', 'autodelivery' );
				$log = array( 'acting' => $this->acting_card, 'key' => $rand, 'result' => $response_settlement['res_err_code'], 'data' => $response_settlement );
				usces_save_order_acting_error( $log );
