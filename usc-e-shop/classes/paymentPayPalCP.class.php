...
※「お問い合わせ内容を具体的にご記入ください」の欄に、Welcart にて<?php echo esc_html( $billing ); ?>機能の利用を希望の旨ご記入ください。<br />
※ページ上に「新規お申し込み専用窓口」と記載がありますが、既存アカウントをお持ちの方でもこちらからお申し込み頂けます。</p>
				<?php
			endif;
			?>
<p>クレジットカード決済のご利用には別途ペイパルへのお申込みと審査が必要です。<br />
クレジットカード決済の申込みは<a href="https://www.paypal.com/jp/webapps/mpp/ucc-sales-form">こちら</a>から行えます。</p>
	</div>
	...
	public function enqueue_scripts() {
		...
		if ( ! is_admin() && ( 'delivery' == $usces->page || 'member_update_settlement' == $usces->page || 'member_register_settlement' == $usces->page ) && $this->is_validity_acting( 'card' ) ) {
...
jQuery( document ).ready( function( $ ) {
	$( "input[name='deletemember']" ).css( "display", "none" );
			...
			if ( 'member_update_settlement' == $usces->page || 'member_register_settlement' == $usces->page ) :
				$acting_opts  = $this->get_acting_settings();
				$cp_client_id = ( isset( $acting_opts['client_id'] ) ) ? $acting_opts['client_id'] : '';
				$card_vault   = ( isset( $acting_opts['card_vault'] ) ) ? $acting_opts['card_vault'] : 'off';
				if ( 'on' !== $card_vault ) {
