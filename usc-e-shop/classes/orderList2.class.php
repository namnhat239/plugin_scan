...
class WlcOrderList { // phpcs:ignore
	 ...
	 * Order hook init.
	 */
	public function usces_order_hook_init() {
		add_filter('usces_filter_orderlist_column', array( $this, 'self_filter_orderlist_column' ), 10, 2 );
		add_filter('usces_filter_orderlist_all_column', array( $this, 'self_filter_orderlist_all_column' ), 10, 2 );
		add_filter('usces_filter_orderlist_sql_after_note', array( $this, 'self_filter_orderlist_sql_after_note' ), 10, 2);
	}

	/**
	 * Filter order list column.
	 */
	public function self_filter_orderlist_column( $arr_column, $_this ) {
	...
	/**
	 * Filter order list all column.
	 */
	public function self_filter_orderlist_all_column( $arr_column, $_this ) {
		$arr_mail_print_fields = get_option( 'usces_order_mail_print_fields' );
		foreach ( $arr_mail_print_fields as $key => $value ) {
			$arr_column[$key] = $value['label'];
		}
	...
	/**
	 * Filter orderlist sql after note.
	 */
	public function self_filter_orderlist_sql_after_note( $qstr, $_this ) {
		$select_field = ", ord.order_check \n";
		if ( strpos( $qstr, $select_field ) !== false ) {
			return $qstr;
		}
		$qstr .= $select_field;
		return $qstr;
