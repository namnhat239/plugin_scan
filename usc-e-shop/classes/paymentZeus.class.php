...
jQuery(document).ready(function($) {
	...
	public function settlement_update() {
		...
		if ( isset( $_POST['acting'] ) && $this->paymod_id != wp_unslash( $_POST['acting'] ) ) { // phpcs:ignore
			return;
		}
	...
	public function settlement_tab_body() {
		...
		if ( in_array( 'zeus', (array) $settlement_selected ) ) :
			...
			$connection_link = '';
			$connection_api  = '';
			if ( isset( $acting_opts['connection'] ) && 1 === (int) $acting_opts['connection'] ) {
				$connection_link = ' checked';
			} elseif ( isset( $acting_opts['connection'] ) && 2 === (int) $acting_opts['connection'] ) {
				$connection_api = ' checked';
			}
			$authkey          = ( isset( $acting_opts['authkey'] ) ) ? $acting_opts['authkey'] : '';
			$threedsecure_on  = '';
			$threedsecure_off = '';
			if ( isset( $acting_opts['3dsecur'] ) && 1 === (int) $acting_opts['3dsecur'] ) {
				$threedsecure_on = ' checked';
			} else {
				$threedsecure_off = ' checked';
			}
			$security_on  = '';
			$security_off = '';
			if ( isset( $acting_opts['security'] ) && 1 === (int) $acting_opts['security'] ) {
				$security_on = ' checked';
			} else {
				$security_off = ' checked';
			}
			$quickcharge_on  = '';
			$quickcharge_off = '';
			if ( isset( $acting_opts['quickcharge'] ) && 'on' === $acting_opts['quickcharge'] ) {
				$quickcharge_on = ' checked';
			} else {
				$quickcharge_off = ' checked';
			}
			$batch_on  = '';
			$batch_off = '';
			if ( isset( $acting_opts['batch'] ) && 'on' === $acting_opts['batch'] ) {
				$batch_on = ' checked';
			} else {
				$batch_off = ' checked';
			}
			$howpay_on  = '';
			$howpay_off = '';
			if ( isset( $acting_opts['howpay'] ) && 'on' === $acting_opts['howpay'] ) {
				$howpay_on = ' checked';
			} else {
				$howpay_off = ' checked';
			}
			...
			if ( isset( $acting_opts['auto_settlement_mail'] ) && 'on' === $acting_opts['auto_settlement_mail'] ) {
				$auto_settlement_mail_on = ' checked';
			} else {
				$auto_settlement_mail_off = ' checked';
			}
			$bank_activate_on  = '';
			$bank_activate_off = '';
			if ( isset( $acting_opts['bank_activate'] ) && 'on' === $acting_opts['bank_activate'] ) {
				$bank_activate_on = ' checked';
			} else {
				$bank_activate_off = ' checked';
			}
			$bank_ope_test   = '';
			$bank_ope_public = '';
			if ( isset( $acting_opts['bank_ope'] ) && 'public' === $acting_opts['bank_ope'] ) {
				$bank_ope_public = ' checked';
			} else {
				$bank_ope_test = ' checked';
			}
			...
			if ( isset( $acting_opts['conv_activate'] ) && 'on' === $acting_opts['conv_activate'] ) {
				$conv_activate_on = ' checked';
			} else {
				$conv_activate_off = ' checked';
			}
			$conv_ope_test   = '';
			$conv_ope_public = '';
			if ( isset( $acting_opts['conv_ope'] ) && 'public' === $acting_opts['conv_ope'] ) {
				$conv_ope_public = ' checked';
			} else {
				$conv_ope_test = ' checked';
			}
			...
			$test_type_conv_1 = '';
			$test_type_conv_2 = '';
			if ( isset( $acting_opts['test_type_conv'] ) && 1 === (int) $acting_opts['test_type_conv'] ) {
				$test_type_conv_1 = ' checked';
			} elseif ( isset( $acting_opts['test_type_conv'] ) && 2 === (int) $acting_opts['test_type_conv'] ) {
				$test_type_conv_2 = ' checked';
			} else {
				$test_type_conv_0 = ' checked';
			}
			$pay_cvs_d001 = '';
			$pay_cvs_d002 = '';
			$pay_cvs_d030 = '';
			...
			$pay_cvs_d050 = '';
			$pay_cvs_d060 = '';
			if ( isset( $acting_opts['pay_cvs'] ) ) {
				if ( in_array( 'D001', (array) $acting_opts['pay_cvs'] ) ) {
					$pay_cvs_d001 = ' checked';
				}
				if ( in_array( 'D002', (array) $acting_opts['pay_cvs'] ) ) {
					$pay_cvs_d002 = ' checked';
				}
				if ( in_array( 'D030', (array) $acting_opts['pay_cvs'] ) ) {
					$pay_cvs_d030 = ' checked';
				}
				if ( in_array( 'D015', (array) $acting_opts['pay_cvs'] ) ) {
					$pay_cvs_d015 = ' checked';
				}
				if ( in_array( 'D050', (array) $acting_opts['pay_cvs'] ) ) {
					$pay_cvs_d050 = ' checked';
				}
				if ( in_array( 'D060', (array) $acting_opts['pay_cvs'] ) ) {
					$pay_cvs_d060 = ' checked';
				}
			}
	...
	<div class="settlement_service"><span class="service_title"><?php echo esc_html( $this->acting_formal_name ); ?></span></div>
			<?php if ( isset( $_POST['acting'] ) && 'zeus' === $_POST['acting'] ) : // phpcs:ignore ?>
	...
	<form action="" method="post" name="zeus_form" id="zeus_form">
		<table class="settle_table">
			<tr>
				<th>クレジットカード決済</th>
				<td><label><input name="card_activate" type="radio" class="card_activate_zeus" id="card_activate_zeus_1" value="on"<?php echo esc_html( $card_activate_on ); ?> /><span>利用する</span></label><br />
					<label><input name="card_activate" type="radio" class="card_activate_zeus" id="card_activate_zeus_2" value="off"<?php echo esc_html( $card_activate_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			<tr class="card_zeus">
			...
			<tr class="card_zeus">
				...
				<td><label><input name="connection" type="radio" id="connection_zeus_1" value="1"<?php echo esc_html( $connection_link ); ?> /><span>Secure Link</span></label><br />
					<label><input name="connection" type="radio" id="connection_zeus_2" value="2"<?php echo esc_html( $connection_api ); ?> /><span>Secure API</span></label>
				</td>
			</tr>
			...
			<tr class="card_zeus 3dsecur_zeus">
				...
				<td><label><input name="3dsecur" type="radio" id="3dsecur_zeus_1" value="1"<?php echo esc_html( $threedsecure_on ); ?> /><span>利用する</span></label><br />
					<label><input name="3dsecur" type="radio" id="3dsecur_zeus_2" value="2"<?php echo esc_html( $threedsecure_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			...
			<tr class="card_zeus">
				...
				<td><label><input name="security" type="radio" id="security_zeus_1" value="1"<?php echo esc_html( $security_on ); ?> /><span>利用する</span></label><br />
					<label><input name="security" type="radio" id="security_zeus_2" value="2"<?php echo esc_html( $security_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			...
			<tr class="card_zeus">
				...
				<td><label><input name="quickcharge" type="radio" id="quickcharge_zeus_1" value="on"<?php echo esc_html( $quickcharge_on ); ?> /><span>利用する</span></label><br />
					<label><input name="quickcharge" type="radio" id="quickcharge_zeus_2" value="off"<?php echo esc_html( $quickcharge_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			...
			<tr class="card_zeus">
				...
				<td><label><input name="batch" type="radio" id="batch_zeus_1" value="on"<?php echo esc_html( $batch_on ); ?> /><span>利用する</span></label><br />
					<label><input name="batch" type="radio" id="batch_zeus_2" value="off"<?php echo esc_html( $batch_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			...
			<tr class="card_zeus">
				...
				<td><label><input name="auto_settlement_mail" type="radio" id="auto_settlement_mail_zeus_on" value="on"<?php echo esc_html( $auto_settlement_mail_on ); ?> /><span><?php esc_html_e( 'Send', 'usces' ); ?></span></label><br />
					<label><input name="auto_settlement_mail" type="radio" id="auto_settlement_mail_zeus_off" value="off"<?php echo esc_html( $auto_settlement_mail_off ); ?> /><span><?php esc_html_e( "Don't send", 'usces' ); ?></span></label>
				</td>
			</tr>
			...
			<tr class="card_zeus">
				...
				<td><label><input name="howpay" type="radio" id="howpay_zeus_1" value="on"<?php echo esc_html( $howpay_on ); ?> /><span>分割払いに対応する</span></label><br />
					<label><input name="howpay" type="radio" id="howpay_zeus_2" value="off"<?php echo esc_html( $howpay_off ); ?> /><span>一括払いのみ</span></label>
				</td>
			</tr>
		...
		<table class="settle_table">
			<tr>
				...
				<td><label><input name="bank_activate" type="radio" class="bank_activate_zeus" id="bank_activate_zeus_1" value="on"<?php echo esc_html( $bank_activate_on ); ?> /><span>利用する</span></label><br />
					<label><input name="bank_activate" type="radio" class="bank_activate_zeus" id="bank_activate_zeus_2" value="off"<?php echo esc_html( $bank_activate_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			...
			<tr class="bank_zeus">
				...
				<td><label><input name="bank_ope" type="radio" id="bank_ope_zeus_1" value="test"<?php echo esc_html( $bank_ope_test ); ?> /><span>テスト環境</span></label><br />
					<label><input name="bank_ope" type="radio" id="bank_ope_zeus_2" value="public"<?php echo esc_html( $bank_ope_public ); ?> /><span>本番環境</span></label>
				</td>
			</tr>
		...
		<table class="settle_table">
			<tr>
				...
				<td><label><input name="conv_activate" type="radio" class="conv_activate_zeus" id="conv_activate_zeus_1" value="on"<?php echo esc_html( $conv_activate_on ); ?> /><span>利用する</span></label><br />
					<label><input name="conv_activate" type="radio" class="conv_activate_zeus" id="conv_activate_zeus_2" value="off"<?php echo esc_html( $conv_activate_off ); ?> /><span>利用しない</span></label>
				</td>
			</tr>
			...
			<tr class="conv_zeus">
				...
				<td><label><input name="conv_ope" type="radio" id="conv_ope_zeus_1" value="test"<?php echo esc_html( $conv_ope_test ); ?> /><span>テスト環境</span></label><br />
					<label><input name="conv_ope" type="radio" id="conv_ope_zeus_2" value="public"<?php echo esc_html( $conv_ope_public ); ?> /><span>本番環境</span></label>
				</td>
			</tr>
			...
			<tr class="conv_zeus">
				...
				<td><label><input name="test_type" type="radio" id="conv_testtype_zeus_1" value="0"<?php echo esc_html( $test_type_conv_0 ); ?> /><span>入金テスト無し</span></label><br />
					<label><input name="test_type" type="radio" id="conv_testtype_zeus_2" value="1"<?php echo esc_html( $test_type_conv_1 ); ?> /><span>売上確定テスト</span></label><br />
					<label><input name="test_type" type="radio" id="conv_testtype_zeus_3" value="2"<?php echo esc_html( $test_type_conv_2 ); ?> /><span>売上取消テスト</span></label>
				</td>
			</tr>
			...
			<tr class="conv_zeus">
				...
				<td><label><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D001" value="D001"<?php echo esc_html( $pay_cvs_d001 ); ?> /><span><?php echo esc_html( usces_get_conv_name( 'D001' ) ); ?></span></label></td>
			</tr>
			<tr class="conv_zeus">
				<td><label><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D002" value="D002"<?php echo esc_html( $pay_cvs_d002 ); ?> /><span><?php echo esc_html( usces_get_conv_name( 'D002' ) ); ?></span></label></td>
			</tr>
			<tr class="conv_zeus">
				<td><label><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D030" value="D030"<?php echo esc_html( $pay_cvs_d030 ); ?> /><span><?php echo esc_html( usces_get_conv_name( 'D030' ) ); ?></span></label></td>
			</tr>
			<tr class="conv_zeus">
				<td><label><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D015" value="D015"<?php echo esc_html( $pay_cvs_d015 ); ?> /><span><?php echo esc_html( usces_get_conv_name( 'D015' ) ); ?></span></label></td>
			</tr>
			<tr class="conv_zeus">
				<td><label><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D050" value="D050"<?php echo esc_html( $pay_cvs_d050 ); ?> /><span><?php echo esc_html( usces_get_conv_name( 'D050' ) ); ?></span></label></td>
			</tr>
			<tr class="conv_zeus">
				<td><label><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D060" value="D060"<?php echo esc_html( $pay_cvs_d060 ); ?> /><span><?php echo esc_html( usces_get_conv_name( 'D060' ) ); ?></span></label></td>
			</tr>
			...
			<tr>
				...
				<td>当日＋<select name="conv_span">
					<option value=""></option>
			<?php for ( $i = 2; $i <= 59; $i++ ) : ?>
					<option value="<?php echo esc_html( $i ); ?>"<?php selected( $conv_span, $i, true ); ?>><?php echo esc_html( $i ); ?></option>
			<?php endfor; ?>
				</select>日</td>
			</tr>
	...
	<div class="settle_exp">
		...
		詳しくは<a href="https://www.cardservice.co.jp/" target="_blank">株式会社ゼウス</a>（代表：03-3498-9030）にお問い合わせください。</p>
		<p>　</p>
	...
	public function acting_construct() {
		...
		if ( ! empty( $_SERVER['REMOTE_ADDR'] ) && in_array( $_SERVER['REMOTE_ADDR'], $acting_opts['ipaddrs'] ) ) {
			$sendpoint   = ( isset( $_REQUEST['sendpoint'] ) ) ? $_REQUEST['sendpoint'] : ''; // phpcs:ignore
			$acting_data = usces_get_order_acting_data( $sendpoint );
			if ( empty( $acting_data['sesid'] ) ) {
				if ( isset( $_REQUEST['result'] ) && 'OK' == $_REQUEST['result'] && isset( $_REQUEST['money'] ) && '0' == $_REQUEST['money'] ) { // phpcs:ignore
				} else {
					$log = array(
						'acting' => 'zeus',
						'key'    => $sendpoint,
				...
				if ( isset( $_REQUEST['acting'] ) && 'zeus_bank' == $_REQUEST['acting'] ) { // phpcs:ignore
	...
	public function get_link_key( $linkkey, $results ) {
		if ( isset( $results['order_number'] ) && isset( $results['sendpoint'] ) ) {
			$linkkey = $_REQUEST['sendpoint']; // phpcs:ignore
		}
		return $linkkey;
	}
	...
	public function admin_ajax() {
		...
		switch ( $mode ) {
			...
			case 'get_zeus_card':
				...
				$order_num   = ( isset( $post_data['order_num'] ) ) ? $post_data['order_num'] : '';
				$tracking_id = ( isset( $post_data['tracking_id'] ) ) ? $post_data['tracking_id'] : '';
				...
				if ( ! empty( $latest_log['order_no'] ) ) {
					...
					if ( 'TEST' === $order_ref['status'] ) {
						...
						$res        .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>
							<td><input type="tel" class="settlement-amount" value="' . intval( $latest_log['amount'] ) . '" disabled="disabled" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>
							</tr>';
						$res        .= '</table>';
						...
						if ( ! empty( $member_id ) ) {
							$res .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>
								<td><input type="tel" class="settlement-amount" id="amount_change" value="' . intval( $latest_log['amount'] ) . '" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>
								</tr>';
							$res .= '</table>';
							$res .= '<div class="settlement-admin-button">';
			...
			case 'cancel_zeus_card':
				...
				if ( ! empty( $latest_log['order_no'] ) ) {
					...
					if ( false !== strpos( $page_cancel, 'SuccessOK' ) ) {
						...
						$res          .= '<table class="settlement-admin-table">';
						$res          .= '<tr><th>' . __( 'Settlement amount', 'usces' ) . '</th>
							<td><input type="tel" class="settlement-amount" id="amount_change" value="0" />' . __( usces_crcode( 'return' ), 'usces' ) . '</td>
							</tr>';
						$res          .= '</table>';
						$res          .= '<div class="settlement-admin-button">';
						$res          .= '<input id="re_settlement" type="button" class="button" value="新規決済" />';
	...
	private function settlement_history( $order_id, $tracking_id ) {
		...
		if ( $log_data ) {
			...
			foreach ( (array) $log_data as $data ) {
				...
				if ( ! in_array( $data['result'], $this->payment_normal_results ) ) {
...
<div id="settlement_dialog" title="">
	...
	public function admin_update_memberdata( $member_id, $res ) {
		...
		if ( isset( $_POST['zeus_pcid'] ) && 'delete' == $_POST['zeus_pcid'] ) { // phpcs:ignore
	...
	public function delivery_check( $mes ) {
		...
		switch ( $acting_flg ) {
			case 'acting_zeus_card':
				if ( isset( $_POST['acting'] ) && 'zeus' != $_POST['acting'] ) { // phpcs:ignore
					$mes .= 'カード決済データが不正です！';
				} elseif ( empty( $_POST['zeus_card_option'] ) || ( 'new' == $_POST['zeus_card_option'] && empty( $_POST['zeus_token_value'] ) ) ) { // phpcs:ignore
					$mes .= 'カード決済データが不正です！';
				}
				break;

			case 'acting_zeus_conv':
				if ( WCUtils::is_blank( $_POST['username_conv'] ) ) { // phpcs:ignore
					$mes .= 'お名前を入力してください。<br />';
				} elseif ( ! preg_match( '/^[ァ-ヶー]+$/u', $_POST['username_conv'] ) ) { // phpcs:ignore
	...
	public function delivery_secure_form_loop( $nouse, $payment ) {
		...
		switch ( $payment['settlement'] ) {
			case 'acting_zeus_card':
				...
				$howpay = ( isset( $_POST['howpay'] ) ) ? $_POST['howpay'] : '1'; // phpcs:ignore
				$cbrand = ( isset( $_POST['cbrand'] ) ) ? $_POST['cbrand'] : ''; // phpcs:ignore
				$div    = ( isset( $_POST['div'] ) ) ? $_POST['div'] : ''; // phpcs:ignore

				$form_howpay = '';
				$member_page = ( isset( $_GET['usces_page'] ) && 'member_update_settlement' == wp_unslash( $_GET['usces_page'] ) ) ? true : false; // phpcs:ignore
			...
			case 'acting_zeus_conv':
				...
				$acting_opts = $this->get_acting_settings();
				$pay_cvs     = ( isset( $_POST['pay_cvs'] ) ) ? wp_unslash( $_POST['pay_cvs'] ) : ''; // phpcs:ignore
				$entry       = $usces->cart->get_entry();
				$username    = ( isset( $_POST['username_conv'] ) ) ? wp_unslash( $_POST['username_conv'] ) : $entry['customer']['name3'] . $entry['customer']['name4']; // phpcs:ignore
	...
	public function confirm_inform( $form, $payments, $acting_flg, $rand, $purchase_disabled ) {
		...
		switch ( $acting_flg ) {
			case 'acting_zeus_card':
				...
				$zeus_card_option = ( isset( $_POST['zeus_card_option'] ) ) ? wp_unslash( $_POST['zeus_card_option'] ) : ''; // phpcs:ignore
				$zeus_token_value = ( isset( $_POST['zeus_token_value'] ) ) ? wp_unslash( $_POST['zeus_token_value'] ) : ''; // phpcs:ignore
				$form             = '<form id="purchase_form" action="' . USCES_CART_URL . '" method="post" onKeyDown="if (event.keyCode == 13) {return false;}">
				<input type="hidden" name="card_option" value="' . $zeus_card_option . '">
				<input type="hidden" name="token_key" value="' . $zeus_token_value . '">
			...
			case 'acting_zeus_conv':
				...
				$username_conv = ( isset( $_POST['username_conv'] ) ) ? wp_unslash( $_POST['username_conv'] ) : ''; // phpcs:ignore
				$form          = '<form id="purchase_form" action="' . USCES_CART_URL . '" method="post" onKeyDown="if (event.keyCode == 13) {return false;}">
			...
			case 'acting_zeus_bank':
				...
				if ( isset( $acting_opts['bank_ope'] ) && 'test' == $acting_opts['bank_ope'] ) {
					$form .= '<input type="hidden" name="username" value="' . esc_attr( trim( $entry['customer']['name3'] ) . trim( $entry['customer']['name4'] ) . '_' . $acting_opts['testid_bank'] ) . '">';
					$form .= '<input type="hidden" name="telno" value="99999999999">';
				} else {
					$form .= '<input type="hidden" name="username" value="' . esc_attr( trim( $entry['customer']['name3'] ) . trim( $entry['customer']['name4'] ) ) . '">';
		...
		switch ( $acting_flg ) {
			case 'acting_zeus_card':
				...
				if ( isset( $_POST['zeus_card_option'] ) ) {
					$form .= '<input type="hidden" name="zeus_card_option" value="' . esc_attr( $_POST['zeus_card_option'] ) . '">';
				}
				if ( isset( $_POST['zeus_card_option'] ) && 'new' == $_POST['zeus_card_option'] ) {
					$form .= '<input type="hidden" name="zeus_token_value" value="' . esc_attr( $_POST['zeus_token_value'] ) . '">';
				}
				// phpcs:enable
				break;
	...
	public function pre_purchase() {
		...
		if ( empty( $cart ) ) {
			$tracking_id = ( isset( $_GET['sendpoint'] ) ) ? $_GET['sendpoint'] : ''; // phpcs:ignore
			if ( ! empty( $tracking_id ) ) {
	...
	public function acting_processing( $acting_flg, $post_query ) {
		...
		if ( empty( $cart ) ) {
			if ( isset( $_REQUEST['sendpoint'] ) ) {
				usces_restore_order_acting_data( $_REQUEST['sendpoint'] ); // phpcs:ignore
			...
			if ( ! wp_verify_nonce( $_REQUEST['_nonce'], $acting_flg ) ) { // phpcs:ignore
				wp_redirect( USCES_CART_URL );
			}
		}
		...
		if ( 'acting_zeus_card' === $acting_flg && 2 === (int) $acting_opts['connection'] ) {
			...
			/* 3D Secure */
			if ( 1 === (int) $acting_opts['3dsecur'] ) {
				if ( ! isset( $_REQUEST['PaRes'] ) ) {
					/* Enrol Reqest */
					// usces_log( 'zeus card 3dsecure entry data (acting_processing) : ' . print_r( $_REQUEST, true ), 'acting_transaction.log' );
				...
				// usces_log( 'zeus card no3d entry data (acting_processing) : ' . print_r( $_REQUEST, true ), 'acting_transaction.log' );
			...
			// usces_log( 'zeus card securelink entry data (acting_processing) : ' . print_r( $_POST, true ), 'acting_transaction.log' );
			...
			$page = $this->secure_link_batch( $acting_opts['card_url'], $params );
			if ( false !== strpos( $page, 'Success_order' ) ) {
				// usces_auth_order_acting_data( $sendpoint );
			...
			if ( $fp ) {
				...
				if ( false !== strpos( $page, 'Success_order' ) ) {
					// usces_log( 'zeus conv entry data (acting_processing) : ' . print_r( $_POST, true ), 'acting_transaction.log' );
					$result_data = array(
						'acting'        => 'zeus_conv',
						'acting_return' => '1',
					);
	...
	public function pre_acting_return() {
		...
		$tracking_id = ( isset( $_GET['wctid'] ) ) ? $_GET['wctid'] : ''; // phpcs:ignore
		if ( empty( $tracking_id ) ) {
			$tracking_id = ( isset( $_GET['sendpoint'] ) ) ? $_GET['sendpoint'] : ''; // phpcs:ignore
		}
		if ( ! empty( $tracking_id ) ) {
	...
	public function acting_return( $results ) {
		$acting = ( isset( $_GET['acting'] ) ) ? $_GET['acting'] : ''; // phpcs:ignore
		switch ( $acting ) {
			case 'zeus_card':
				// $results = wp_unslash( $_REQUEST );
	...
	public function register_orderdata( $args ) {
		global $usces;
		extract( $args ); /* $cart, $entry, $order_id, $member_id, $payments, $charging_type, $results */ // phpcs:ignore
	...
	public function error_page_message( $form ) {
		...
		$acting = ( isset( $_REQUEST['acting'] ) ) ? wp_unslash( $_REQUEST['acting'] ) : ''; // phpcs:ignore
		if ( 'zeus_card' == $acting ) {
			$form .= '<div class="support_box">';
			if ( isset( $_GET['code'] ) ) {
				$code  = wp_unslash( $_GET['code'] ); // phpcs:ignore
				$form .= '<br />エラーコード：' . esc_html( $code );
				if ( in_array( $code, array( '02130514', '02130517', '02130619', '02130620', '02130621', '02130640' ) ) ) {
					$form .= '<br />カード番号が正しくないようです。';
				} elseif ( in_array( $code, array( '02130714', '02130717', '02130725', '02130814', '02130817', '02130825' ) ) ) {
					$form .= '<br />カードの有効期限が正しくないようです。';
				} elseif ( in_array( $code, array( '02130922' ) ) ) {
					$form .= '<br />カードの有効期限が切れているようです。';
				} elseif ( in_array( $code, array( '02131117', '02131123', '02131124' ) ) ) {
					$form .= '<br />カードの名義が正しくないようです。';
				} elseif ( in_array( $code, array( '02131414', '02131417', '02131437' ) ) ) {
					$form .= '<br />お客様情報の電話番号が正しくないようです。';
				} elseif ( in_array( $code, array( '02131527', '02131528', '02131529', '02131537' ) ) ) {
				...
				if ( isset( $_GET['err_code'] ) ) {
					$form .= '<br />エラーコード：' . esc_html( wp_unslash( $_GET['err_code'] ) ); // phpcs:ignore
				}
				$form .= '<br />
		...
		if ( isset( $_POST['zeus_card_update'] ) ) {
			...
			if ( ! $check_verify_recaptcha ) {
				...
				if ( ! wp_verify_nonce( $_POST['wc_nonce'], 'member_update_settlement' ) || empty( $_POST['zeus_token_value'] ) ) {
					...
					$params['send']         = 'mall';
					$params['clientip']     = $acting_opts['clientip'];
					$params['token_key']    = $_POST['zeus_token_value']; // phpcs:ignore
					$params['cardnumber']   = '8888888888888882';
					$params['expyy']        = '00';
					$params['expmm']        = '00';
					$params['money']        = '0';
					...
					if ( false !== strpos( $page, 'Success_order' ) ) {
						...
						if ( ! empty( $_POST['zeus_token_masked_card_no'] ) ) {
							$partofcard = substr( $_POST['zeus_token_masked_card_no'], -4 ); // phpcs:ignore
...
<div class="whitebox">
	...
	<form id="member-card-info" name="member_update_settlement" action="<?php echo esc_url( $update_settlement_url ); ?>" method="post" onKeyDown="if(event.keyCode == 13) {return false;}">
		...
		<div class="send">
			...
			<input type="button" id="card-register" class="card-update" value="<?php esc_html_e( 'Register' ); ?>" />
			<?php else : ?>
			<input type="hidden" name="zeus_card_update" value="update" />
			<input type="button" id="card-update" class="card-update" value="<?php esc_html_e( 'update it', 'usces' ); ?>" />
			<?php endif; ?>
			<input type="button" name="back" value="<?php esc_html_e( 'Back to the member page.', 'usces' ); ?>" onclick="location.href='<?php echo esc_url( USCES_MEMBER_URL ); ?>'" />
			<input type="button" name="top" value="<?php esc_html_e( 'Back to the top page.', 'usces' ); ?>" onclick="location.href='<?php echo esc_url( home_url() ); ?>'" />
		</div>
	...
	<div class="footer_explanation">
		...
		<?php else : ?>
<p><?phpesc_html_e( 'Sorry, no posts matched your criteria.', 'usces' ); ?></p>
		<?php endif; ?>
</div><!-- end of catbox -->
</div><!-- end of content -->
		<?php
	...
	public function continue_member_list_condition( $condition, $member_id, $order_id, $continue_data ) {
		...
		if ( isset( $continue_data['acting'] ) && 'acting_zeus_card' == $continue_data['acting'] ) {
			...
			if ( 'continuation' === $continue_data['status'] ) {
				...
				if ( isset( $latest_log['result'] ) && ! in_array( $latest_log['result'], $this->payment_normal_results ) ) {
...
<input name="usces_referer" type="hidden" id="usces_referer" value="<?php echo urlencode( $curent_url ); ?>" />
	...
	public function auto_settlement_message( $member_id, $order_id, $order_data, $data, $continue_data, $html = true ) {
		...
		if ( usces_is_html_mail() && $html ) {
			...
			if ( isset( $data['reminder'] ) ) {
				...
				if ( isset( $data['result'] ) && in_array( $data['result'], $this->payment_normal_results ) ) {
					$message .= '<tr><td style="background-color: #f9f9f9; padding: 12px; width: 33%; border: 1px solid #ddd; text-align: left;">';
	...
	protected function zeus_3dsecure_enrol() {
		...
		$_nonce      = ( isset( $_POST['_nonce'] ) ) ? $_POST['_nonce'] : wp_create_nonce( 'acting_zeus_card' );
		...
		if ( 'outside' == $EnrolRes['response']['result']['status'] ) {
			...
			if ( empty( $xml ) ) {
				$log = array(
					'acting' => 'zeus_card_API(3D Payment)',
					'key'    => $sendpoint,
					'result' => 'PayRes Error',
					'data'   => $PayReq,
				);
			...
			if ( 'success' == $PayRes['response']['result']['status'] ) {
				...
				$log = array(
					'acting' => 'zeus_card_API(3D Payment)',
					'key'    => $sendpoint,
					'result' => $PayRes['response']['result']['status'] . ':' . $PayRes['response']['result']['code'],
					'data'   => $PayRes,
				);
				usces_save_order_acting_error( $log );
			...
			$term_url    = add_query_arg( $result_data, USCES_CART_URL );
			?>
			...
			$log = array(
				'acting' => 'zeus_card_API(3D Enrol)',
				'key'    => $sendpoint,
				'result' => $EnrolRes['response']['result']['status'] . ':' . $EnrolRes['response']['result']['code'],
				'data'   => $EnrolRes,
			);
			usces_save_order_acting_error( $log );
	...
	protected function zeus_3dsecure_auth() {
		...
		if ( 'success' != $AuthRes['response']['result']['status'] ) {
			$log = array(
				'acting' => 'zeus_card_API(3D Auth)',
				'key'    => $sendpoint,
				'result' => $AuthRes['response']['result']['status'] . ':' . $AuthRes['response']['result']['code'],
				'data'   => $AuthRes,
			);
			usces_save_order_acting_error( $log );
	...
	protected function zeus_secure_payreq() {
		...
		usces_log( 'zeus_secure_payreq PayRes=' . print_r( $PayRes, true ), 'test.log' );
	...
	private function get_status_name( $payment_status, $payment_result = '' ) {
		...
		switch ( $payment_status ) {
			...
			case 'autodelivery':
				...
				break;
			default:
				if ( in_array( $payment_result, $this->payment_normal_results ) ) {
