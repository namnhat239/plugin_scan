...
if($member_action == 'new'){
	...
	$data = array(
			...
			'mem_zip' => ( isset($_POST['member']['zipcode']) ) ? usces_convert_zipcode( $_POST['member']['zipcode'] ) : '',
	...
	if ( is_array( $admb_meta ) ) {
		...
		foreach ( $keys as $key ) {
			if ( isset( $_POST['admin_custom_member'][ $key ] ) ) {
				$admb_meta[ $key ]['data'] = $_POST['admin_custom_member'][ $key ];
			}
		}
	}
	$navibutton = '';
	...
	if ( is_array( $admb_meta ) ) {
		$keys = array_keys( $admb_meta );
		foreach ( $keys as $key ) {
			$admb_meta[ $key ]['data'] = maybe_unserialize( $this->get_member_meta_value( 'admb_' . $key, $ID ) );
		}
	}
...
<script type="text/javascript">
	jQuery(document).ready(function($){
		...
		welPageNav.checkShowNextprevPage( 'wel_member_current_page_ids', sub_uri_link, <?php echo esc_attr( $ID ); ?> );
	});
...
<?php endif; ?>
<?php
	echo( uesces_get_admin_addressform( 'member', $data, $csmb_meta, 'return' ) );
...
<tr>
<td colspan="2"><?php do_action( 'usces_action_member_edit_form_left_blank', $ID, $data, $csmb_meta ); ?></td>
</tr>
</table>
<?php if( !usces_is_membersystem_point() ) : ?>
...
<th class="historyrow"><?php echo $tax_label; ?></th>
<?php endif; ?>
<?php if( $member_system_point && 0 == $point_coverage ) : ?>
...
<th class="historyrow"><?php echo $tax_label; ?></th>
<?php endif; ?>
<?php if( $member_system_point && 1 == $point_coverage ) : ?>
<th class="historyrow"><?php _e('Used points','usces'); ?></th>
<?php endif; ?>
...
<?php if( ! empty( $tracking_number ) ) : 
	$colspan++; ?>
<th class="historyrow"><?php _e('Tracking number', 'usces'); ?></th>
<?php endif; ?>
</tr>
<tr>
...
<?php if ( ! empty( $r_status ) ) :?>
<td><?php echo esc_attr( $r_status ); ?></td>
<?php endif; ?>
...
<?php if( $tax_display && 'products' == $tax_target ) : ?>
<td class="rightnum"><?php echo usces_order_history_tax( $umhs, $tax_mode ); ?></td>
<?php endif; ?>
<?php if( $member_system_point && 0 == $point_coverage ) : ?>
...
<?php if( ! empty( $tracking_number ) ) :
	if( ! empty( $delivery_company_url ) ) : ?>
		<td><a href="<?php echo esc_url( $delivery_company_url ); ?>"><?php echo esc_attr( $tracking_number ); ?></a></td>
	<?php else : ?>
		<td><?php echo esc_attr( $tracking_number ); ?></td>
<?php endif;
endif; ?>
</tr>
<tr>
