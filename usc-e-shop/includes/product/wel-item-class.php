...
class ItemData {
	 ...
	 * @param boolean $cache Switch of cache.
	 */
	public function set_data( $post_id, $cache ) {
		if ( ! $post_id ) {
		...
		if ( $cache && is_array( $product_data ) ) {
			...
			$this->opt_data = $this->get_opts();
			$this->sku_data = $this->get_skus();
	...
	public function update_sku_data( $sku ) {
		...
		if ( ! isset( $sku['meta_id'] ) || empty( $sku['meta_id'] ) ) {
			$target  = $this->get_sku_by_code( $sku['code'] );
		...
		// Recache.
		$this->get_post_meta( '_isku_', false );
	...
	public function update_opt_data( $opt ) {
		...
		// Recache.
		$this->get_post_meta( '_iopt_', false );
