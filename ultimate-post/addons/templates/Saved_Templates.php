...
class Saved_Templates {
    ...
    public function custom_head_templates() {
        if( 'ultp_templates' == get_current_screen()->post_type && (!defined('ULTP_PRO_VER')) ) {
            ...
            if( $post_count > 0 ) { ?>
                <span class="ultp-saved-templates-action" data-link="<?php echo esc_url(ultimate_post()->get_premium_link()); ?>" data-text="Go Pro for Unlimited Templates" data-count="<?php echo esc_attr($post_count); ?>" style="display:none;"></span>
            <?php }
        }
    }
