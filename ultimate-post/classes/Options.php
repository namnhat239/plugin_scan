...
class Options{
    ...
    public function plugin_settings_meta( $links, $file ) {
        if ( strpos( $file, 'ultimate-post.php' ) !== false ) {
            $new_links = array(
                ...
                'ultp_support' =>  '<a href="'.ultimate_post()->get_premium_link( 'https://www.wpxpo.com/contact/' ).'" target="_blank">'.esc_html__('Support', 'ultimate-post').'</a>'
            );
    ...
    public function plugin_action_links_callback( $links ) {
        ...
        if(!defined('ULTP_PRO_VER')){
            $upgrade_link = array(
                'ultp_pro' => '<a href="'.ultimate_post()->get_premium_link().'" target="_blank"><span style="color: #e83838; font-weight: bold;">'.esc_html__('Go Pro', 'ultimate-post').'</span></a>'
            );
        }
    ...
    public static function menu_page_callback() {
        ...
        if( !function_exists('ultimate_post_pro') ){
            require_once ULTP_PATH . 'classes/options/Upgrade.php';
            new \ULTP\Upgrade();
        }
