    ...
    public function ajax_setup()
    {
        ...
        $params = array(
            ...
            'settings' => array(
                ...
                'trap_focus' => $this->settings['advanced']['trap_focus'],
                'trap_elements' => implode(',', apply_filters('age_gate/trap_elements', []))
            ),
            'misc' => [
                'i' => $this->id,
                't' => $this->type,
    ...
    private function _validate($post)
    {
      ...
      // 'age_gate_nonce' => 'required|nonce'
        ...
        if ($this->settings['restrictions']['input_type'] !== 'buttons') {
            $post['date'] = sprintf('%s-%s-%s', $post['age_gate_y'], str_pad($post['age_gate_m'], 2, "0", STR_PAD_LEFT), str_pad($post['age_gate_d'], 2, "0", STR_PAD_LEFT));
