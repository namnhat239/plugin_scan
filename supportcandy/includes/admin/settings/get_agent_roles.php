	...
	<?php foreach ( $agent_role as $key => $val ) :?>
		<li class="ui-state-default">
			<div class="wpsc-flex-container" style="background-color:#1E90FF;color:#fff;">
				<div class="wpsc-sortable-label"><?php echo esc_attr($val['label'])?></div>
				<div class="wpsc-sortable-edit" onclick="wpsc_get_edit_agent_role(<?php echo esc_attr($key)?>);" style="margin-left:40px;"><i class="fa fa-edit"></i></div>
				<div class="wpsc-sortable-delete" onclick="wpsc_delete_agent_role(<?php echo esc_attr($key)?>);"><i class="fa fa-trash"></i></div>
			</div>
		</li>
	<?php endforeach;?>
</ul>
	...
	function wpsc_delete_agent_role(role_id){
		...
		if (flag) {
			var data = {
				...
				role_id : role_id,
				_ajax_nonce: '<?php echo wp_create_nonce('delete_agent_role')?>'
			};
