	...
	<?php foreach ( $fields as $field ) :
		...
		<li class="ui-state-default" data-id="<?php echo esc_attr($field->term_id)?>">
			<div class="wpsc-flex-container" style="background-color:#1E90FF;color:#fff;">
				...
				<div class="wpsc-sortable-delete" onclick="wpsc_delete_agent_list_field(<?php echo esc_attr($field->term_id)?>);" style="padding-left:30px;"><i class="fa fa-trash"></i></div>
			</div>
		</li>
	<?php endforeach;?>
</ul>
...
<script>
    ...
    jQuery( ".wpsc-sortable" ).sortable({ handle: '.wpsc-sortable-handle' });
		jQuery( ".wpsc-sortable" ).on("sortupdate",function(event,ui){
			...
			field_ids : ids,
			_ajax_nonce: '<?php echo wp_create_nonce('set_agent_list_order')?>'
		  };
	...
	function wpsc_set_add_agent_list_field(){
		...
		var data = {
			...
			_ajax_nonce: '<?php echo wp_create_nonce('set_add_agent_list_field')?>',
			field_id : field_id
		};
		jQuery.post(wpsc_admin.ajax_url, data, function(response_str) {
			wpsc_modal_close();
  ...
  function wpsc_delete_agent_list_field(field_id){
		...
		if (flag) {
			var data = {
				...
				_ajax_nonce: '<?php echo wp_create_nonce('delete_agent_list_field')?>',
				field_id : field_id
			};
			jQuery.post(wpsc_admin.ajax_url, data, function(response_str) {
				var response = JSON.parse(response_str);
