...
<form id="frm_get_ticket_change_status" method="post">
	...
	<div class="form-group">
      ...
      foreach ( $statuses as $status ) :
				...
				echo '<option '.esc_attr($selected).' value="'.esc_attr($status->term_id).'">'.esc_attr($wpsc_custom_status_localize['custom_status_'.$status->term_id]).'</option>';
			endforeach;
			?>
		</select>
	</div>
	...
	<div class="form-group">
		...
		<select class="form-control" name="category" >
			...
			foreach ( $categories as $category ) :
				...
				echo '<option '.esc_attr($selected).' value="'.esc_attr($category->term_id).'">'.esc_attr($wpsc_custom_category_localize['custom_category_'.$category->term_id]).'</option>';
			endforeach;
			?>
		</select>
	</div>
	...
	<div class="form-group">
		...
		<select class="form-control" name="priority">
			...
			foreach ( $priorities as $priority ) :
				...
				echo '<option '.esc_attr($selected).' value="'.esc_attr($priority->term_id).'">'.esc_attr($wpsc_custom_priority_localize['custom_priority_'.$priority->term_id]).'</option>';
			endforeach;
			?>
		</select>
	</div>
  ...
  <input type="hidden" name="_ajax_nonce" value="<?php echo wp_create_nonce('set_change_ticket_status')?>">
...
<button type="button" class="btn wpsc_popup_action" style="background-color:<?php echo esc_attr($wpsc_appearance_modal_window['wpsc_action_button_bg_color'])?> !important;color:<?php echo esc_attr($wpsc_appearance_modal_window['wpsc_action_button_text_color'])?> !important;" onclick="wpsc_set_change_ticket_status(<?php echo esc_attr($ticket_id)?>);"><?php _e('Save','supportcandy');?></button>
<?php
$footer = ob_get_clean();
