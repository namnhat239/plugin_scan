...
<form id="frm_get_ticket_fields" method="post">
	<div id="wpsc_search_cf" class="row form-group col-md-4 col-xs-12" style="float:right;">	
		<input type="text" id="search_cf" class="form-control" placeholder="<?php _e('Search','supportcandy') ?>" name="search_cf" autocomplete="off">
	</div>
	<div class="row" id="wpsc_edit_form_fields" style="clear:both">
	...
	jQuery('#search_cf').keyup(function(e) {
		...
		jQuery('#wpsc_edit_form_fields > .wpsc_form_field').each(function(){
...
<button type="button" class="btn wpsc_popup_action" style="background-color:<?php echo esc_attr($wpsc_appearance_modal_window['wpsc_action_button_bg_color'])?> !important;color:<?php echo esc_attr($wpsc_appearance_modal_window['wpsc_action_button_text_color'])?> !important;" onclick="wpsc_set_change_ticket_fields(<?php echo esc_attr($ticket_id) ?>);"><?php _e('Save','supportcandy');?></button>
<?php
$footer = ob_get_clean();
