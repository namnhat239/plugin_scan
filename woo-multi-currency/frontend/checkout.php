...
class WOOMULTI_CURRENCY_F_Frontend_Checkout {
	...
	public function woocommerce_checkout_update_order_review( $data ) {
		...
		if ( $payment_method === 'ppcp-gateway' ) {
			...
			if ( ! $this->settings->get_enable_multi_payment() && ( $this->settings->get_current_currency() !== $default_currency || $this->settings->getcookie( 'wmc_current_currency_old' ) !== $default_currency ) ) {
