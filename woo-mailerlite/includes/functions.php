 ...
 * @return mixed
 */
function woo_ml_get_orders_by_email($email, $args = array()) {
 ...
 */
function woo_ml_reset_tracked_orders($recreate = true)
{

    $finished = woo_ml_reset_tracked_orders_process( $recreate );
 ...
 */
function woo_ml_reset_tracked_orders_process($recreate = true)
{
    set_time_limit(1800);

    $mailerliteClient = new PlatformAPI( MAILERLITE_WP_API_KEY );

    // Remove shop and recreate
    if ($mailerliteClient->getApiType() === ApiType::REWRITE) {

        $shop = get_option('woo_ml_shop_id', false);

        if ($shop !== false && $recreate === true) {
            if ($mailerliteClient->deleteShop($shop) === true) {
                ...
                $result = $mailerliteClient->createShop($shop_name, $url, $currency, $group_id, $popups_enabled, '');

                if (isset($result->id) && $mailerliteClient->responseCode() === 201) {
...
function woo_ml_sync_untracked_orders() {
    ...
    try {
        ...
        if ( is_array( $checkOrders ) && sizeof( $checkOrders ) > 0 ) {
            foreach ($checkOrders as $checkOrderPost) {
                ...
                if (!isset($data[$checkOrderEmail])) {
                    ...
                    $orders = woo_ml_get_orders_by_email($checkOrderEmail);
            ...
            foreach ($data as $customer_email => $order_ids) {
                ...
                if ( $mailerliteClient->getApiType() === ApiType::REWRITE ) {
                    ...
                    $synced_products = woo_ml_get_tracked_products();
    ...
    try {
        ...
        if ( is_array( $checkProducts ) && sizeof( $checkProducts ) > 0 ) {
            ...
            foreach ($checkProducts as $post) {
                ...
                $request = [
                    'method' => 'PUT',
                    'path' => 'api/ecommerce/shops/' . $shop . '/products/' . $productID . '/categories/multiple/multiple?with_resource_id',
                    'body' => [
                        'replace' => true,
            ...
            if (count($syncProducts) > 0) {
                ...
                if ( $mailerliteClient->responseCode() == 201 || $mailerliteClient->responseCode() == 200 ) {
                    ...
                    $batches = array_chunk($batchRequests, 50);

                    foreach ($batches as $requests) {

                        $batchResult = $mailerliteClient->batch([ 'requests' => $requests ]);

                        if ($batchResult === false || $mailerliteClient->responseCode() != 200) {
                        ...
                        foreach($batchResult as $batchResponse) {

                            if ($batchResponse->code == 200) {

                                woo_ml_complete_product_tracking($batchResponse->body->data->resource_id);
                            }
                        }
                    }
                }
...
function woo_ml_get_checkout_data($cookie_email = null, $subscribe = null)
{
    ...
    if (! empty($customer_email)) {
        ...
        foreach($cart_items as $key => $value) {
            $line_items[] = $value;
            $total += intval($value['quantity']) * floatval($value['data']->get_price());
        }
 ...
 * @return mixed|null
 */
function woo_ml_product_image($product, $size = 'thumbnail')
{
