...
function wcd_get_wishlist( $user_id = 0 ) {
	...
	elseif( isset( $_COOKIE[ $_wishlist_key ] ) && is_array( $_COOKIE[ $_wishlist_key ] ) ) {
		$_wishlist = array_map( 'codesigner_sanitize_number', json_decode( stripslashes( sanitize_key( $_COOKIE[ $_wishlist_key ] ) ), ARRAY_N ) );
	}
