...
class Smart_SEO_Tool_Common {
	...
	public static function detect_broken_url() {
		...
		$list = $wpdb->get_results( "SELECT * FROM $t WHERE url_md5 IS NOT NULL AND (check_date IS NULL OR DATE_ADD(check_date ,INTERVAL $day DAY) < NOW() ) LIMIT 30 " );
	...
	public static function broken_url_count() {
		...
		$t    = $wpdb->prefix . 'wb_sst_broken_url';
		$list = $wpdb->get_results( "SELECT COUNT(1) num,`code` FROM $t WHERE url_md5 IS NOT NULL GROUP BY `code` " );
		foreach ( $list as $r ) {
		    if(!$r->code){
                $sum['check'] += $r->num;
