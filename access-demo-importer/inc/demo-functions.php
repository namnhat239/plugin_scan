...
if ( ! class_exists( 'ADI_Demos' ) ) {
	...
	class ADI_Demos {
		...
		public function __construct() {
			...
			if ( ! is_admin() || ! current_user_can('install_plugins') || is_customize_preview() ) {
				return;
			}
		...
		public function init() {
			...
			// Smart Slider
			// add_action( 'wp_ajax_adi_ajax_importSliderSmart', array($this, 'ajax_importSliderSmart') );
		...
		public static function get_demos_data() {
			...
			if( !current_user_can('install_plugins') )
				die('Access Denied. You do not have required capability.');
...
// 			$git_url  	= 'https://raw.githubusercontent.com/WPaccesskeys/WPaccesskeys.github.io/master/theme-demos/'.get_template().'-demos/config.json';
			$config_url  	= 'https://accesspressthemes.com/theme-demos/'.get_template().'-demos/config.json';
			
			$config_url 	= apply_filters('adi_config_location', $config_url );

			$data 		= ADI_Demos_Helpers::get_remote( $config_url );
		...
		public static function ajax_demo_data() {
			...
			if ( !current_user_can('import') || !current_user_can('install_plugins') || ! wp_verify_nonce( $_GET['demo_data_nonce'], 'get-demo-data' ) ) {
			...
			$smart_slider   = isset($demos[$demo]['smart_slider']) 		? $demos[$demo]['smart_slider'] 	: '';
					...
					<ul class="adi-popup-text ap-importer-choices">
						...
						<li>
							<label for="adi_import_sliders">
								...
								<strong><?php esc_html_e( 'Import Revolution Slider', 'access-demo-importer' ); ?></strong>
							</label>
						</li>
						<?php endif; ?>
						<!--<?php if($smart_slider): ?>
						<li>
							<label for="adi_import_smartsliders">
								<input id="adi_import_smartsliders" type="checkbox" name="adi_import_smartsliders" checked="checked" />
								<strong><?php esc_html_e( 'Import Smart Slider', 'access-demo-importer' ); ?></strong>
							</label>
						</li>
						<?php endif; ?> -->
						<?php if($import_redux): ?>
		...
		*/
		public function plugin_offline_installer_callback() {
			if ( !is_admin() || !current_user_can('install_plugins') || !wp_verify_nonce( $_POST['adi_install_pro_plugin_nonce'], 'adi_install_pro_plugin_nonce' ) ) {
				die( 'This action was stopped for security purposes.' );
			}
			$plugin = array();
		...
		public function ajax_required_plugins_activate() {
			check_ajax_referer( 'adi_activate_req_plugin_nonce', 'security' );
		...
		public function ajax_get_import_data() {
			...
			if ( ! current_user_can( 'import' ) )
				die('Access denied. You do not have import capability.');
			...
			echo json_encode( 
				array(
					...
					array(
						...
						'loader' 		=> esc_html__( 'Importing Revolution Slider', 'access-demo-importer' )
					),

					// array(
					// 	'input_name' 	=> 'adi_import_smartsliders',
					// 	'action' 		=> 'adi_ajax_importSliderSmart',
					// 	'method' 		=> 'ajax_importSliderSmart',
					// 	'loader' 		=> esc_html__( 'Importing Smart Slider', 'access-demo-importer' )
					// ),
		...
		public function ajax_import_xml() {
			if ( !current_user_can('import') || ! wp_verify_nonce( $_POST['adi_import_demo_data_nonce'], 'adi_import_demo_data_nonce' ) ) {
				die( 'This action was stopped for security purposes.' );
			}
		...
		public function ajax_import_theme_settings() {
			if ( !current_user_can('edit_theme_options') || ! wp_verify_nonce( $_POST['adi_import_demo_data_nonce'], 'adi_import_demo_data_nonce' ) ) {
				die( 'This action was stopped for security purposes.' );
			}
        ...
        public function ajax_importSliderRev() {
			...
			$upload_dir =  wp_upload_dir();
			$zip_dir = $upload_dir['basedir'].'/adi-temp-data';
			if ( ! file_exists( $zip_dir ) ) {
        		wp_mkdir_p( $zip_dir );
    		}
    		$slider_temp_zip = $zip_dir.'/revslider_temp.zip';
    		if ( ! file_exists( $slider_temp_zip ) ) {
    			$zip = new ZipArchive();
				if( $zip->open( $slider_temp_zip, ZIPARCHIVE::CREATE ) ){
					file_put_contents( $slider_temp_zip, $response );
				}
    		} else {
    			file_put_contents( $slider_temp_zip, $response );
    		}
            ...
            if ( class_exists( 'RevSlider' ) ) {
                ...
                $slider->importSliderFromPost( true, true, $slider_temp_zip );

                echo 'successful import';
                file_put_contents( $slider_temp_zip, '' );
        ...
        public function ajax_importSliderSmart() {
        	if ( !current_user_can('import') || ! wp_verify_nonce( $_POST['adi_import_demo_data_nonce'], 'adi_import_demo_data_nonce' ) ) {
				die( 'This action was stopped for security purposes.' );
			}

			// Include form importer
			include ADI_PATH . '/inc/importers/class-smartslider-importer.php';

			// Get the selected demo
			$demo_type 			= sanitize_text_field(wp_unslash($_POST['adi_import_demo']));
			$demo 				= ADI_Demos::get_demos_data()[ $demo_type ];
			$slider_file 		= isset( $demo['smart_slider'] ) ? $demo['smart_slider'] : '';
			$response 			= ADI_Demos_Helpers::get_remote( $slider_file );

			// No sample data found
			if ( $response === false ) {
				return new WP_Error( 'xml_import_error', __( 'Cannot retrieve slider data. The server may be down at the moment please try again later. If you still have issues contact the theme developer for assistance.', 'access-demo-importer' ) );
			}

			// Import settings.
			$slider_importer = new ADI_SmartSlider_Importer();
			$result = $slider_importer->process_import_file( $slider_file );

		    // Clear sample data content from temp slider file
		    $temp_slider = ADI_PATH.'/temp-data/slider.ss3';
	    ...
	    */
        public function ajax_importThemeOptions() {
        	if ( !current_user_can('edit_theme_options') || ! wp_verify_nonce( $_POST['adi_import_demo_data_nonce'], 'adi_import_demo_data_nonce' ) ) {
				die( 'This action was stopped for security purposes. You do not have capabitlity to edit theme options' );
			}
		...
		public function ajax_after_import() {
			if ( !current_user_can('install_plugins') || !current_user_can('edit_theme_options') || !current_user_can('manage_options') || ! wp_verify_nonce( $_POST['adi_import_demo_data_nonce'], 'adi_import_demo_data_nonce' ) ) {
				die( 'This action was stopped for security purposes.' );
			}
		...
		public function process_xml( $file ) {
			if( !current_user_can('install_plugins') )
				die('Access Denied. You do not have required capability to install plugins.');
			...
			// $temp_xml = 'https://accesspressthemes.com/theme-demos/temp-data/temp.xml';
			file_put_contents( $temp_xml, $response );

			// Set temp xml to attachment url for use
			$attachment_url = $temp_xml;
			$file_headers = @get_headers($attachment_url);
		...
		private function import_xml( $file ) {
			...
			if( !current_user_can('import') )
				die('You do not have required capability to import.');
