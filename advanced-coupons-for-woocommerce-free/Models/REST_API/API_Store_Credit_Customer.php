...
class API_Store_Credit_Customer implements Model_Interface
{
    ...
    public function get_store_credit_statistics($request)
    {
        ...
        $start_period = $request->get_param('startPeriod');
        $end_period   = $request->get_param('endPeriod');

        $unclaimed    = \ACFWF()->Store_Credits_Calculate->calculate_unclaimed_store_credits();
        $period_stats = \ACFWF()->Store_Credits_Calculate->calculate_store_credits_period_statistics($start_period, $end_period);
