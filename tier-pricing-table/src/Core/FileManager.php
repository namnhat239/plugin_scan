...
class FileManager {
	...
	public function includeTemplate( $__template, array $__variables = array() ) {
		...
		if ( $__template ) {
			...
			do_action( 'tier_pricing_table/template/before_render', $__template, $__variables );
			include( $__template );
			do_action( 'tier_pricing_table/template/after_render', $__template, $__variables );
		}
	}
	...
	public function locateTemplate( $template ) {
		...
		return apply_filters( 'tier_pricing_table/template/location', $file, $template );
	}
