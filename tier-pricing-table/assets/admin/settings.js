jQuery(document).ready(function ($) {
    var TieredPricingSettings = function () {
        this.init = function () {
            ...
            this.$priceAtCatalogType = this.getRow('tiered_price_at_catalog_type');

            this.$showSummary.on('change', (function () {

                this.$summaryType.trigger('change');

                if (this.$showSummary.is(':checked')) {
            ...
            this.$summaryType.on('change', (function () {
                ...
                if (this.$summaryType.val() === 'inline' && this.$showSummary.is(':checked')) {
            ...
            this.$priceAtCatalogType.on('change', (function (e) {
                if ($(e.target).val() === 'lowest' && this.$displayAtCatalog.is(':checked')) {
            ...
            this.$displayAtCatalog.on('change', (function () {
                if (this.$displayAtCatalog.is(':checked')) {
                    this.showRow(this.getRow('lowest_prefix'));
                    this.showRow(this.$priceAtCatalogType);
                    this.showRow(this.getRow('tiered_price_at_catalog_for_variable'));
                    this.showRow(this.getRow('tiered_price_at_product_page'));
                    this.showRow(this.getRow('tiered_price_at_catalog_cache_for_variable'));
                } else {
                    this.hideRow(this.$priceAtCatalogType);
                    this.hideRow(this.getRow('tiered_price_at_catalog_for_variable'));
                    this.hideRow(this.getRow('tiered_price_at_product_page'));
                    this.hideRow(this.getRow('lowest_prefix'));
                    this.hideRow(this.getRow('tiered_price_at_catalog_cache_for_variable'));

                }

                this.$priceAtCatalogType.trigger('change');
            ...
            this.$changePriceAtProductPage.on('change', (function () {
                if (this.$changePriceAtProductPage.is(':checked')) {
