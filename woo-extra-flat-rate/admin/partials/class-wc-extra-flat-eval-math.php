						...
						if ( preg_match( "/^([A-Za-z]\w*)\($/", $stack->last( 2 ), $matches ) ) { // did we just close a function?
							...
							if ( in_array( $fnn, self::$fb, true ) ) { // check the argument count
								...
								if ( $arg_count !== count( self::$f[ $fnn ]['args'] ) ) {
									return wp_die( sprintf( esc_html__( 'wrong number of arguments (%d given, %d expected', 'advanced-flat-rate-shipping-for-woocommerce' ), esc_attr( $arg_count ), esc_attr( count( self::$f[ $fnn ]['args'] ) ) ) );
								}
