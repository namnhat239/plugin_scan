...
class TeamWidget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$fName        = in_array( 'name', $settings['detail_page_fields'], true ) ? true : false;
		$fDesignation = in_array( 'designation', $settings['detail_page_fields'], true ) ? true : false;
		$fShort_bio   = in_array( 'short_bio', $settings['detail_page_fields'], true ) ? true : false;
		$fSocial      = in_array( 'social', $settings['detail_page_fields'], true ) ? true : false;
		...
		if ( $teamQuery->have_posts() ) {
			while ( $teamQuery->have_posts() ) :
				...
				if ( $designation && $fDesignation ) {
					$html .= "<h4 class='designation'>$designation</h4>";
				}

				if ( $bio && $fShort_bio ) {
					$html .= "<div class='designation'>$bio</div>";
				}
				...
				if ( ! empty( $sLink ) && is_array( $sLink ) && $fSocial ) {
					...
					foreach ( $sLink as $id => $link ) {
							@$html .= "<li><a class='fa fa-{$link['id']}' href='{$link['url']}' title='{$link['id']}' target='_blank'></a></li>";
					}
					$html .= '</ul>';
				}
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'tlp-team' ); ?></label>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'member' ) ); ?>"><?php esc_html_e( 'Number of member to show:', 'tlp-team' ); ?></label>
			<input type="text" size="2" id="<?php echo esc_attr( $this->get_field_id( 'member' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'member' ) ); ?>" value="<?php echo esc_attr( $instance['member'] ); ?>" /></p>
		<?php
	}
