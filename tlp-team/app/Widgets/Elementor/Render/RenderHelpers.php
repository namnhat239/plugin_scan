...
class RenderHelpers {
	...
	public static function renderPagination( $wpQuery, $meta, $limit, $perPage, $page_num = null ) {
		...
		if ( $htmlUtility ) {
			...
			if ( 'layout-el-4' === $meta['layout'] ) {
				$l4toggle = 'data-l4toggle="' . $l4toggle . '"';
			}

			$html .= '<div class="rt-pagination-wrap" data-total-pages="' . $totalPage . '" data-posts-per-page="' . $postPp . '" data-type="' . $meta['posts_loading_type'] . '" ' . $l4toggle . '>' . $htmlUtility . '</div>';
		}
