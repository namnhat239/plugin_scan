...
class SinglePopup {
	...
	protected function init() {
		add_action( 'wp_ajax_tlp_md_popup_single', array( $this, 'response' ) );
		add_action( 'wp_ajax_nopriv_tlp_md_popup_single', array( $this, 'response' ) );
	}
	...
	public function response() {
		...
		if ( isset( $_REQUEST['id'] ) && $post_id = absint( $_REQUEST['id'] ) ) {
			...
			if ( $post && $post->post_type == rttlp_team()->post_type ) {
				...
				$tlp_skill                = $tlp_skill ? unserialize( $tlp_skill ) : array();
				$sLink                    = get_post_meta( $post->ID, 'social', true );
				$sLink                    = $sLink ? $sLink : array();
