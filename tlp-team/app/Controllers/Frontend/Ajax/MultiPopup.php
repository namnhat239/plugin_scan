...
class MultiPopup {
	...
	protected function init() {
		add_action( 'wp_ajax_tlp_multi_popup_single', array( $this, 'response' ) );
		add_action( 'wp_ajax_nopriv_tlp_multi_popup_single', array( $this, 'response' ) );
	}
		...
		if ( isset( $_REQUEST['id'] ) && $post_id = absint( $_REQUEST['id'] ) ) {
			...
			if ( $post && $post->post_type == rttlp_team()->post_type ) {
				...
				if ( $tlp_member_content && in_array( 'content', $fields ) ) {
					$html .= '<div class="tlp-member-detail">' . $tlp_member_content . '</div>';
				}
