...
class SpecialLayout {
	...
	protected function init() {
		add_action( 'wp_ajax_rtGetSpecialLayoutData', array( $this, 'response' ) );
		add_action( 'wp_ajax_nopriv_rtGetSpecialLayoutData', array( $this, 'response' ) );
	}
	...
	public function response() {
		...
		if ( $memberId ) {
			...
			if ( $name && in_array( 'name', $fields ) ) {
				if ( $link ) {
					$htmlName = "<h3><a class='{$anchorClass}' data-id='{$memberId}' href='{$pLink}'>{$name}</a></h3>";
				} else {
					$htmlName = "<h3>{$name}</h3>";
				}
			}
			...
			if ( $designation && in_array( 'designation', $fields ) ) {
				if ( $link ) {
					$htmlDesignation = "<div class='tlp-position'><a class='{$anchorClass}' data-id='{$memberId}' href='{$pLink}'>{$designation}</a></div>";
				} else {
					$htmlDesignation = "<div class='tlp-position'>{$designation}</div>";
				}
			}

			if ( $short_bio && in_array( 'short_bio', $fields ) ) {
				$htmlShortBio = "<div class='special-selected-short-bio short-bio'><p>{$short_bio}</p></div>";
			}

			if ( $email && in_array( 'email', $fields ) ) {
				$htmlCInfo .= "<li class='tlp-email'><i class='far fa-envelope'></i> <a href='mailto:{$email}'><span>{$email}</span></a> </li>";
			}

			if ( $telephone && in_array( 'telephone', $fields ) ) {
				$htmlCInfo .= "<li class='tlp-phone'><i class='fa fa-phone'></i> <a href='tel:{$telephone}'>{$telephone}</a></li>";
			}

			if ( $fax && in_array( 'fax', $fields ) ) {
				$htmlCInfo .= "<li class='tlp-fax'><i class='fa fa-fax'></i> <a href='fax:{$fax}'> <span>{$fax}</span> </a> </li>";
			}

			if ( $mobile && in_array( 'mobile', $fields ) ) {
				$htmlCInfo .= "<li class='tlp-mobile'><i class='fa fa-mobile'></i> <a href='tel:{$mobile}'><span>{$mobile}</span></a> </li>";
			}

			if ( $location && in_array( 'location', $fields ) ) {
				$htmlCInfo .= "<li class='tlp-location'><i class='fa fa-map-marker'></i>  <span>{$location}</span> </li>";
			}

			if ( $web_url && in_array( 'web_url', $fields ) ) {
				$htmlCInfo .= "<li class='tlp-web-url'><i class='fa fa-globe'></i>  <a href='{$web_url}'>{$web_url}</a> </li>";
			}
