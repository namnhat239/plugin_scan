...
class ShortcodeMeta {
	...
	protected function init() {
		...
		if ( ( isset( $_GET['post'] ) && get_post_type( $_GET['post'] ) == 'team-sc' ) ||
			( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'team-sc' ) ||
			( isset( $_GET['post'] ) && get_post_type( $_GET['post'] ) == 'team' ) ||
			( isset( $_GET['post_type'] ) && $_GET['post_type'] == 'team' ) ||
			( isset( $_GET['page'] ) && $_GET['page'] == 'tlp_team_settings' )
		) {
	...
	public function remove_all_meta_boxes_team_sc() {
		...
		$wp_meta_boxes[ rttlp_team()->shortCodePT ] = array(
	...
	public function save_team_sc_meta_data( $post_id, $post, $update ) {
		...
		// error_log( print_r( 'Save metadata' , true ), 3, __DIR__ . '/log.txt' );
		// save current tab
