...
class Review {
	...
	protected function init() {
		add_action( 'admin_init', array( __CLASS__, 'rtteam_check_installation_time' ) );
		add_action( 'admin_init', array( __CLASS__, 'rtteam_spare_me' ), 5 );
	}
	...
	public static function rtteam_check_installation_time() {
		...
		$now         = strtotime( 'now' );
		if ( $now >= $remind_due ) {
			add_action( 'admin_notices', array( __CLASS__, 'rtteam_display_admin_notice' ) );
	...
	public static function rtteam_spare_me() {
		...
		if ( isset( $_GET['rtteam_remind_me'] ) && ! empty( $_GET['rtteam_remind_me'] ) ) {
			$remind_me = $_GET['rtteam_remind_me'];
			if ( 1 == $remind_me ) {
		...
		if ( isset( $_GET['rtteam_rated'] ) && ! empty( $_GET['rtteam_rated'] ) ) {
			$rtteam_rated = $_GET['rtteam_rated'];
			if ( 1 == $rtteam_rated ) {
