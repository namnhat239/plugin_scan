...
class Custom_Traffic_Column {
	...
	public function add_post_columns() : void {
		...
		add_action( 'manage_pages_columns', [ $this, 'set_custom_columns' ], 10, 2 );
		add_action( 'manage_posts_columns', [ $this, 'set_custom_columns' ], 10, 2 );
		...
		// For Categories.
		add_action( 'manage_edit-category_columns', [ $this, 'set_custom_columns_tax' ] );
		add_action( 'manage_category_custom_column', [ $this, 'custom_column_category' ], 10, 3 );
		// For Product Categories.
		if ( ! ( new Ahrefs_Seo_Content() )->is_disabled_for_products() ) {
			add_action( 'manage_edit-product_cat_columns', [ $this, 'set_custom_columns_tax' ] );
			add_action( 'manage_product_cat_custom_column', [ $this, 'custom_column_product_cat' ], 10, 3 );
		}
	}
	 ...
	 * @param string                $post_type Curent post type.
	 ...
	 */
	public function custom_column( $column, $post_id, $taxonomy = '' ) : void {
		// Callback. Do not use parameter types.
		switch ( $column ) {
			case 'ahrefs_organic':
				...
				if ( ! $post_tax->is_tax_or_published() || is_null( $traffic ) || ( is_null( $traffic['organic_month'] ) && is_null( $traffic['organic_total'] ) ) || ( $traffic['organic_month'] < 0 ) || ( $traffic['organic_total'] < 0 ) ) {
					...
					<div class="ahrefs-traffic-wrapper">
						...
						<span class="screen-reader-text"><?php esc_html_e( 'No info', 'ahrefs-seo' ); ?></span>
					</div>
					<?php
				} else {
					?>
					<div class="ahrefs-traffic-wrapper">
						<span class="ahrefs_traffic_month"><span class="ahrefs_value"><?php echo esc_html( $traffic['organic_month'] ?? '—' ); ?></span> <?php esc_html_e( '/month', 'ahrefs-seo' ); ?></span>
						<span class="ahrefs_traffic_total"><?php esc_html_e( 'Total:', 'ahrefs-seo' ); ?> <span class="ahrefs_value"><?php echo esc_html( $traffic['organic_total'] ?? '—' ); ?></span></span>
					</div>
					<?php
				}
