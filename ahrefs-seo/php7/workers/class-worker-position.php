...
class Worker_Position extends Worker_GSC {
	 ...
	 * @return void
	 */
	public function update_posts_info( array $post_taxes, bool $fast_update = false ) : void {
		if ( ! is_null( $this->snapshot_id ) ) {
			...
			foreach ( $post_taxes as $post_tax ) {
				...
				if ( ! empty( $post_tax_kw->get_keyword_curent() ) ) {
			...
			if ( count( $post_taxes_kw ) ) {
				...
				foreach ( $post_taxes_kw as $post_tax_kw ) {
					$row   = $post_tax_kw->get_keywords_pos() ?? [];
					...
					if ( ! is_null( $data ) ) {
						...
						$this->content_audit->update_position_values( $post_tax_kw, Ahrefs_Seo_Data_Content::POSITION_MAX, sprintf( '[GSC] URL %s has no position info for keyword "%s"', $post_tax_kw->get_url(), (string) $post_tax_kw->get_keyword_curent() ) );
					}
