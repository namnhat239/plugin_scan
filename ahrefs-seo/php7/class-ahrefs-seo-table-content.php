...
class Ahrefs_Seo_Table_Content extends Ahrefs_Seo_Table {
	...
	public const FILTER_KEYWORDS_ALL         = '';
	public const FILTER_KEYWORDS_SUGGESTED   = 0;
	public const FILTER_KEYWORDS_APPROVED    = 1;
	public const FILTER_KEYWORDS_NO_DETECTED = 2;
	public const FILTER_KEYWORDS_DUPLICATED  = 3;
	public const FILTER_KEYWORDS_YOAST       = 4;
	public const FILTER_KEYWORDS_RANKMATH    = 5;
	public const FILTER_KEYWORDS_AIOSEO      = 6;
	...
	public function __construct( $args = [] ) {
		...
		$this->tab_hints = [
			...
			'excluded'         => sprintf(
				...
				$this->prepare_quotes(
					__( 'These pages do not participate in the content audit. Automatically excluded pages are:', 'ahrefs-seo' )
				),
				$this->prepare_quotes(
					implode(
						'</li><li>',
						[
							__( 'Noindex page', 'ahrefs-seo' ),
							__( 'Non-canonical page', 'ahrefs-seo' ),
							__( 'Redirected page', 'ahrefs-seo' ),
							__( 'Out of scope', 'ahrefs-seo' ),
							__( 'Newly published', 'ahrefs-seo' ),
							__( 'Added since last audit', 'ahrefs-seo' ),
							__( 'Error analyzing', 'ahrefs-seo' ),
						]
					)
				),
				$this->prepare_quotes(
	 ...
	 */
	public static function get_tab_names() : array {
		return [
	 ...
	 */
	private function prepare_hint( string $text ) : string {
		return sprintf(
			'<span class="help-small" title="%s">&nbsp;</span>',
			esc_attr( $text )
		);
	}
	 ...
	 */
	public function prepare_quotes( string $html ) : string {
		return str_replace(
			'"',
			'&quot;', // so replace possible double qoute with html entity.
			$html
		);
	}
	...
	public function get_columns() : array {
		$settings_content   = add_query_arg(
			[
				...
				'tab'  => Ahrefs_Seo_Screen_Settings::TAB_CONTENT,
			],
			admin_url( 'admin.php' )
		);
		...
		$columns            = [
			...
			'position'       => sprintf(
				'<span title="%s">%s</span>',
				esc_attr__( 'The average position of this page in search results for a target keyword over the last 3 months. It is retrieved from your Google Search Console account.', 'ahrefs-seo' ),
				'' === $country_code ?
				esc_html_x( 'Position', 'Table column title', 'ahrefs-seo' ) :
				/* Translators: %s: country code */
				sprintf( esc_html_x( 'Position in %s', 'Table column title', 'ahrefs-seo' ), $country_code )
			),
			'total'          => sprintf(
				'<span title="%s">%s</span>',
				sprintf(
			...
			'backlinks'      => sprintf(
				'<span title="%s">%s</span>',
				esc_attr__( 'How many links point to your target in total. Not to be confused with the number of pages linking to your target, as a single page can give multiple backlinks.', 'ahrefs-seo' ),
				esc_html_x( 'Backlinks', 'Table column title', 'ahrefs-seo' )
			),
			'refdomains'     => sprintf(
				'<span title="%s">%s</span>',
				esc_attr__( 'Number of domains containing at least one backlink that links to the target.', 'ahrefs-seo' ),
				esc_html_x( 'Ref. domains', 'Table column title', 'ahrefs-seo' )
			),
			...
			sprintf(
				...
				esc_attr(
					sprintf(
						...
						sprintf(
							"<a href='%s'>%s</a>",
							esc_attr( $settings_content ),
							esc_html__( 'content audit settings', 'ahrefs-seo' )
						)
					)
				),
			...
			sprintf(
				'<span class="show_tooltip" title="" data-tooltip="%s">%s</span>',
				esc_attr__( 'The reason for post being excluded or not analyzed.', 'ahrefs-seo' ),
				esc_html_x( 'Reasons', 'Table column title', 'ahrefs-seo' )
			),
		];
	...
	protected function column_cb( $item ) : void {
		// Note: can not define type of parameters, because not defined in parent class.
		printf(
			'<input type="checkbox" name="link[]" value="%s" data-id="%s" data-ver="%d" %s/>',
			esc_attr( $item->id ),
			esc_attr( $item->id ),
			intval( $item->ver ),
			isset( $item->post_tax ) ? disabled( ! $item->post_tax->user_can_manage(), true, false ) : false
		);
	}
	...
	protected function column_title( \stdClass $item ) : void {
		$title    = $item->title ? $item->title : __( '(no title)', 'ahrefs-seo' );
		$url_edit = '';
		$post_tax = $item->post_tax ?? Post_Tax::create_from_string( (string) $item->id );
	 ...
	 * @return void
	 */
	public static function print_keyword_source_badge( ?string $keyword, ?string $is_approved_keyword, ?string $kw_low, ?string $kw_source ) : void {
		if ( Sources::SOURCE_YOASTSEO === $kw_source ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-imported <?php echo esc_attr( 'badge-keyword-yoast' ); ?>">✓ <?php /* Translators: as short text as possible */ echo esc_html_x( 'PULLED FROM YOAST', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		} elseif ( Sources::SOURCE_AIOSEO === $kw_source ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-imported <?php echo esc_attr( 'badge-keyword-aioseo' ); ?>">✓ <?php /* Translators: as short text as possible */ echo esc_html_x( 'PULLED FROM AIOSEO', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		} elseif ( Sources::SOURCE_RANKMATH === $kw_source ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-imported <?php echo esc_attr( 'badge-keyword-rankmath' ); ?>">✓ <?php /* Translators: as short text as possible */ echo esc_html_x( 'PULLED FROM RANKMATH', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		} elseif ( $kw_low && empty( $keyword ) ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-too-low"><?php /* Translators: as short text as possible */ echo esc_html_x( 'CONTENT LENGTH TOO SHORT', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		} elseif ( ! $is_approved_keyword && empty( $keyword ) ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-empty"><?php /* Translators: as short text as possible */ echo esc_html_x( 'NO KEYWORD DETECTED', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		} elseif ( $is_approved_keyword ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-approved">✓ <?php /* Translators: as short text as possible */ echo esc_html_x( 'APPROVED', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		} elseif ( ! is_null( $keyword ) && ( '' !== $keyword ) ) {
			?>
			<a href="#" class="badge-keyword badge-keyword-suggested"><?php /* Translators: as short text as possible */ echo esc_html_x( 'SUGGESTED KEYWORD', 'Badge title', 'ahrefs-seo' ); ?></a>
			<?php
		}
	}
	...
	protected function column_categories( \stdClass $item ) : void {
		...
		if ( $edit_link ) {
			?>
			<a href="<?php echo esc_attr( $edit_link ); ?>" class="content-post-button" target="_blank"><?php echo esc_html( $badge ); ?></a>
			<?php
		}
	}
	 ...
	 */
	public static function get_post_type_badge( string $badge ) : string {
		switch ( $badge ) {
			case 'post':
	 ...
	 * @param \stdClass $item Item.
	 */
	protected function column_last_well_date( \stdClass $item ) : void {
	 ...
	 * @param \stdClass $item Item.
	 */
	protected function column_refdomains( \stdClass $item ) : void {
	...
	protected function column_action( \stdClass $item ) : void {
		...
		?>
		<a class="status-action <?php echo esc_attr( "status-$action" ); ?> content-more-button button" href="#"><span class="arrow-down"></span><span class="arrow-text"><?php echo esc_html( $title ); ?></span></a>
		<?php
	}
	 ...
	 */
	public static function get_action_title( string $action ) : string {
	 ...
	 * @return void
	 */
	protected static function some_error_message() : void {
		?>
	 ...
	 * @return void
	 */
	protected static function some_empty_message() : void {
		?>
		—
		<?php
	}
	 ...
	 * @return void
	 */
	public static function print_position( ?float $position ) : void {
		if ( ! is_null( $position ) ) {
			if ( $position >= 0 ) {
			...
			if ( $post_tax->user_can_manage() ) {
				...
				if ( in_array( $action, [ Ahrefs_Seo_Data_Content::ACTION4_ADDED_SINCE_LAST ], true ) ) {
					$actions['include'] = sprintf(
						'<a href="%s" data-id="%s" class="submit-include" aria-label="%s">%s</a>',
						'#',
						$item->id,
						esc_attr__( 'Run audit', 'ahrefs-seo' ),
						__( 'Run audit', 'ahrefs-seo' )
					);
					...
					$actions['include'] = sprintf(
						...
						'#',
						$item->id,
						esc_attr__( 'Analyze page again', 'ahrefs-seo' ),
						__( 'Analyze page again', 'ahrefs-seo' )
					);
					...
					$actions['recheck'] = sprintf(
						'<a href="%s" data-id="%s" class="submit-recheck" aria-label="%s">%s</a>',
						'#',
						$item->id,
						esc_attr__( 'Recheck status', 'ahrefs-seo' ),
						__( 'Recheck status', 'ahrefs-seo' )
					);
					$actions['include'] = sprintf(
						'<a href="%s" data-id="%s" class="submit-include" aria-label="%s">%s</a>',
						'#',
						$item->id,
						esc_attr__( 'Include to audit', 'ahrefs-seo' ),
						__( 'Include to audit', 'ahrefs-seo' )
					);
					...
					$actions['exclude'] = sprintf(
						'<a href="%s" data-id="%s" class="submit-exclude" aria-label="%s">%s</a>',
						'#',
						$item->id,
						esc_attr__( 'Exclude from audit', 'ahrefs-seo' ),
						__( 'Exclude from audit', 'ahrefs-seo' )
					);
				}
	...
	protected function months_dropdown_content() : void {
		...
		$m = $this->date;
		?>
		<label for="filter-by-date" class="screen-reader-text"><?php esc_html_e( 'Filter by date', 'ahrefs-seo' ); ?></label>
		<select name="m" id="filter-by-date">
			<option<?php selected( $m, 0 ); ?> value="0"><?php esc_html_e( 'All dates', 'ahrefs-seo' ); ?></option>
			<?php
	...
	protected function authors_dropdown_content() : void {
		...
		?>
		<label class="screen-reader-text" for="author"><?php esc_html_e( 'Filter by author', 'ahrefs-seo' ); ?></label>
		<select name="author" id="author" class="postform">
			<option value="0" class="current"<?php selected( $this->author, 0 ); ?>><?php esc_html_e( 'All authors', 'ahrefs-seo' ); ?></option>
			<?php
			if ( $list ) {
	...
	protected function keywords_dropdown_content() : void {
		?>
		<label class="screen-reader-text" for="keywords"><?php esc_html_e( 'Filter by keywords type', 'ahrefs-seo' ); ?></label>
		<select name="keywords" id="keywords" class="postform">
			<option value="<?php echo esc_attr( self::FILTER_KEYWORDS_ALL ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_ALL ); ?>><?php esc_html_e( 'All keywords', 'ahrefs-seo' ); ?></option>
			<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_APPROVED ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_APPROVED ); ?>><?php esc_html_e( 'Approved by you', 'ahrefs-seo' ); ?></option>
			<?php
			if ( Sources::get()->has_active_source( Sources::SOURCE_YOASTSEO ) ) {
				?>
					<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_YOAST ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_YOAST ); ?>><?php esc_html_e( 'Pulled from Yoast', 'ahrefs-seo' ); ?></option>
					<?php
			}
			if ( Sources::get()->has_active_source( Sources::SOURCE_AIOSEO ) ) {
				?>
					<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_AIOSEO ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_AIOSEO ); ?>><?php esc_html_e( 'Pulled from AIOSEO', 'ahrefs-seo' ); ?></option>
					<?php
			}
			if ( Sources::get()->has_active_source( Sources::SOURCE_RANKMATH ) ) {
				?>
					<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_RANKMATH ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_RANKMATH ); ?>><?php esc_html_e( 'Pulled from RankMath', 'ahrefs-seo' ); ?></option>
					<?php
			}
			?>
			<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_SUGGESTED ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_SUGGESTED ); ?>><?php esc_html_e( 'Suggested by the plugin', 'ahrefs-seo' ); ?></option>
			<?php if ( 'excluded' !== $this->tab ) { // do not show "Duplicated keywords" at the Excluded tab. We do not analyze these content, so it make no sense. ?>
			<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_DUPLICATED ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_DUPLICATED ); ?>><?php esc_html_e( 'Duplicated keywords', 'ahrefs-seo' ); ?></option>
			<?php } ?>
			<option value="<?php echo esc_attr( (string) self::FILTER_KEYWORDS_NO_DETECTED ); ?>" class="current"<?php selected( $this->keywords, self::FILTER_KEYWORDS_NO_DETECTED ); ?>><?php esc_html_e( 'No keyword detected', 'ahrefs-seo' ); ?></option>
		</select>
		<?php
	}
	 ...
	 * @since 0.8.6
	 */
	protected function reasons_dropdown_content() : void {
		$reasons = [
			Ahrefs_Seo_Data_Content::ACTION4_NOINDEX_PAGE,
			Ahrefs_Seo_Data_Content::ACTION4_NONCANONICAL,
			Ahrefs_Seo_Data_Content::ACTION4_REDIRECTED,
			Ahrefs_Seo_Data_Content::ACTION4_OUT_OF_SCOPE,
			Ahrefs_Seo_Data_Content::ACTION4_NEWLY_PUBLISHED,
			Ahrefs_Seo_Data_Content::ACTION4_ADDED_SINCE_LAST,
			Ahrefs_Seo_Data_Content::ACTION4_ERROR_ANALYZING,
			Ahrefs_Seo_Data_Content::ACTION4_MANUALLY_EXCLUDED,
		];
		?>
		<label class="screen-reader-text" for="keywords"><?php esc_html_e( 'Filter by exclusion reason', 'ahrefs-seo' ); ?></label>
		<select name="reason" id="reason" class="postform">
			<option value="<?php echo esc_attr( self::FILTER_KEYWORDS_ALL ); ?>" class="current"<?php selected( $this->reason, '' ); ?>><?php esc_html_e( 'All exclusion reasons', 'ahrefs-seo' ); ?></option>
			<?php
			foreach ( $reasons as $reason ) {
				?>
				<option value="<?php echo esc_attr( $reason ); ?>" class="current"<?php selected( $this->reason, $reason ); ?>><?php echo esc_html( $this::get_action_title( $reason ) ); ?></option>
				<?php
			}
			?>
		</select>
	...
	protected function get_views() {
		...
		foreach ( $this->tabs as $id => $title ) {
			...
			$count            = $counts[ $id ] ?? 0;
			$tab_hint         = $this->tab_hints[ $id ] ?? ''; // html code of hint.
