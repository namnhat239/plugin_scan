...
use ahrefs\AhrefsSeo\Content_Tips\Tip_Suggested_First;
use ahrefs\AhrefsSeo\Content_Tips\Tip_Suggested_Subsequent;
use ahrefs\AhrefsSeo\Content_Tips\Tip_Duplicated;
...
class Ahrefs_Seo_Screen_Content extends Ahrefs_Seo_Screen_With_Table {
	...
	public const ACTION_NEW_AUDIT_RUN = 'ahrefs_seo_screen_new_audit_run';
	public const CSV_TAB_EXPORT       = 'export-csv';
	 ...
	 * @return Ahrefs_Seo_Table_Content
	 */
	protected function new_table_instance() : Ahrefs_Seo_Table_Content {
		...
		if ( ! is_null( $this->table ) ) {
			...
			<div class="wrap content-wrap">
				<form id="content_form" class="table-form wp-clearfix" method="get">
					...
					?>
					<input type="hidden" id="current_lang" value="<?php echo esc_attr( Helper_Content::get()->get_lang( true ) ?? '' ); ?>">
				</form>
			</div>
			<?php
		}
	 ...
	 * @return void
	 */
	private function new_audit_run( bool $exit_with_json_answer ) : void {
		if ( ! current_user_can( Ahrefs_Seo::CAP_CONTENT_AUDIT_RUN ) ) {
	...
	public function ajax_content_ping() : void {
		...
		if ( check_ajax_referer( Ahrefs_Seo_Table::ACTION ) && current_user_can( Ahrefs_Seo::CAP_CONTENT_AUDIT_VIEW ) && isset( $_REQUEST['items'] ) && ( is_array( $_REQUEST['items'] ) || 'false' === $_REQUEST['items'] ) ) {
			...
			$fields['tips'] = $this->prepare_tips( $stop, empty( $_POST['unpause_audit'] ) );
	 ...
	 * @return bool
	 */
	private function is_current_snapshot_different( array $updated_ids ) : bool {
	...
	public function ajax_content_get_keyword_popup() : void {
		...
		if ( check_ajax_referer( Ahrefs_Seo_Table::ACTION ) && current_user_can( Ahrefs_Seo::CAP_CONTENT_AUDIT_VIEW ) && isset( $_REQUEST['post'] ) ) {
	 ...
	 * @return void
	 */
	public function ajax_content_refdomains_update() : void {
	 ...
	 */
	public function bulk_actions( string $doaction, array $post_taxes ) : void {
		$sendback = [];
	 ...
	 */
	protected function prepare_tips( string $stop_displayed = '', bool $hide_compatibility = false ) : array {
		$result = [];
