...
class Ahrefs_Seo_Noindex extends Ahrefs_Seo_Abstract_Api {
	 ...
	 * @return array<string,Result_Noindex> Associave array page_tax_string => Result_Noindex.
	 */
	public function is_noindex( array $page_id_to_url_list ) : array {
		...
		foreach ( $page_id_to_url_list as $post_tax_string => $url ) {
			...
			$results[ $post_tax_string ] = $result ?? ( new Result_Noindex( $post_tax, $url, null ) )->set_is_noindex( 0 ); // indexed by default.
		}
		return $results;
	}
	 ...
	 *
	 * @param array<string,string> $page_id_to_url_list Associative array, page_tax_string => url.
	 * @return array<string, Result_Canonical> Associave array page_tax_string => Result_Canonical.
	 */
	public function is_noncanonical( array $page_id_to_url_list ) : array {
		$results = [];
	 ...
	 *
	 * @param array<string,string> $page_id_to_url_list Associative array, page_tax_string => url.
	 * @return array<string,Result_Redirected> Associave array page_tax_string => Result_Redirected.
	 */
	public function is_redirected( array $page_id_to_url_list ) : array {
		$results = [];
	 ...
	 */
	private function is_same_urls( string $url_a, string $url_b ) : bool {
		return $url_a === $url_b;
	}
}
