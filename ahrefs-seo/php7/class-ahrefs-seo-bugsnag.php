...
class Ahrefs_Seo_Bugsnag {
	...
	public function report_callback( $report ) {
		try {
			...
			$version  = null;
			try {
				$tokens    = Data_Tokens_Storage::OPTION_TOKENS;
				$analytics = Ahrefs_Seo_Analytics::get();
				$advanced  = $analytics->get_adv_options_raw();
				$list      = $analytics->load_gsc_accounts_list( true ); // cached value.
				$version   = '';
				if ( class_exists( '\\ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\Client' ) ) {
					$version = defined( '\\ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\Client::VERSION' ) ? \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Client::VERSION : ( defined( '\\ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\Client::MAJOR_VERSION' ) ? \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Client::MAJOR_VERSION : 'unknown' );
				}
			} catch ( \Exception $e ) {
			...
			$report->setMetaData(
				[
					'info' => [
						...
						'ga-options'     => ! is_null( $tokens ) ? get_option( $tokens, '' ) : null, // @phpstan-ignore-line
						'advanced'       => $advanced,
	 ...
	 * @return void
	 */
	public function set_grouping_hash( \Bugsnag\Report $report ) : void {
		try {
			$api_type = '';
			$methods  = [
	 ...
	 * @return void
	 */
	public function set_grouping_hash_for_error( \Bugsnag\Report $report, array $original_error ) : void {
		try {
	 ...
	 * @return string|null
	 */
	protected function exception_description( \Exception $e ) : ?string {
		$hash = 'other';
		if ( $e instanceof Google_Service_Exception ) {
			$hash = 'google-other';
			/** @var \ahrefs\AhrefsSeo_Vendor\Google_Service_Exception $e */
			$code  = $e->getCode();
			$error = $e->getErrors();
			if ( is_array( $error ) && isset( $error[0] ) && isset( $error[0]['reason'] ) ) {
				$hash = "google-{$code}-{$error[0]['reason']}";
			} elseif ( preg_match( '/<title>Error (\d+)/', $e->getMessage(), $m ) ) { // parse html title.
				$hash = "google-html-{$m[1]}"; // example: 'google-html-502'.
			}
		} elseif ( $e instanceof ConnectException ) {
			/** @var \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Exception\ConnectException $e */
			...
			/** @var \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Exception\RequestException $e */
			$message = $e->getMessage();
