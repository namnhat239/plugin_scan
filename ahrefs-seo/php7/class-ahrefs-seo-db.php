...
class Ahrefs_Seo_Db {
	...
	private const OPTION_COUNTER      = 'ahrefs-seo-db-update-counter';
	private const OPTION_LAST_MESSAGE = 'ahrefs-seo-db-update-message';
	private const OPTION_STOP_ERROR   = 'ahrefs-seo-db-update-stop-error'; // error, preventing plugin from correct work.
	private const TRANSIENT_UPDATING  = 'ahrefs-seo-db-updating';
	private const RESET_AFTER         = 5;
	 ...
	 * @return bool
	 */
	private static function retry_table_update( string $sql, string $table_name ) : bool {
		/** @var \wpdb $wpdb */
		global $wpdb;
	 ...
	 */
	private static function check_stop_error( string $last_error ) : bool {
			...
			self::set_stop_error(
				sprintf(
					...
					sprintf(
						...
						esc_html__( 'Ahrefs support', 'ahrefs-seo' )
					)
				),
				$last_error
			);
	 ...
	 * @return bool Success.
	 */
	private static function reset_before_v0_7() : bool {
		global $wpdb;
	 ...
	 *
	 * @deprecated 0.7.0, used for old options cleaning.
	 *
	 * @param bool   $is_today Is today.
	 * @param string $dofollow Is dofollow link.
	 ...
	 * @return void
	 */
	private static function update_before_v0_8() : void {
		global $wpdb;
	 ...
	 * @return void
	 */
	private static function update_before_v0_8_4() : void {
		global $wpdb;
		...
		if ( is_string( $info['time_end'] ) ) {
			...
			if ( ! is_null( $new_snapshot_id ) && ( $current_snapshot_id !== $new_snapshot_id ) ) { // copy last well date from current snapshot to new, that is being in progress.
	 ...
	 * @return void
	 */
	private static function update_before_v0_8_5() : void {
		global $wpdb;
		$snapshot            = new Snapshot();
		$current_snapshot_id = $snapshot->get_current_snapshot_id();
		$info                = $snapshot->get_snapshot_info( $current_snapshot_id );
	 ...
	 * @return string
	 */
	private static function get_transient_name() : string {
	 ...
	 * @return void
	 */
	private static function counter_reset() : void {
	 ...
	 * @return int
	 */
	private static function counter_get() : int {
	 ...
	 * @return int
	 */
	private static function counter_increase() : int {
	 ...
	 */
	private static function get_last_message() : string {
	 ...
	 * @return void
	 */
	private static function set_stop_error( string $header, string $message ) : void {
		update_option(
	 ...
	 * @return void
	 */
	public static function reset_stop_error() : void {
	 ...
	 * @return string|null
	 */
	private static function get_stop_error() : ?string {
	 ...
	 * @return string|null
	 */
	private static function get_stop_header() : ?string {
		$result = get_option( self::OPTION_STOP_ERROR, null );
