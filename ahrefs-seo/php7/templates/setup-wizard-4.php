...
<form method="post" action="<?php echo esc_attr( add_query_arg( 'page', Ahrefs_Seo::SLUG, admin_url( 'admin.php' ) ) ); ?>" class="ahrefs-seo-wizard ahrefs-audit">
	...
	?>
	<div class="card-item">
		<div class="block-title"><?php esc_html_e( 'Running your first audit', 'ahrefs-seo' ); ?></div>

		<div class="block-text">
			<?php esc_html_e( 'We’re hard at work reviewing all your pages, analyzing your site, and preparing recommendations. Feel free to check back in a few minutes’ time. In the future, this process will run in the background.', 'ahrefs-seo' ); ?>
		</div>
		...
		<div id="progressbar" <?php if ( $finish ) { // phpcs:ignore Squiz.ControlStructures.ControlSignature.NewlineAfterOpenBrace ?> class="completed"<?php } ?>>
			<div class="position"
				...
				if ( $percents > 0 ) {
					?>
					style="<?php echo esc_attr( 'width:' . $percents . '%' ); ?>"<?php } ?>
					data-position="<?php echo esc_attr( $percents > 0 ? $percents : '0' ); ?>"
				>
			</div>
			<div class="progress"><?php echo esc_html( str_replace( '{0}', number_format( $percents, $percents < 10 ? 2 : 1 ), _x( 'Analyzing: {0}%', 'button title', 'ahrefs-seo' ) ) ); ?></div>
			<div class="progress-completed"><?php esc_html_e( 'Completed', 'ahrefs-seo' ); ?></div>
		</div>
	</div>

	<div class="button-wrap">
		<a href="#" class="button button-hero button-primary"
			id="ahrefs_seo_submit"><?php esc_html_e( 'View report', 'ahrefs-seo' ); ?></a>
	</div>
</form>
