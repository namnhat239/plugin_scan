...
<div class="input-wrap block-content scope-block-n post-tax-option">
	<span href="#" class="show-collapsed block-subtitle">
		<label>
			<input type="checkbox" value="1" name="<?php echo esc_attr( $var_enabled_name ); ?>" class="checkbox-main" <?php checked( $is_enabled ); ?>>
			<?php
			echo esc_html( $title );
			?>
						...
						foreach ( $pages_list as $id => $title ) {
							...
							<li>
								<label class="selectit">
									<input type="checkbox" name="pages[]" value="<?php echo esc_attr( "$id" ); ?>" id="<?php echo esc_attr( 'page_' . $id ); ?>" <?php checked( in_array( $id, $id_values, true ) ); ?>>
