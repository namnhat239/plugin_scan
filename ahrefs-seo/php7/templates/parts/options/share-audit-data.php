<?php
/**
 * Settings: My latest audit data part template
 *
 * @var bool $updated
 */
...
<div class="input-wrap block-content">
	<a href="#" class="show-last-audit-data show-collapsed block-subtitle"><?php esc_html_e( 'My latest audit data', 'ahrefs-seo' ); ?></a>
	<div class="collapsed-wrap">
		...
		<div class="download-csv-wrap">
			...
			if ( current_user_can( Ahrefs_Seo::CAP_EXPORT_ZIP ) ) {
				?>
				<a href="<?php echo esc_attr( $is_available ? $url : '#' ); ?>" class="button button-large with-icon download-button block-content<?php echo $is_available ? '' : ' disabled'; ?>"><?php esc_html_e( 'Download', 'ahrefs-seo' ); ?></a>
				<?php if ( ! $is_available ) : ?>
					<div class="notice notice-error">
						<p><strong><?php esc_html_e( 'Class "ZipArchive" not found.', 'ahrefs-seo' ); ?></strong></p>
						<p><?php _e( 'For the <a href="http://www.php.net/manual/en/class.ziparchive.php">ZipArchive class</a> to be present, PHP needs to have the <a href="http://www.php.net/manual/en/book.zip.php">zip extension</a> installed. See the <a href="http://www.php.net/manual/en/zip.installation.php">Installation instructions</a>.', 'ahrefs-seo' ); ?></p>
					</div>
				<?php endif; ?>
				<?php
			} else { // not allowed.
