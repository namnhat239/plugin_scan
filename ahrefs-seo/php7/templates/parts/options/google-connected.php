...
?>
<form method="post" action="" class="ahrefs-seo-wizard ahrefs-analytics <?php echo esc_attr( implode( ' ', $classes ) ); ?>">
		...
		<div class="disconnect-wrap">
			...
			<div class="your-account no-account-detected"><?php esc_html_e( 'Your Google account is connected but no suitable profiles were detected', 'ahrefs-seo' ); ?></div>
				...
				if ( current_user_can( Ahrefs_Seo::CAP_SETTINGS_ACCOUNTS_SAVE ) ) {
					?>
				<a href="<?php echo esc_attr( $locals['disconnect_link'] ); ?>" class="disconnect-button" id="ahrefs_disconnect"><span class="text"><?php esc_html_e( 'Disconnect', 'ahrefs-seo' ); ?></span></a>
					<?php
				}
				?>
		</div>
