...
class Duplicated_Keywords {
	 ...
	 * @return void
	 */
	public function fill_duplicated_for_snapshot( int $snapshot_id ) : void {
		global $wpdb;
	 ...
	 * @return void
	 */
	public function fill_duplicated_for_keyword( int $snapshot_id, string $keyword ) : void {
		global $wpdb;
		Ahrefs_Seo::breadcrumbs( __METHOD__ . (string) wp_json_encode( func_get_args() ) );
	 ...
	 * @return void
	 */
	public function update_is_duplicated( Post_Tax $post_tax, ?string $keyword, bool $is_inactive ) : void {
		Ahrefs_Seo::breadcrumbs( __METHOD__ . (string) wp_json_encode( [ (string) $post_tax, $keyword, $is_inactive ] ) );

		if ( $is_inactive ) {
			$this->reset_for_post( $post_tax );
		} elseif ( is_null( $keyword ) || ( '' === $keyword ) ) { // no keyword set.
	 ...
	 * @return void
	 */
	public function reset_for_post( Post_Tax $post_tax ) : void {
	 ...
	 * @return void
	 */
	public function set_not_duplicated( Post_Tax $post_tax ) : void {
	 ...
	 * @return void
	 */
	private function set( Post_Tax $post_tax, int $value ) : void {
		global $wpdb;
		$wpdb->update(
			$wpdb->ahrefs_content,
			[ 'is_duplicated' => $value ],
	 ...
	 * @return string
	 */
	public function filter_where() : string {
		global $wpdb;
