...
class Snapshot {
	 ...
	 */
	private function add_posts_by_clause( int $snapshot_id, array $params, string $action ) : array {
	 ...
	 */
	private function add_terms_by_clause( int $snapshot_id, string $taxonomy, array $checked ) : array {
		Ahrefs_Seo::breadcrumbs( __METHOD__ . (string) wp_json_encode( func_get_args() ) );
		global $wpdb;
		$results = [];
		$paged   = 0;
		...
		do {
			...
			/** @var array<int,string> we query for [term id => name ] pairs. */
	 ...
	 * @return void
	 */
	public function analysis_approve_items( array $post_taxes, bool $approve = true ) : void {
		$new_snapshot_id     = $this->get_new_snapshot_id();
	 ...
	 */
	public function get_country_code( int $snapshot_id ) : string {
		global $wpdb;
