...
class Export_Audit_Data {
	...
	public const ACTION = 'ahrefs_seo_export';
	 ...
	 * @return string URL.
	 */
	public function get_export_url( bool $export_csv = false, string $tab = '' ) : string {
		return add_query_arg(
			[
				'page'   => $export_csv ? Ahrefs_Seo::SLUG_CONTENT : Ahrefs_Seo::SLUG_SETTINGS,
				'tab'    => $export_csv ? $tab : Ahrefs_Seo_Screen_Settings::TAB_DATA, // fallback to the My audit data page.
	 ...
	 * @return bool
	 */
	public function has_zip_archive() : bool {
	 ...
	 */
	public function export_data_zip() : bool {
	 ...
	 * @return bool False: error, otherwise the execution terminated.
	 */
	public function export_data_tab( string $tab, array $columns ) : bool {
		$this->error_message = null;
		header( 'Cache-Control: no-cache, no-store, must-revalidate' );
		header( 'Pragma: no-cache' );
		header( 'Expires: 0' );
	 ...
	 * @return string
	 */
	private function get_tmp_file_path() : string {
	 ...
	 */
	private function create_and_output_archive() : bool {
	 ...
	 * @return bool True: success, false: error.
	 */
	private function create_and_output_csv( string $tab, array $columns ) : bool {
		global $wpdb;
				...
				foreach ( $data as $item ) {
					if ( is_null( $headers ) ) {
						...
						foreach ( $columns as $id => $title ) {
							switch ( $id ) { // some data may have multiple columns.
								case 'title':
									$headers[] = $title;
					...
					foreach ( $columns as $column_id => $title ) {
						switch ( $column_id ) {
							case 'title':
								$fields[] = $item->title ? $item->title : __( '(no title)', 'ahrefs-seo' );
							...
							case 'keyword':
								...
								$fields[] = mb_strtolower( trim( str_replace( '✓', '', sanitize_text_field( (string) ob_get_clean() ) ) ) );
								break;
							case 'categories':
								$value = '';
	 ...
	 *
	 * @param \ZipArchive $zip_archive Archive instance.
	 * @return string List of temporary files to remove.
	 */
	private function read_snapshots( ?\ZipArchive $zip_archive = null ) : ?string {
		global $wpdb;
		$headers = null;
		$fname   = $this->get_tmp_file_path();
		$fp      = fopen( $fname, 'w' );
	 ...
	 * @return string List of temporary files to remove.
	 */
	private function read_content( int $snapshot_id, ?\ZipArchive $zip_archive = null ) : ?string {
		global $wpdb;
		$headers = null;
		$fname   = $this->get_tmp_file_path();
		$fp      = fopen( $fname, 'w' );
	 ...
	 * @return string
	 */
	private function get_file_name_export_zip() : string {
	 ...
	 * @return string
	 */
	private function get_file_name_export_data_csv( string $tab_title ) : string {
	 ...
	 * @return string
	 */
	public function get_file_name_export_keywords_csv( string $postname ) : string {
	 ...
	 * @return void
	 */
	public function print_keywords_for_textarea( array $items ) : void {
		echo '"' . esc_textarea( __( 'Additional keywords', 'ahrefs-seo' ) ) . '","'
		. esc_textarea( __( 'Position', 'ahrefs-seo' ) ) . '","'
		. esc_textarea( __( 'Clicks', 'ahrefs-seo' ) ) . '","'
		. esc_textarea( __( 'Impressions', 'ahrefs-seo' ) ) . "\"\n";
