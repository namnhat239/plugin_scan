...
class Ahrefs_Seo_Keywords {
	 ...
	 * @return bool
	 */
	public function post_keywords_set( Post_Tax $post_tax, ?Data_Keyword $data_keyword, ?string $keyword_manual = null, bool $reanalyze_everything = true, $changed_by_user = false ) : bool {
		global $wpdb;
	 ...
	 * @return Post_Tax_With_Keywords[]
	 */
	public function get_full_detail_for_posts( array $post_taxes, ?int $limit = null, bool $without_totals = false, $skip_gsc_errors = false, $load_imported_info = false ) : array {
		$v    = func_get_args();
		...
		foreach ( $post_taxes as $post_tax ) {
			...
			if ( $post_tax->exists() && $post_tax->is_tax_or_published() && ( '' !== $post_tax->get_url() ) ) { // only for existing and published posts or terms with non empty url.
				...
				if ( $this->analytics->is_gsc_set() ) {
					if ( ! is_null( $post_tax_kw->get_keyword_curent() ) && ( '' !== $post_tax_kw->get_keyword_curent() ) ) { // current keyword is not empty.
						$request_keywords[] = new Data_Keyword( $post_tax_kw->get_keyword_curent(), null, null, $url, $country_code );
					}
					...
					$requests[ (string) $post_tax_kw ] = new Data_Keyword( '', null, null, $url, $country_code ); // will load suggestions from GSC.
				} else {
	...
	public function get_suggestions( Post_Tax $post_tax, bool $use_cached_data = false ) : array {
		...
		$keyword                = $post_tax_kw->get_keyword_curent() ?? '';
		...
		if ( ! empty( $data_keywords ) && ! empty( $data_keywords['result'] ) ) {
			...
			foreach ( $data_keywords['result'] as $item ) {
				if ( isset( $item['query'] ) && is_string( $item['query'] ) ) {
					$word       = $item['query'];
					$keywords[] = [ false, $item['query'], Sources::SOURCE_GSC, $item['pos'], $item['clicks'], $item['impr'] ];
				}
			}
		}
	...
	public function load_position_value_fast( array $post_taxes ) : array {
		...
		array_walk(
			...
			function( Post_Tax_With_Keywords &$post_tax_kw ) use ( &$url_with_keywords, &$url_to_item ) {
				...
				$keyword_current = $post_tax_kw->get_keyword_curent();
		...
		if ( $url_with_keywords ) { // have something to load?
			...
			if ( $rows ) {
				...
				$by_url = [];
				foreach ( $rows as $key => $data_keyword ) {
				...
				foreach ( $post_taxes_kw as $post_tax_kw ) {
					...
					// check keyword.
					$_keyword_curent = $post_tax_kw->get_keyword_curent();
					if ( ! is_null( $_keyword_curent ) ) {
						$position = $this->load_position_from_caches_raw( $_keyword_curent, $post_tax_kw->get_keywords(), $post_tax_kw->get_keywords_pos() );
						if ( ! is_null( $position ) ) {
	...
	public function load_position_value( array $post_taxes ) : array {
		...
		foreach ( $post_taxes_with_kw as &$post_tax_kw ) {
			$current_keyword_lower = function_exists( 'mb_strtolower' ) ? mb_strtolower( $post_tax_kw->get_keyword_curent() ?? '' ) : strtolower( $post_tax_kw->get_keyword_curent() ?? '' );
			$keywords              = $post_tax_kw->get_keywords();
