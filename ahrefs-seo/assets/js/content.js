...
if ( jQuery( '#content_table' ).length ) {
	...
	( function($) {
		...
		$( document ).on(
			'click',
			'.sugggested-tip-close-button, #keywords_tip_got_it', // 'x' button on top-right corner or [Got it] button.
			function() {
		...
		window.content = {
			...
			keyword_popup_show: function( post_id, title ) {
				...
				var url = ajaxurl + '?action=ahrefs_content_get_keyword_popup&post=' + encodeURIComponent( post_id ) + '&_wpnonce=' + content.$form.find( '#table_nonce' ).val() + '&width=' + width + '&height=' + height;
