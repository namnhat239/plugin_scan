	...
	$(
		function() {
			...
			$( document ).tooltip(
				{
					...
					content: function() {
						...
						if ( element.hasClass( "with-long-title" ) || element.hasClass( "badge-keyword" )  ) {
							var $with_overflow = $( element ).clone().css( {display: 'inline', width: 'auto', visibility: 'hidden'} ).appendTo( 'body' );
			...
			if ( $( '.how-much-selected' ).length ) {
				var func_update_count = function($item) {
					var $block    = $item.closest( '.block-content' );
					var $subitems = $item.closest( '.block-content' ).find( '.subitems-n' );
					var $inputs   = $subitems.find( 'input[type="checkbox"]' );
					...
					var $main_chechbox = $block.find( '.checkbox-main:first' );
					if ( checked && ! $main_chechbox.is( ':checked' ) ) {
						$main_chechbox.attr( 'checked', 'checked' ).prop( 'checked',true ); // set main choice.
					} else if ( ! checked && $main_chechbox.is( ':checked' ) ) {
						$main_chechbox.removeAttr( 'checked', 'checked' ).prop( 'checked',false ); // unset main choice.
					}
				}
				...
				$( '.how-much-selected' ).each(
					function() {
						...
						func_update_count( $subitems.find( 'input[type="checkbox"]:first' ) );
					}
				);
			...
			if ( $( '.scope-block-n' ).length ) {
				...
				var form_scope_allowed = false;
				$( $form ).on(
					'submit',
					function() {
						if ( form_scope_allowed ) {
							return;
						}
						...
						$form.find( 'ul.subitems-n[data-tax]' ).each(
							function() {
								...
								console.log( $block, var_name, selected );
								$form.append( $( '<input type="hidden" class="form-hidden-js">' ).attr( 'name', var_name ).val( selected ) );
								$block.find( 'input' ).each( function() { $( this ).attr( 'disabled','disabled' )} );
							}
						)
						$form.append( $( '<input type="hidden" class="form-hidden-js" name="ah_options_n" value="1">' ) );
						form_scope_allowed = true;
						setTimeout(
							function(){
								$form.submit();
							},
