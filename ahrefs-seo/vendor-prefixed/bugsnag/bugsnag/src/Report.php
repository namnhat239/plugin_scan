...
class Report
{
     ...
     */
    const PAYLOAD_VERSION = \Bugsnag\HttpClient::NOTIFY_PAYLOAD_VERSION;
    /**
     * The config object.
     *
     ...
     * @return static
     */
    public static function fromPHPError(\Bugsnag\Configuration $config, $code, $message, $file, $line, $fatal = \false)
    {
     ...
     * @return static
     */
    public static function fromPHPThrowable(\Bugsnag\Configuration $config, $throwable)
    {
        // @phpstan-ignore-next-line
        $report = new static($config);
     ...
     * @return static
     */
    public static function fromNamedError(\Bugsnag\Configuration $config, $name, $message = null)
    {
        // @phpstan-ignore-next-line
        $report = new static($config);
        $report->setName($name)->setMessage($message)->setStacktrace(\Bugsnag\Stacktrace::generate($config))->setUnhandled(\false)->setSeverityReason(['type' => 'handledError']);
        return $report;
    }
    /**
     ...
     * @return void
     */
    protected function __construct(\Bugsnag\Configuration $config)
    {
        $this->config = $config;
        $this->time = \Bugsnag\DateTime\Date::now();
    }
    /**
    ...
    public function setPHPThrowable($throwable)
    {
        ...
        $this->setName(\get_class($throwable))->setMessage($throwable->getMessage())->setStacktrace(\Bugsnag\Stacktrace::fromBacktrace($this->config, $throwable->getTrace(), $throwable->getFile(), $throwable->getLine()));
    ...
    public function setPHPError($code, $message, $file, $line, $fatal = \false)
    {
        ...
        if ($fatal) {
            ...
            $stacktrace = \Bugsnag\Stacktrace::fromFrame($this->config, $file, $line);
        } else {
            $stacktrace = \Bugsnag\Stacktrace::generate($this->config);
        }
        $this->setName(\Bugsnag\ErrorTypes::getName($code))->setMessage($message)->setSeverity(\Bugsnag\ErrorTypes::getSeverity($code))->setStacktrace($stacktrace);
        return $this;
    }
    /**
     ...
     * @return $this
     */
    protected function setStacktrace(\Bugsnag\Stacktrace $stacktrace)
    {
    ...
    {
        if (\in_array($severity, ['error', 'warning', 'info', null], \true)) {
     ...
     * @return void
     */
    public function addBreadcrumb(\Bugsnag\Breadcrumbs\Breadcrumb $breadcrumb)
    {
        $data = $breadcrumb->toArray();
        if ($metaData = $this->cleanupObj($breadcrumb->getMetaData(), \true)) {
            $data['metaData'] = $metaData;
            if (\strlen(\json_encode($data)) > \Bugsnag\Breadcrumbs\Breadcrumb::MAX_SIZE) {
                unset($data['metaData']);
            }
        }
    ...
    protected function cleanupObj($obj, $isMetaData)
    {
        ...
        if (\is_string($obj)) {
            return \function_exists('mb_detect_encoding') && !\mb_detect_encoding($obj, 'UTF-8', \true) ? \utf8_encode($obj) : $obj;
        }
        if (\is_object($obj)) {
            if ($obj instanceof \UnitEnum) {
                return $this->enumToString($obj);
            }
            return $this->cleanupObj(\json_decode(\json_encode($obj), \true), $isMetaData);
        }
        return $obj;
    }
    /**
    ...
    protected function shouldFilter($key, $isMetaData)
    {
        ...
        foreach ($this->config->getRedactedKeys() as $redactedKey) {
            if (@\preg_match($redactedKey, $key) === 1) {
                return \true;
            } elseif (\Bugsnag\Utils::stringCaseEquals($redactedKey, $key)) {
                return \true;
            }
        }
        return \false;
