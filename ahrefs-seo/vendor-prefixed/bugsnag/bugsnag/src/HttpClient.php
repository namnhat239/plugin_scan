...
class HttpClient
{
     ...
     */
    public function __construct(\Bugsnag\Configuration $config, \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface $guzzle)
    {
        $this->config = $config;
        $this->guzzle = $guzzle;
    }
     ...
     * @return void
     */
    public function queue(\Bugsnag\Report $report)
    {
        $this->queue[] = $report;
    }
    /**
    ...
    public function sendBuildReport(array $buildInfo)
    {
        ...
        if (isset($buildInfo['builder'])) {
            ...
            $data['builderName'] = \Bugsnag\Utils::getBuilderName();
        }
    ...
    {
        return ['Bugsnag-Api-Key' => $this->config->getApiKey(), 'Bugsnag-Sent-At' => \Bugsnag\DateTime\Date::now(), 'Bugsnag-Payload-Version' => $version, 'Content-Type' => 'application/json'];
    }
    /**
    ...
    {
        if (\Bugsnag\Internal\GuzzleCompat::isUsingGuzzle5()) {
