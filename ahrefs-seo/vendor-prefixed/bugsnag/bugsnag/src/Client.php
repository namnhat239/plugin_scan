...
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp;
class Client
{
    /**
     ...
     */
    const ENDPOINT = \Bugsnag\Configuration::NOTIFY_ENDPOINT;
    /**
     * The config instance.
     *
     ...
     * @return static
     */
    public static function make($apiKey = null, $notifyEndpoint = null, $defaults = \true)
    {
        $env = new \Bugsnag\Env();
        $config = new \Bugsnag\Configuration($apiKey ?: $env->get('BUGSNAG_API_KEY'));
    ...
    public function __construct(\Bugsnag\Configuration $config, \Bugsnag\Request\ResolverInterface $resolver = null, \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface $guzzle = null, \Bugsnag\Shutdown\ShutdownStrategyInterface $shutdownStrategy = null)
    {
        ...
        $this->config = $config;
        $this->resolver = $resolver ?: new \Bugsnag\Request\BasicResolver();
        $this->recorder = new \Bugsnag\Breadcrumbs\Recorder();
        $this->pipeline = new \Bugsnag\Pipeline();
        $this->http = new \Bugsnag\HttpClient($config, $guzzle);
        $this->sessionTracker = new \Bugsnag\SessionTracker($config, $this->http);
        $this->registerMiddleware(new \Bugsnag\Middleware\NotificationSkipper($config));
        $this->registerMiddleware(new \Bugsnag\Middleware\DiscardClasses($config));
        $this->registerMiddleware(new \Bugsnag\Middleware\BreadcrumbData($this->recorder));
        $this->registerMiddleware(new \Bugsnag\Middleware\SessionData($this));
        // Shutdown strategy is used to trigger flush() calls when batch sending is enabled
        $shutdownStrategy = $shutdownStrategy ?: new \Bugsnag\Shutdown\PhpShutdownStrategy();
    ...
    {
        $key = \Bugsnag\Internal\GuzzleCompat::getBaseUriOptionName();
        $options[$key] = $base ?: \Bugsnag\Configuration::NOTIFY_ENDPOINT;
        $path = static::getCaBundlePath();
        if ($path) {
            $options['verify'] = $path;
        }
        return \Bugsnag\Internal\GuzzleCompat::applyRequestOptions($options, ['timeout' => self::DEFAULT_TIMEOUT_S, 'connect_timeout' => self::DEFAULT_TIMEOUT_S]);
    }
    /**
     ...
     * @return void
     */
    private function syncNotifyEndpointWithGuzzleBaseUri(\Bugsnag\Configuration $configuration, \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface $guzzle)
    {
        ...
        if ($configuration->getNotifyEndpoint() !== \Bugsnag\Configuration::NOTIFY_ENDPOINT) {
            return;
        }
        $base = \Bugsnag\Internal\GuzzleCompat::getBaseUri($guzzle);
    ...
    {
        if (\version_compare(\PHP_VERSION, '5.6.0') >= 0 || !\class_exists(\Composer\CaBundle\CaBundle::class)) {
            return \false;
        }
        return \realpath(\Composer\CaBundle\CaBundle::getSystemCaRootBundlePath());
    }
    /**
    ...
    {
        $this->registerMiddleware(new \Bugsnag\Middleware\CallbackBridge($callback));
        return $this;
    }
    /**
    ...
    {
        $this->registerCallback(new \Bugsnag\Callbacks\GlobalMetaData($this->config))->registerCallback(new \Bugsnag\Callbacks\RequestMetaData($this->resolver))->registerCallback(new \Bugsnag\Callbacks\RequestSession($this->resolver))->registerCallback(new \Bugsnag\Callbacks\RequestUser($this->resolver))->registerCallback(new \Bugsnag\Callbacks\RequestContext($this->resolver));
        return $this;
    }
    /**
    ...
    {
        $type = \in_array($type, \Bugsnag\Breadcrumbs\Breadcrumb::getTypes(), \true) ? $type : \Bugsnag\Breadcrumbs\Breadcrumb::MANUAL_TYPE;
        $this->recorder->record(new \Bugsnag\Breadcrumbs\Breadcrumb($name, $type, $metaData));
    }
    /**
    ...
    public function notify(\Bugsnag\Report $report, callable $callback = null)
    {
        $this->pipeline->execute($report, function ($report) use($callback) {
            if ($callback) {
                $resolvedReport = null;
                $bridge = new \Bugsnag\Middleware\CallbackBridge($callback);
        ...
        $this->leaveBreadcrumb($report->getName(), \Bugsnag\Breadcrumbs\Breadcrumb::ERROR_TYPE, $report->getSummary());
     ...
     * @return $this
     */
    public function setMetaData(array $metaData, $merge = \true)
    {
