...
use ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\DES;
...
class SSH1
{
    ...
    function _connect()
    {
        ...
        if (\defined('ahrefs\\AhrefsSeo_Vendor\\NET_SSH1_LOGGING')) {
        ...
        $server_key_public_exponent = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        $this->server_key_public_exponent = $server_key_public_exponent;
        if (\strlen($response[self::RESPONSE_DATA]) < 2) {
            return \false;
        }
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $server_key_public_modulus = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        ...
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_exponent = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        $this->host_key_public_exponent = $host_key_public_exponent;
        if (\strlen($response[self::RESPONSE_DATA]) < 2) {
            return \false;
        }
        $temp = \unpack('nlen', $this->_string_shift($response[self::RESPONSE_DATA], 2));
        $host_key_public_modulus = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($this->_string_shift($response[self::RESPONSE_DATA], \ceil($temp['len'] / 8)), 256);
        ...
        $session_key = \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Random::string(32);
        ...
        switch ($cipher) {
            ...
            case self::CIPHER_DES:
                $this->crypto = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\DES();
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
                $this->crypto->setKey(\substr($session_key, 0, 8));
                break;
            case self::CIPHER_3DES:
                $this->crypto = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\TripleDES(\ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\TripleDES::MODE_3CBC);
                $this->crypto->disablePadding();
                $this->crypto->enableContinuousBuffer();
    ...
    function login($username, $password = '')
    {
        ...
        if (\defined('ahrefs\\AhrefsSeo_Vendor\\NET_SSH1_LOGGING') && NET_SSH1_LOGGING == self::LOG_COMPLEX) {
            $data = \pack('CNa*', NET_SSH1_CMSG_AUTH_PASSWORD, \strlen('password'), 'password');
    ...
    function _send_binary_packet($data)
    {
        ...
        $padding = \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Random::string(8 - ($length & 7));
        $orig = $data;
        $data = $padding . $data;
    ...
    function _crc($data)
    {
        static $crc_lookup_table = array(0x0, 0x77073096, 0.0, 2567524794.0, 0x76dc419, 0x706af48f, 0.0, 0.0, 0xedb8832, 0x79dcb8a4, 0.0, 2547177864.0, 0x9b64c2b, 0x7eb17cbd, 0.0, 2428444049.0, 0x1db71064, 0x6ab020f2, 4089016648.0, 0.0, 0x1adad47d, 0x6ddde4eb, 4107580753.0, 2211677639.0, 0x136c9856, 0x646ba8c0, 4251122042.0, 0.0, 0x14015c4f, 0x63066cd9, 4195302755.0, 2366115317.0, 0x3b6e20c8, 0x4c69105e, 0.0, 2724688242.0, 0x3c03e4d1, 0x4b04d447, 3524101629.0, 2768942443.0, 0x35b5a8fa, 0x42b2986c, 3686517206.0, 2898065728.0, 0x32d86ce3, 0x45df5c75, 3705015759.0, 2882616665.0, 0x26d930ac, 0x51de003a, 3369554304.0, 3218104598.0, 0x21b4f4b5, 0x56b3c423, 3485111705.0, 3099436303.0, 0x2802b89e, 0x5f058808, 3322730930.0, 0.0, 0x2f6f7c87, 0x58684c11, 3244367275.0, 3060149565.0, 0x76dc4190, 0x1db7106, 2563907772.0, 0.0, 0x71b18589, 0x6b6b51f, 0.0, 0.0, 0x7807c9a2, 0xf00f934, 0.0, 0.0, 0x7f6a0dbb, 0x86d3d2d, 2439277719.0, 0.0, 0x6b6b51f4, 0x1c6c6162, 2238001368.0, 0.0, 0x6c0695ed, 0x1b01a57b, 2181625025.0, 4111451223.0, 0x65b0d9c6, 0x12b7e950, 0.0, 4240017532.0, 0x62dd1ddf, 0x15da2d49, 2362670323.0, 4224994405.0, 0x4db26158, 0x3ab551ce, 2747007092.0, 0.0, 0x4adfa541, 0x3dd895d7, 2765210733.0, 3554079995.0, 0x4369e96a, 0x346ed9fc, 2909243462.0, 3663771856.0, 0x44042d73, 0x33031de5, 2852801631.0, 3708648649.0, 0x5005713c, 0x270241aa, 0.0, 3373015174.0, 0x5768b525, 0x206f85b3, 3110523913.0, 0.0, 0x5edef90e, 0x29d9c998, 2966460450.0, 3352799412.0, 0x59b33d17, 0x2eb40d81, 3082640443.0, 3233442989.0, 0.0, 2596254646.0, 0x3b6e20c, 0x74b1d29a, 0.0, 2647816111.0, 0x4db2615, 0x73dc1683, 0.0, 2489596804.0, 0xd6d6a3e, 0x7a6a5aa8, 0.0, 2466906013.0, 0xa00ae27, 0x7d079eb1, 4027552580.0, 2265490386.0, 0x1e01f268, 0x6906c2fe, 4150417245.0, 2154129355.0, 0x196c3671, 0x6e6b06e7, 0.0, 0.0, 0x10da7a5a, 0x67dd4acc, 4189708143.0, 0.0, 0x17b7be43, 0x60b08ed5, 0.0, 0.0, 0x38d8c2c4, 0x4fdff252, 3518719985.0, 2797360999.0, 0x3fb506dd, 0x48b2364b, 3624741850.0, 2936675148.0, 0x36034af6, 0x41047a60, 0.0, 2825379669.0, 0x316e8eef, 0x4669be79, 3412177804.0, 3160834842.0, 0x256fd2a0, 0x5268e236, 3423369109.0, 3138078467.0, 0x220216b9, 0x5505262f, 0.0, 2998733608.0, 0x2bb45a92, 0x5cb36a04, 3268935591.0, 3050360625.0, 0x2cd99e8b, 0x5bdeae1d, 2607071920.0, 0.0, 0x756aa39c, 0x26d930a, 2617837225.0, 0.0, 0x72076785, 0x5005713, 2512341634.0, 0.0, 0x7bb12bae, 0xcb61b38, 0.0, 0.0, 0x7cdcefb7, 0xbdbdf21, 2262029012.0, 0.0, 0x68ddb3f8, 0x1fda836e, 0.0, 4139329115.0, 0x6fb077e1, 0x18b74777, 0.0, 4279200368.0, 0x66063bca, 0x11010b5c, 0.0, 0.0, 0x616bffd3, 0x166ccf45, 0.0, 0.0, 0x4e048354, 0x3903b3c2, 2808555105.0, 3495958263.0, 0x4969474d, 0x3e6e77db, 0.0, 3654703836.0, 0x40df0b66, 0x37d83bf0, 0.0, 0.0, 0x47b2cf7f, 0x30b5ffe9, 3183342108.0, 3401237130.0, 0x53b39330, 0x24b4a3a6, 3134207493.0, 3453421203.0, 0x54de5729, 0x23d967bf, 0.0, 3294710456.0, 0x5d681b02, 0x2a6f2b94, 0.0, 0.0, 0x5a05df1b, 0x2d02ef8d);
    ...
    function _rsa_crypt($m, $key)
    {
        ...
        */
        ...
        while (\strlen($random) != $length) {
            $block = \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Random::string($length - \strlen($random));
            $block = \str_replace("\x00", '', $block);
            $random .= $block;
        }
        $temp = \chr(0) . \chr(2) . $random . \chr(0) . $m;
        $m = new \ahrefs\AhrefsSeo_Vendor\phpseclib\Math\BigInteger($temp, 256);
