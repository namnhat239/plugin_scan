...
class SSH2
{
    ...
    function _get_binary_packet($skip_channel_filter = \false)
    {
        ...
        if ($remaining_length < -$this->decrypt_block_size || $remaining_length > 0x9000 || $remaining_length % $this->decrypt_block_size != 0) {
            if (!$this->bad_key_size_fix && $this->_bad_algorithm_candidate($this->decrypt->name) && !($this->bitmap & \ahrefs\AhrefsSeo_Vendor\phpseclib\Net\SSH2::MASK_LOGIN)) {
        ...
        switch ($this->signature_format) {
            ...
            case 'rsa-sha2-512':
                ...
                */
                ...
                switch ($this->signature_format) {
                    case 'rsa-sha2-512':
                        $h = \pack('N5a*', 0x305130, 0xd060960, 2252865893.0, 0x3040203, 0x5000440, $hashObj->hash($this->exchange_hash));
                        break;
                    case 'rsa-sha2-256':
                        $h = \pack('N5a*', 0x303130, 0xd060960, 2252865893.0, 0x3040201, 0x5000420, $hashObj->hash($this->exchange_hash));
                        break;
                    //case 'ssh-rsa':
                    default:
                        $hash = 'sha1';
