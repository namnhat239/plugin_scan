 ...
 * @access  public
 */
class TripleDES extends \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\DES
{
    /**
     ...
     */
    const MODE_CBC3 = \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Base::MODE_CBC;
    /**
     * Key Length (in bytes)
     *
     ...
     * @access public
     */
    function __construct($mode = \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Base::MODE_CBC)
    {
        switch ($mode) {
            // In case of self::MODE_3CBC, we init as CRYPT_DES_MODE_CBC
            // and additional flag us internally as 3CBC
            case self::MODE_3CBC:
                parent::__construct(\ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Base::MODE_CBC);
                $this->mode_3cbc = \true;
                // This three $des'es will do the 3CBC work (if $key > 64bits)
                $this->des = array(new \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\DES(\ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Base::MODE_CBC), new \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\DES(\ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Base::MODE_CBC), new \ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\DES(\ahrefs\AhrefsSeo_Vendor\phpseclib\Crypt\Base::MODE_CBC));
    ...
    {
        if ($this->mode_3cbc && \strlen($this->key) > 8) {
            return $this->_unpad($this->des[0]->decrypt($this->des[1]->encrypt($this->des[2]->decrypt(\str_pad($ciphertext, \strlen($ciphertext) + 7 & 4294967288.0, "\x00")))));
        }
