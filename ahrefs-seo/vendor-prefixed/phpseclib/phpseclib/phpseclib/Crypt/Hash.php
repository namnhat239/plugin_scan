...
class Hash
{
    ...
    function _rightRotate($int, $amt)
    {
        $invamt = 32 - $amt;
        $mask = (1 << $invamt) - 1;
        return $int << $invamt & 4294967295.0 | $int >> $amt & $mask;
    }
    /**
     * Right Shift
     *
    ...
    function _add()
    {
        ...
        foreach ($arguments as $argument) {
            $result += $argument < 0 ? ($argument & 0x7fffffff) + 2147483648.0 : $argument;
        }
        if ((\php_uname('m') & "\xdf\xdf\xdf") != 'ARM') {
            return \fmod($result, $mod);
        }
        return \fmod($result, 2147483648.0) & 0x7fffffff | (\fmod(\floor($result / 2147483648.0), 2) & 1) << 31;
    }
    /**
     * String Shift
     *
