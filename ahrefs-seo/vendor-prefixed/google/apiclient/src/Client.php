 ...
 */
namespace ahrefs\AhrefsSeo_Vendor\Google;

use ahrefs\AhrefsSeo_Vendor\Google\AccessToken\Revoke;
use ahrefs\AhrefsSeo_Vendor\Google\AccessToken\Verify;
use ahrefs\AhrefsSeo_Vendor\Google\Auth\ApplicationDefaultCredentials;
use ahrefs\AhrefsSeo_Vendor\Google\Auth\Cache\MemoryCacheItemPool;
...
use ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\ServiceAccountCredentials;
use ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\UserRefreshCredentials;
use ahrefs\AhrefsSeo_Vendor\Google\AuthHandler\AuthHandlerFactory;
use ahrefs\AhrefsSeo_Vendor\Google\Http\REST;
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Client as GuzzleClient;
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface;
use ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Ring\Client\StreamHandler;
...
use ahrefs\AhrefsSeo_Vendor\Monolog\Handler\StreamHandler as MonologStreamHandler;
use ahrefs\AhrefsSeo_Vendor\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
...
class Client
{
    ...
    public function __construct(array $config = array())
    {
        ...
        if (!\is_null($this->config['credentials'])) {
            if ($this->config['credentials'] instanceof \ahrefs\AhrefsSeo_Vendor\Google\Auth\CredentialsLoader) {
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \ahrefs\AhrefsSeo_Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface $http = null)
    {
    ...
    {
        $tokenRevoker = new \ahrefs\AhrefsSeo_Vendor\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \ahrefs\AhrefsSeo_Vendor\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\ahrefs\AhrefsSeo_Vendor\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \ahrefs\AhrefsSeo_Vendor\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \ahrefs\AhrefsSeo_Vendor\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\ahrefs\AhrefsSeo_Vendor\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
    ...
    {
        $logger = new \ahrefs\AhrefsSeo_Vendor\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \ahrefs\AhrefsSeo_Vendor\Monolog\Handler\SyslogHandler('app', \LOG_USER, \ahrefs\AhrefsSeo_Vendor\Monolog\Logger::NOTICE);
        } else {
            $handler = new \ahrefs\AhrefsSeo_Vendor\Monolog\Handler\StreamHandler('php://stderr', \ahrefs\AhrefsSeo_Vendor\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param ClientInterface $http
     */
    public function setHttpClient(\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    {
        $guzzleVersion = null;
        if (\defined('\\ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        } elseif (\defined('\\ahrefs\\AhrefsSeo_Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            $guzzleVersion = (int) \substr(\ahrefs\AhrefsSeo_Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        if (5 === $guzzleVersion) {
            $options = ['base_url' => $this->config['base_path'], 'defaults' => ['exceptions' => \false]];
            if ($this->isAppEngine()) {
                // set StreamHandler on AppEngine by default
                $options['handler'] = new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Client($options);
    }
    /**
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \ahrefs\AhrefsSeo_Vendor\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            ...
            $credentials = \ahrefs\AhrefsSeo_Vendor\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \ahrefs\AhrefsSeo_Vendor\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \ahrefs\AhrefsSeo_Vendor\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \ahrefs\AhrefsSeo_Vendor\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        ...
        return new \ahrefs\AhrefsSeo_Vendor\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
