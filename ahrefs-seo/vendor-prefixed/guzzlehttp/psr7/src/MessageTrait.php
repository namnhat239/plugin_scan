...
trait MessageTrait
{
    ...
    {
        if (!$this->stream) {
            $this->stream = \ahrefs\AhrefsSeo_Vendor\GuzzleHttp\Psr7\Utils::streamFor('');
        }
        return $this->stream;
    }
    public function withBody(\ahrefs\AhrefsSeo_Vendor\Psr\Http\Message\StreamInterface $body)
    {
        if ($body === $this->stream) {
            return $this;
        }
