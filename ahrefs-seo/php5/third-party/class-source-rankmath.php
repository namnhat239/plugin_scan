...
class Source_Rankmath extends Source implements Assigned_Keyword, Canonical_Url {
	...
	public function is_noindex( Post_Tax $post_tax ) {
		...
		try {
			if ( $this->is_available ) {
				if ( class_exists( '\\RankMath\\Helper' ) && method_exists( '\\RankMath\\Helper', 'is_post_indexable' ) && method_exists( '\\RankMath\\Helper', 'is_term_indexable' ) ) {
	...
	public function get_canonical_url( Post_Tax $post_tax ) {
		$result = null;
		if ( $this->is_available ) {
			if ( class_exists( '\\RankMath\\Helper' ) && method_exists( '\\RankMath\\Helper', 'get_post_meta' ) && method_exists( '\\RankMath\\Helper', 'get_term_meta' ) ) {
				// @phpstan-ignore-line -- Plugin may be active or not exists.
				if ( $post_tax->is_post() ) {
