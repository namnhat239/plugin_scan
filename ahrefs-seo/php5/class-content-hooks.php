...
class Content_Hooks {
	...
	public function __construct() {
		...
		add_action( 'wp_update_term_data', [ $this, 'update_term_data' ], 999, 3 );
	}
	/**
	 ...
	 * @return void
	 */
	public function remove_post_from_audit( $post_id ) {
	...
	public function post_updated( $post_id, $post_after, $post_before ) {
		...
		if ( is_int( $post_id ) && is_object( $post_after ) && is_object( $post_before ) && $post_after instanceof \WP_Post && $post_before instanceof \WP_Post && in_array( get_post_type( $post_id ), ( new Ahrefs_Seo_Content() )->get_custom_post_types_enabled( true ), true ) && 'publish' === $post_after->post_status ) {
	 ...
	 * @return void
	 */
	public function add_new_term_to_audit( $term_id, $tt_id, $taxonomy ) {
		// Note: Callback, do not use parameter types.
	 ...
	 * @return void
	 */
	public function remove_term_from_audit( $term_id, $tt_id, $taxonomy ) {
		// Note: Callback, do not use parameter types.
		if ( is_string( $taxonomy ) && in_array( $taxonomy, [ 'category', 'product_cat' ], true ) ) {
	...
	public function transition_post_status( $new_status, $old_status, $post ) {
		// @phpstan-ignore-next-line -- ignore types from phpDoc.
		if ( is_object( $post ) && $post instanceof \WP_Post && in_array( $post->post_type, ( new Ahrefs_Seo_Content() )->get_custom_post_types_enabled( true ), true ) ) {
	...
	public function update_term_data( $data, $term_id, $taxonomy ) {
		// Note: Callback, do not use parameter types.
		// @phpstan-ignore-next-line -- ignore types from phpDoc.
		if ( is_int( $term_id ) && is_string( $taxonomy ) && in_array( $taxonomy, [ 'category', 'product_cat' ], true ) ) {
