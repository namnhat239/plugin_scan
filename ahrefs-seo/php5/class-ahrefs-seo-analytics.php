...
class Ahrefs_Seo_Analytics extends Ahrefs_Seo_Abstract_Api {
	 ...
	 * @return void
	 */
	public function set_client( Google_Client $client ) {
		$this->client = $client;
	}
	/**
	 ...
	 */
	public function set_data_token( Data_Tokens_Storage $data_tokens ) {
	 ...
	 */
	private function check_gsc_using_bulk_results_strings( $gsc_site, $with_clicks_only = true ) {
	...
	public function get_visitors_by_page( array $page_slugs = null, $start_date, $end_date, $max_results = null, $ua_id = null ) {
		...
		$result = 0 === strpos( is_null( $ua_id ) ? $this->data_tokens->get_ua_id() : $ua_id, 'properties/' ) ? $this->get_visitors_by_page_ga4( $page_slugs, $start_date, $end_date, $max_results, $ua_id ) : $this->get_visitors_by_page_ga( $page_slugs, $start_date, $end_date, $max_results, $ua_id );
			...
			do {
				try {
					...
					if ( ! empty( $report ) ) {
						$continue = false;
						$rows     = $report->getRows();
						$totals   = ! empty( $report->getRowCount() ) ? $report->getRowCount() : 0;
						if ( ! empty( $rows ) ) {
							foreach ( $rows as $row ) {
								...
								if ( ! isset( $result[ $_slug ][ "{$_type}" ] ) ) {
									...
									$result[ $_slug ]['total']      = $_traffic_count + ( isset( $result[ $_slug ]['total'] ) ? $result[ $_slug ]['total'] : 0 );
								} else {
									$result[ $_slug ][ "{$_type}" ] += $_traffic_count;
	 ...
	 * @return bool False on error.
	 */
	protected function set_ga4_property( \ahrefs\AhrefsSeo_Vendor\Google\Service\AnalyticsData\RunReportRequest &$request, $property_id ) {
				...
				if ( is_array( $errors ) && 0 < count( $errors ) && isset( $errors[0]['reason'] ) ) {
					...
					if ( 'forbidden' === $reason ) {
						if ( $is_gsc && $this->data_tokens->get_gsc_site() ) { // if was not disconnected before.
							$site = preg_match( "/site '([^']+)'/", isset( $errors[0]['message'] ) ? $errors[0]['message'] : $e->getMessage(), $m ) ? $m[1] : $this->data_tokens->get_gsc_site();
	 ...
	 * @return void
	 */
	private function prepare_gsc_query( $key, Google_Http_Batch &$batch, Google_Service_SearchConsole $service_searchconsole, $gsc_site, array $parameters ) {
		$result = [];
		try {
			$post_body = new Google_Service_SearchConsole_SearchAnalyticsQueryRequest( $parameters );
	 ...
	 */
	protected function parse_gsc_response( Google_Service_SearchConsole_SearchAnalyticsQueryResponse $response = null ) {
		$result = [];
		if ( ! empty( $response ) ) {
			/**
			 * @var \ahrefs\AhrefsSeo_Vendor\Google\Service\SearchConsole\ApiDataRow $row
	...
	public function get_clicks_and_impressions_by_urls( array $data_url_and_country_list, $start_date = null, $end_date = null, $limit = null, $without_totals = false, array $data_keyword_list = [] ) {
		...
		try {
			...
			foreach ( $data_url_and_country_list as $key => $data_keyword ) {
				...
				if ( ! $without_totals ) {
					$this->prepare_gsc_query( "{$key}-total", $batch, $service_searchconsole, $this->data_tokens->get_gsc_site(), array_merge( $parameters, [ 'dimensions' => [ 'page' ] ] ) );
				}
				// Top 10 clicks, positions, impressions.
				$this->prepare_gsc_query( "{$key}-q", $batch, $service_searchconsole, $this->data_tokens->get_gsc_site(), array_merge( $parameters, [ 'dimensions' => [ 'query', 'page' ] ] ) );
			}
			$e = null;
			try {
				// execute requests.
	...
	public function get_position_fast( array $list_url_keyword ) {
		...
		try {
			...
			foreach ( $list_url_keyword as $key => $row ) {
				...
				$this->prepare_gsc_query( "{$key}-f", $batch, $service_searchconsole, $this->data_tokens->get_gsc_site(), $parameters );
			}
			try {
				// execute requests.
				$time0 = microtime( true );
	 ...
	 * @return void
	 */
	public function set_adv_options( $gsc_uses_uppercase, $ga_not_urlencoded, $ga_uses_full_url ) {
