...
class Worker_Noindex extends Worker {
	 ...
	 *
	 * @param array<string, Result_Noindex> $results Associave array page_tax_string => Result_Noindex.
	 * @return void
	 */
	protected function update_noindex_values( array $results ) {
		...
		foreach ( $results as $post_tax_string => $value ) {
			try {
				$this->content_audit->update_noindex_values( $value );
			} catch ( \Error $e ) {
				$error = Ahrefs_Seo_Compatibility::on_type_error( $e, __METHOD__, __FILE__, __( 'Update "is noindex" info failed.', 'ahrefs-seo' ) );
			} catch ( \Exception $e ) {
	 ...
	 * @since 0.9.1
	 *
	 * @param array<string, Result_Canonical> $results Associave array page_tax_string => Result_Canonical.
	 * @return void
	 */
	protected function update_noncanonical_values( array $results ) {
		Ahrefs_Seo::breadcrumbs( sprintf( '%s: %s', __METHOD__, (string) wp_json_encode( $results ) ) );
		foreach ( $results as $post_tax_string => $value ) {
			try {
				$this->content_audit->update_canonical_values( $value );
			} catch ( \Error $e ) {
				$error = Ahrefs_Seo_Compatibility::on_type_error( $e, __METHOD__, __FILE__, __( 'Update "is non-canonical" info failed.', 'ahrefs-seo' ) );
			} catch ( \Exception $e ) {
	 ...
	 * @since 0.9.2
	 *
	 * @param array<string,Result_Redirected> $results Associave array page_tax_string => Result_Redirected.
	 * @return void
	 */
	protected function update_redirected_values( array $results ) {
		Ahrefs_Seo::breadcrumbs( sprintf( '%s: %s', __METHOD__, (string) wp_json_encode( $results ) ) );
		foreach ( $results as $post_tax_string => $value ) {
			try {
				$this->content_audit->update_redirected_values( $value );
			} catch ( \Error $e ) {
				$error = Ahrefs_Seo_Compatibility::on_type_error( $e, __METHOD__, __FILE__, __( 'Update "is redirected" info failed.', 'ahrefs-seo' ) );
			} catch ( \Exception $e ) {
				Ahrefs_Seo::notify( $e, 'Update "is non-canonical" info failed.' );
