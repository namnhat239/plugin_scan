	...
	public function update_posts_info( array $post_taxes ) {
		if ( ! is_null( $this->snapshot_id ) ) {
			...
			foreach ( $post_taxes_kw as $post_tax_kw ) {
				...
				if ( empty( $keywords ) || empty( $keywords['result'] ) ) { // it make no sense to load position if GSC already returned no results for keywords.
				...
				} elseif ( is_null( $suggested_keyword ) || '' === $suggested_keyword ) {
					// if current keyword is empty.
					$this->content_audit->update_position_values( $post_tax_kw, Ahrefs_Seo_Data_Content::POSITION_MAX, sprintf( '[GSC] URL %s skip empty keyword "%s"', $post_tax_kw->get_url(), (string) $post_tax_kw->get_keyword_curent() ) );
				}
			}
		} else {
