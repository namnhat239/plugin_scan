...
$keyword_current   = isset( $locals['keyword_current'] ) ? $locals['keyword_current'] : '';
if ( $has_keyword && $is_keyword_unique ) {
	...
	<li>
		...
		<p>
		<?php
		esc_html_e( 'If your page isn’t ranking in the top 20 positions for its target keyword, it’s probably going to take a lot of time and effort to crack the first page. You’ll need to decide if the effort is really worth the reward. If it isn’t, one option is to leave the page as-is and exclude it from future content audits so that it doesn’t affect your overall score. If the page isn’t important to your business at all, you might consider deleting it.', 'ahrefs-seo' );
		?>
	</p>
		<p class="with-button">
			<a href="#" class="button action-stop"><span></span>
			<?php
			esc_html_e( 'Exclude from audit', 'ahrefs-seo' );
			?>
	</a>
		</p>
	</li>
	...
	<li>
		<p>
		<?php
		esc_html_e( 'Decide if this page is important', 'ahrefs-seo' );
		?>
	</p>
		<p>
		<?php
