...
if ( ! function_exists( 'output_refdomains_count' ) ) {
	/**
	 * Print a block with refdomains count string or a script for loading freash value
	 *
	 * @param int|null $ref_domains Number of ref.domains.
	 * @param Post_Tax $post_tax Current post or term.
	 * @return void
	 */
	function output_refdomains_count( $ref_domains = null, Post_Tax $post_tax ) {
		?>
		<p class="ref-domains-count-wrap">
			...
			if ( ! is_null( $ref_domains ) && $ref_domains >= 0 ) {
				...
				<script type="text/javascript">
					...
					content.refdomains_refresh( '.ref-domains-count-wrap', 
					<?php
					echo wp_json_encode( (string) $post_tax );
					?>
			 );
				</script>
				<?php
...
if ( $keyword ) {
	...
	<li>
		...
		<p class="with-button">
			...
			<?php
			esc_html_e( 'How to get more backlinks', 'ahrefs-seo' );
			?>
	</a>
		</p>
	</li>
	...
	<li>
		...
		<p>
		<?php
		esc_html_e( 'Not ranking well doesn’t automatically mean that there’s a problem with your content – you might not have enough backlinks to compete in the SERP. We suggest checking on your competitors in the SERP and seeing how you compare to their numbers.', 'ahrefs-seo' );
		?>
	</p>
		<?php
		output_refdomains_count( $ref_domains, $post_tax );
