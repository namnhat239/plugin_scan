...
if ( ! is_null( $post_tax ) && $post_tax instanceof Post_Tax && $post_tax->exists() ) {
	...
	<div class="ahrefs-seo-modal-keywords" id="ahrefs_seo_modal_keywords" data-id="<?php echo esc_attr( $id ); ?>">
		<div class="keywords-wrap-body">
			...
			<?php
			esc_html_e( 'Select target keyword for', 'ahrefs-seo' );
			?>
	 <a class="keyword-post-title" href="<?php echo esc_attr( $url_view ); ?>">
	<?php
	...
	?>
			...
			<div class="keyword-table-wrap">
				<div class="keyword-choice-wrap">
					...
					?>
						...
						); // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentAfterEnd,Squiz.PHP.EmbeddedPhp.ContentBeforeEnd ?>">&nbsp;</span></div>
					...
					<div class="keyword-title">
						<?php
						esc_html_e( 'Suggested keywords', 'ahrefs-seo' );
						?>
		...
		<div class="keywords-buttons">
			...
			<a href="#" class="button button-hero button-cancel" id="ahrefs_seo_keyword_cancel">
			<?php
			esc_html_e( 'Cancel', 'ahrefs-seo' );
			?>
	</a>
		</div>

		<script type="text/javascript">
			content.keyword_country_code = 
			<?php
			echo wp_json_encode( $country_code_ahrefs );
			?>
	;
			content.keyword_country_code3 = 
			<?php
			echo wp_json_encode( $country_code );
			?>
	;
	...
	$message = isset( $locals['error'] ) && $locals['error'] instanceof Message ? $locals['error'] : Message::create(
		[
			'type'    => 'error-single',
			'title'   => '',
			'message' => __(
				'This page cannot be found. It is possible that you’ve archived the page or changed the page ID. Please reload the page & try again.',
				'ahrefs-seo'
			),
		]
	);
