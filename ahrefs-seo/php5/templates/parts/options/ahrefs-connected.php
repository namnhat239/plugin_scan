...
$plan            = isset( $data['subscription'] ) ? $data['subscription'] : __( 'Unknown', 'ahrefs-seo' );
$rows_left       = isset( $data['rows_left'] ) ? sprintf( __( '%s left', 'ahrefs-seo' ), number_format( intval( $data['rows_left'] ), 0 ) ) : __( 'Unknown', 'ahrefs-seo' );
...
<?php echo esc_attr( AHREFS_SEO_IMAGES_URL ); ?>ahrefs-wp-plugin-2-x@3x.jpg 3x"
		...
		if ( $is_free_plan ) {
			...
			<div class="disconnect-wrap">
				...
				<?php
				esc_html_e( 'Free Ahrefs account is connected to your WP dashboard', 'ahrefs-seo' );
				?>
	</div>
				<?php
			...
			<a href="https://ahrefs.com/big-data" target="_blank" class="learn-more-link">
				<span class="text">
				<?php
				esc_html_e( 'Learn more about Ahrefs', 'ahrefs-seo' );
				?>
	</span>
			...
			<div class="disconnect-wrap">
				<div class="your-account">
				<?php
				esc_html_e( 'Connection success! Time to let the SEO sparks fly.', 'ahrefs-seo' );
				?>
	</div>
				<?php
				if ( current_user_can( Ahrefs_Seo::CAP_SETTINGS_ACCOUNTS_SAVE ) ) {
			...
			<div class="account-row"><span class="account-title">
			<?php
			esc_html_e( 'Plan', 'ahrefs-seo' );
			?>
			...
			<div class="account-row"><span class="account-title">
			<?php
			esc_html_e( 'Data rows', 'ahrefs-seo' );
			?>
	</span><span class="account-value">
			<?php
			...
			<div class="help">
			<?php
			esc_html_e( 'Data rows are consumed when we update for new backlinks.', 'ahrefs-seo' );
			?>
	</div>
			<?php
		}
