...
class Ahrefs_Seo_Keywords {
	...
	public function get_full_detail_for_posts( array $post_taxes, $limit = null, $without_totals = false, $skip_gsc_errors = false, $load_imported_info = false ) {
		...
		foreach ( $post_taxes as $post_tax ) {
			...
			if ( $post_tax->exists() && $post_tax->is_tax_or_published() && '' !== $post_tax->get_url() ) {
				// only for existing and published posts or terms with non empty url.
				...
				if ( $this->analytics->is_gsc_set() ) {
					if ( ! is_null( $post_tax_kw->get_keyword_curent() ) && '' !== $post_tax_kw->get_keyword_curent() ) {
	...
	public function get_suggestions( Post_Tax $post_tax, $use_cached_data = false ) {
		...
		$keyword                = ! empty( $post_tax_kw->get_keyword_curent() ) ? $post_tax_kw->get_keyword_curent() : '';
	...
	public function load_position_value( array $post_taxes ) {
		...
		foreach ( $post_taxes_with_kw as &$post_tax_kw ) {
			$current_keyword_lower = function_exists( 'mb_strtolower' ) ? mb_strtolower( ! empty( $post_tax_kw->get_keyword_curent() ) ? $post_tax_kw->get_keyword_curent() : '' ) : strtolower( ! empty( $post_tax_kw->get_keyword_curent() ) ? $post_tax_kw->get_keyword_curent() : '' );
			$keywords              = $post_tax_kw->get_keywords();
