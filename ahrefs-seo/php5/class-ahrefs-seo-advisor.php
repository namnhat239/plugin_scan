...
class Ahrefs_Seo_Advisor {
	 ...
	 */
	public function find_relevant_top_performing_pages( Post_Tax $post_tax, $limit = 20 ) {
	 ...
	 * @return Post_Tax[]|null List of posts or terms.
	 */
	public function find_relevant_under_performing_pages( Post_Tax $post_tax, $limit = 20 ) {
		return $this->find_posts_with_same_keys( $post_tax, [ Ahrefs_Seo_Data_Content::ACTION4_UPDATE_YELLOW, Ahrefs_Seo_Data_Content::ACTION4_MERGE, Ahrefs_Seo_Data_Content::ACTION4_EXCLUDE, Ahrefs_Seo_Data_Content::ACTION4_UPDATE_ORANGE, Ahrefs_Seo_Data_Content::ACTION4_REWRITE ], $limit, false );
	}
	/**
	 ...
	 */
	public function find_active_pages_with_same_keyword( Post_Tax $post_tax, $limit = 1000 ) {
		return $this->find_posts_with_same_keys( $post_tax, [ Ahrefs_Seo_Data_Content::ACTION4_DO_NOTHING, Ahrefs_Seo_Data_Content::ACTION4_UPDATE_YELLOW, Ahrefs_Seo_Data_Content::ACTION4_MERGE, Ahrefs_Seo_Data_Content::ACTION4_EXCLUDE, Ahrefs_Seo_Data_Content::ACTION4_REWRITE, Ahrefs_Seo_Data_Content::ACTION4_ANALYZING_FINAL ], $limit, false );
	}
	/**
	 ...
	 * @return bool
	 */
	public function has_active_pages_with_same_keywords( Post_Tax $post_tax ) {
	 ...
	 * @return Post_Tax[]|null List of posts or terms.
	 */
	private function find_posts_with_same_keys( Post_Tax $post_tax, array $actions_filter = [], $limit = 100, $traffic_order_desc = true ) {
		global $wpdb;
		// post keywords.
	 ...
	 */
	public function find_internal_links( Post_Tax $post_tax, $limit = 1000 ) {
		global $wpdb;
		$result = [];
	 ...
	 * @return bool
	 */
	public function has_keyword( Post_Tax $post_tax ) {
		$keyword = Post_Tax_With_Keywords::create_from( $post_tax )->load_from_db()->get_keyword_curent();
	 ...
	 * @param string   $keyword_current Fill with current keyword or empty string.
	 * @param array    $keyword_info Fill with current keyword's data or empty array.
	 * @param array    $items Fill with other possible keywords data.
	 * @return void
	 */
	public function fill_current_and_possible_keywords_data( Post_Tax $post_tax, &$keyword_current = null, array &$keyword_info, array &$items ) {
		$keyword_current = Post_Tax_With_Keywords::create_from( $post_tax )->load_from_db()->get_keyword_curent();
			...
			if ( ! is_null( $kw_data ) ) {
				...
				if ( ! is_null( $keywords ) ) {
					...
					$items = array_filter(
						$keywords['result'],
						function ( $row ) {
							return $row['pos'] > 3.5 && $row['pos'] < 20.5;
							// filter pos 4-20 only.
						}
					);
				}
				unset( $kw_data );
