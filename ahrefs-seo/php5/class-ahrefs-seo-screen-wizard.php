...
class Ahrefs_Seo_Screen_Wizard extends Ahrefs_Seo_Screen {
	...
	private function do_redirect_after_initialized() {
		...
		Helper_Content::wp_redirect( remove_query_arg( 'step', add_query_arg( [ 'page' => Ahrefs_Seo::SLUG ], admin_url( 'admin.php' ) ) ) );
		die;
	}
	/**
	...
	public function show() {
		...
		if ( 1 === $step ) {
			$error = $this->error;
			$this->view->show( 'setup-wizard-1', __( 'Connect Ahrefs to your website', 'ahrefs-seo' ), compact( 'step', 'error', 'header_class' ), $this, 'wizard' );
		} elseif ( 2 === $step ) {
	...
	/**
	 * Check and maybe skip wizard steps
	 *
	 * @return void
	 */
	protected function maybe_skip_wizard() {
		if ( isset( $_GET['skip'] ) && current_user_can( Ahrefs_Seo::CAP_WIZARD_VIEW ) && check_admin_referer( $this->get_nonce_name(), 'skip' ) ) { // phpcs:ignore: WordPress.VIP.SuperGlobalInputUsage.AccessDetected -- we check nonce here.
			Ahrefs_Seo::get()->initialized_set( null, null, null, true, true );
			Helper_Content::wp_redirect( add_query_arg( 'page', Ahrefs_Seo::SLUG, admin_url( 'admin.php' ) ) );
			die;
		}
	}
	/**
