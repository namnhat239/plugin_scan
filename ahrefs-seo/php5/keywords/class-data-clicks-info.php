...
class Data_Clicks_Info {
	 ...
	 */
	public function __construct( $clicks = null, $position = null, $impressions = null ) {
		$this->clicks      = $clicks;
	 ...
	 * @return self
	 */
	public static function from_array( array $data = null ) {
		$clicks      = null;
		$position    = null;
		$impressions = null;
		if ( is_array( $data ) && isset( $data['k'] ) && isset( $data['p'] ) && isset( $data['i'] ) ) {
			$clicks      = ! is_null( $data['k'] ) ? (int) $data['k'] : null;
			$position    = ! is_null( $data['p'] ) ? (float) $data['p'] : null;
			$impressions = ! is_null( $data['i'] ) ? (int) $data['i'] : null;
		} elseif ( is_array( $data ) && isset( $data[0] ) && isset( $data[1] ) && isset( $data[2] ) ) {
			$clicks      = ! is_null( $data[0] ) ? (int) $data[0] : null;
			$position    = ! is_null( $data[1] ) ? (float) $data[1] : null;
			$impressions = ! is_null( $data[2] ) ? (int) $data[2] : null;
		}
