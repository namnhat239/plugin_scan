...
class Ahrefs_Seo {
	 ...
	 */
	const CURRENT_TABLE_VERSION = '81'; // previous published version is '80'.
	/**
	...
	public function __construct() {
		try {
			...
			$error = Ahrefs_Seo_Compatibility::on_type_error( $e, __METHOD__, __FILE__, __( 'Plugin initialization failed.', 'ahrefs-seo' ) );
		} catch ( \Exception $e ) {
	...
	private function init() {
		...
		add_action( 'init', [ $this, 'quick_updates_check' ], 13 );
	...
	public function admin_init() {
		...
		add_action( 'after_plugin_row_' . AHREFS_SEO_BASENAME, [ $this, 'after_plugin_row' ], 20, 3 );
	...
	public function quick_updates_check() {
		try {
			...
			$error = Ahrefs_Seo_Compatibility::on_type_error( $e, __METHOD__, __FILE__, __( 'Quick database update failed', 'ahrefs-seo' ) );
		} catch ( \Exception $e ) {
		...
		$mem_limit = (int) preg_replace_callback(
			...
			function ( $m ) {
				if ( $m[2] ) {
					...
					$pos = strpos( 'BKMG', $m[2] );
					if ( false !== $pos ) {
						return '' . intval( $m[1] ) * pow( 1024, $pos );
					}
				}
				return "{$m[1]}";
			},
	...
	/**
	 * Define and apply max excution time limit
	 *
	 * @return void
	 */
	...
	public function add_action_links( $actions ) {
		...
		if ( $this->initialized_get() ) {
			$actions[] = sprintf( '<a href="%s">%s</>', admin_url( 'admin.php?page=' . self::SLUG_SETTINGS . '&tab=' . Ahrefs_Seo_Screen_Settings::TAB_DATA . '&from-plugins-list=1' ), esc_html__( 'My audit data', 'ahrefs-seo' ) );
		}
		return $actions;
	}
	/**
	...
	public function after_plugin_row( $plugin_file, $plugin_data, $status ) {
		if ( is_multisite() && is_network_admin() && is_plugin_active_for_network( $plugin_file ) ) {
			...
			?>
			<tr class="plugin-update-tr active ahrefs-seo-plugin-notice-row" data-slug="" data-plugin="<?php echo esc_attr( $plugin_file ); ?>"><td colspan="<?php echo esc_attr( (string) $wp_list_table->get_column_count() ); ?>" class="plugin-update colspanchange" style="position:relative;top:-2px;"><div class="notice inline notice-warning notice-alt"><p>
			<?php
