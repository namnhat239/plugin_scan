 ...
 * Called directly on uninstall. Must not use any other code from the plugin.
 *
 * @since 0.9.7
 */
class Ahrefs_Seo_Uninstall {
	 ...
	 */
	const OPTION_SAVE_DATA_ON_UNINSTALL = 'ahrefs-seo-save-data-on-uninstall';
	/**
	 ...
	 * @return void
	 */
	public function set_option_save_data( $save_data ) {
		update_option( self::OPTION_SAVE_DATA_ON_UNINSTALL, $save_data );
	}
	/**
	 * Do we need to save data on uninstall?
	 * Default false.
	 *
	 * @return bool
	 */
	public function get_option_save_data() {
	...
	private function remove_options() {
		...
		foreach ( $items as $item ) {
			$options = $wpdb->get_col( $wpdb->prepare( "SELECT option_name AS name FROM {$wpdb->options}\n\t\t\t\t\tWHERE option_name LIKE %s", $wpdb->esc_like( "{$item}" ) . '%' ) );
	...
	private function remove_transients() {
		...
		foreach ( $items as $item ) {
			$transients = $wpdb->get_col( $wpdb->prepare( "SELECT option_name AS name FROM {$wpdb->options}\n\t\t\t\t\tWHERE option_name LIKE %s", $wpdb->esc_like( "_transient_{$item}" ) . '%' ) );
