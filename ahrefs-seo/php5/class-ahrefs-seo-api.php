...
class Ahrefs_Seo_Api extends Ahrefs_Seo_Abstract_Api {
			...
			if ( '' === $info ) {
				...
				Ahrefs_Seo::notify(
					new Ahrefs_Seo_Exception(
						'Ahrefs API get_subscription_info() returned empty result. ' . (string) wp_json_encode(
							[
								'token' => $token_value,
								'info'  => $info,
								'raw'   => $raw,
	 ...
	 */
	public function get_count_by_url( $url ) {
			...
			try {
				...
				if ( '' === $info ) {
					...
					$e = new Ahrefs_Seo_Exception(
						'Ahrefs API get_metrics_extended() returned empty result. ' . (string) wp_json_encode(
							[
								'info' => $info,
								'raw'  => $raw,
							]
					...
					if ( is_array( $data ) && isset( $data['metrics'] ) && is_array( $data['metrics'] ) && isset( $data['metrics']['backlinks'] ) && isset( $data['metrics']['refdomains'] ) ) {
						...
						$e     = new Ahrefs_Seo_Exception(
							'Ahrefs API get_metrics_extended() returned incorrect result. ' . (string) wp_json_encode(
								[
									'info' => $info,
									'raw'  => $raw,
								]
