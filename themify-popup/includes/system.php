...
class Themify_Popup {
	...
	public static function init() {
		...
		if ( is_admin() ) {
			...
			add_filter( 'themify_builder_layout_providers', array( __CLASS__, 'themify_builder_layout_providers' ) );
		} else {
	...
	public static function render() {
		...
		if (is_singular(array('tbp_template', 'tbuilder_layout_part', 'tglobal_style', )) || (method_exists( 'Themify_Builder_Model', 'is_front_builder_activate' ) && Themify_Builder_Model::is_front_builder_activate() )) {
			return;
		}
	...
	protected static function the_content() {
		...
		if ( isset( $ThemifyBuilder ) ) {
			...
			remove_filter( 'themify_builder_row_classes', array( __CLASS__, 'themify_builder_row_classes' ), 10, 3 );
                ...
                if(function_exists('themify_make_lazy')){//popup has display there is no need to render the images
	...
	protected static function enqueue($page_view=false) {
                wp_enqueue_script( 'themify-popup', themify_metabox_enque(THEMIFY_POPUP_URI . 'assets/scripts.js'), array( 'jquery' ), THEMIFY_POPUP_VERSION, true );
		wp_enqueue_style( 'themify-builder-animate', THEMIFY_POPUP_URI . 'assets/animate.min.css',null,'3.6.2' );
		wp_enqueue_style( 'magnific', themify_metabox_enque(THEMIFY_POPUP_URI . 'assets/lightbox.css'),null,THEMIFY_POPUP_VERSION );
		wp_enqueue_style( 'themify-popup', themify_metabox_enque(THEMIFY_POPUP_URI . 'assets/styles.css'), array( 'themify-builder-animate', 'magnific' ), THEMIFY_POPUP_VERSION );
                
		wp_localize_script( 'themify-popup', 'themifyPopup', array(
			'assets' => THEMIFY_POPUP_URI . 'assets',
			'count_views' => $page_view
		) );
	}
	 ...
	 * @since 1.0.0
	 */
	public static function themify_builder_layout_providers( $providers ) {
