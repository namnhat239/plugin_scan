...
class VI_WOO_ORDERS_TRACKING_ADMIN_WOO_ORDER_EMAIL {
	 ...
	 * @throws Exception
	 */
	public static function include_tracking_info_after_order_item( $item_id, $order, $plain_text = false ) {
		if ( $order && $plain_text === false ) {
			...
			if ( $tracking_code && $tracking_url ) {
				?>
                <div class="<?php echo esc_attr( 'woo-orders-tracking-orders-details' ) ?>"
                     style="margin: 5px 0;display: flex; flex-wrap: wrap;justify-content: space-between">
                    <div class="<?php echo esc_attr( 'woo-orders-tracking-orders-details-tracking-number' ) ?>">
                        <span><?php esc_html_e( 'Tracking Number: ', 'woo-orders-tracking' ) ?></span>
                        <a href="<?php echo esc_attr( $tracking_url ) ?>" rel="nofollow"
                           title="<?php esc_attr_e( "Tracking carrier {$carrier_name}", 'woo-orders-tracking' ) ?>"
                           target="_blank"><?php echo esc_html( $tracking_code ) ?></a>
                    </div>
                    <div class="<?php echo esc_attr( 'woo-orders-tracking-orders-details-tracking-carrier' ) ?>">
