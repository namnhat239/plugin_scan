...
class VI_WOO_ORDERS_TRACKING_ADMIN_ORDERS_TRACK_INFO {
	...
	public function addCustomImportButton() {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                jQuery(".wrap .page-title-action").after("<a class='page-title-action' target='_blank' href='<?php echo esc_attr( esc_url( admin_url( 'admin.php' ) ) . '?page=woo-orders-tracking-import-csv' ); ?>'><?php esc_html_e( 'Import tracking number', 'woo-orders-tracking' ) ?></a>"
                    + "<a class='page-title-action' target='_blank' href='<?php echo esc_attr( esc_url( admin_url( 'admin.php' ) ) . '?page=woo-orders-tracking-export' ); ?>'><?php esc_html_e( 'Export tracking number ', 'woo-orders-tracking' ) ?></a>");
            });
        </script>
		<?php
	}
				...
				if ( count( $line_items ) ) {
						...
						if ( $tracking_number && ! in_array( $tracking_number, $tracking_list ) ) {
                            ...
                            <div class="<?php echo esc_attr( self::set( $container_class ) ) ?>"
                                ...
                                <a class="<?php echo esc_attr( self::set( 'tracking-number' ) ) ?>"
                                   href="<?php echo esc_attr( $tracking_url_show ) ?>"
								...
								if ( $transID && in_array( $paypal_method, $this->settings->get_params( 'supported_paypal_gateways' ) ) ) {
                                    ...
                                    <span class="<?php echo esc_attr( self::set( $paypal_class ) ) ?>"
                                        ...
                                        <img class="<?php echo esc_attr( self::set( 'item-tracking-button-add-to-paypal' ) ) ?>"
                                             ...
                                             src="<?php echo esc_attr( VI_WOO_ORDERS_TRACKING_PAYPAL_IMAGE ) ?>">
                                    </span>
									<?php
								}
					...
					if ( $tracking_info_count > 0 && $this->settings->get_params( 'service_carrier_enable' ) && $service_carrier_type === 'trackingmore' ) {
						...
						for ( $i = 0; $i < $tracking_info_count; $i ++ ) {
                            ...
                            <div class="<?php echo esc_attr( self::set( array(
								...
								if ( $tracking_info[ $i ]['carrier_type'] === 'service-carrier' ) {
									...
									?>
                                    <div class="<?php echo esc_attr( self::set( 'order-tracking-info-tracking-number' ) ) ?> "
                                         style="display: flex; justify-content: space-between;">
                                        <strong class="<?php echo esc_attr( self::set( 'order-tracking-info-tracking-number-code' ) ) ?>"
                                                title="<?php echo esc_attr( $tracking_info[ $i ]['tracking_code'] ) ?>"><?php echo $tracking_info[ $i ]['tracking_code'] ?></strong>
                                        <div class="<?php echo esc_attr( self::set( 'order-tracking-info-refresh' ) ) ?> <?php echo $tracking_info[ $i ]['tracking_url'] ? esc_attr( self::set( 'order-tracking-info-hidden' ) ) : '' ?>"
                                             data-order_id="<?php echo esc_attr( $order_id ) ?>"
                                             data-tracking_number="<?php echo esc_attr( $tracking_info[ $i ]['tracking_code'] ) ?>"
                                             data-carrier_name="<?php echo esc_attr( $tracking_info[ $i ]['carrier_name'] ) ?>"
                                             data-carrier_id="<?php echo esc_attr( $tracking_info[ $i ]['carrier_id'] ) ?>"
									...
									if ( $existing_track_info ) {
										...
										if ( $current_track_info['track_info'] ) {
											?>
                                            <div class="<?php echo esc_attr( self::set( 'order-tracking-info-carrier' ) ) ?>">
                                            ...
                                            <div class="<?php echo esc_attr( self::set( array(
                                                ...
                                                <span title="<?php echo esc_attr( $current_track_info['last_event'] ) ?>"><?php echo $current_track_info['status']; ?></span>
                                            </div>
											<?php
										} else {
						...
						if ( $tracking_service_count ) {
							?>
                            <div class="<?php echo esc_attr( self::set( array( 'order-tracking-info-icon' ) ) ) ?>"
                                 data-order_id="<?php echo esc_attr( $order_id ) ?>">
		...
		if ( $track_info ) {
			...
			if ( ! $data_track ) {
                ...
                <div class="<?php echo esc_attr( self::set( array(
					...
					<?php echo esc_html__( 'Status: ', 'woo-orders-tracking' ); ?>
                    <span title="<?php echo esc_attr( isset( $track_info['last_event'] ) ? $track_info['last_event'] : '' ) ?>"><?php echo isset( $track_info['status'] ) ? $track_info['status'] : ''; ?></span>
                </div>
				<?php
			}
		} else {
	...
	public function posts_where( $where, $wp_query ) {
		...
		if ( isset( $_GET['filter_action'] ) && $tracking_code ) {
