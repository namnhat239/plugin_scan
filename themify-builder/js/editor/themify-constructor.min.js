...
toggleable_fields:{sort(a,b){const c=t.Utils.getMouseEvents();a.addEventListener(c.mousedown,function(d){if(!("touchstart"!==d.type&&1!==d.which||d.target.parentNode!==this&&d.target.parentNode.parentNode!==this)){d.stopImmediatePropagation();let e,f,g,h,k,m=0,p,q,n,r={},v=this.ownerDocument,u=d.target.closest(".tb_toggleable_item"),w,x,C,z=!1,D;const I=function(B){B.stopImmediatePropagation();e&&cancelAnimationFrame(e);e=requestAnimationFrame(function(){if(v){var y=B.touches?B.touches[0].clientX:
B.clientX,F=B.touches?B.touches[0].clientY:B.clientY;y="mousemove"===B.type?B.target:v.elementFromPoint(y,F);var G=F-q-D.getBoundingClientRect().top;if(0<G&&G<C){if(p.style.transform="translateY("+G+"px)",J(F),!(F>=x||F<=w)){if(y&&y!==u&&y.classList.contains("tb_toggleable_item")){G=F>m?"bottom":"top";if(h!==G||g!==y)u.style.display="none","bottom"===G?y.after(u):y.before(u),u.style.display="";g=y;h=G}m=F}}else J(F)}})},J=function(B){if(n)if(B>=x||B<=w){if(!1===z){z=!0;const y=parseInt(x/10);n.scrollTop+=
