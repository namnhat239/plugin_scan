...
if (!class_exists('WWP_Order')) {
    ...
    class WWP_Order
    {
         ...
         * Add order item meta for more accurate wholesale reporting in the future.
         * For WC 2.7.x series.
         * At this current state of the hook, the item ID & the order ID is not being generated yet.
         * Therefore to update the order item meta, we will use the item object.
         *
         * @since 2.0.2
         * @access public
         *
         ...
         */
        public function add_order_item_meta_wc2_7_new($item, $cart_item_key, $values, $order)
        {

            $this->add_order_item_meta(null, $values, $item);

            do_action('wwp_add_order_item_meta', $item, $item, $order);
         ...
         *
         * @param int    $item_id   Order item id.
         * @param array  $cart_item Array of cart item data.
         * @param Object $item      Order item object.
         */
        public function add_order_item_meta($item_id, $cart_item, $item = null)
        {

            $user_wholesale_role = $this->_wwp_wholesale_roles->getUserWholesaleRole();

            if (isset($cart_item['data']->wwp_data) && isset($cart_item['data']->wwp_data['wholesale_role']) &&
                !empty($user_wholesale_role) && $user_wholesale_role[0] == $cart_item['data']->wwp_data['wholesale_role']) {

                if (is_null($item_id) && is_object($item)) {
                    if (isset($cart_item['data']->wwp_data['wholesale_priced'])) {
                        $item->update_meta_data('_wwp_wholesale_priced', $cart_item['data']->wwp_data['wholesale_priced']);
                    }

                    if (isset($cart_item['data']->wwp_data['wholesale_role'])) {
                        $item->update_meta_data('_wwp_wholesale_role', $cart_item['data']->wwp_data['wholesale_role']);
                    }
                } else {
                    if (isset($cart_item['data']->wwp_data['wholesale_priced'])) {
                        wc_add_order_item_meta($item_id, '_wwp_wholesale_priced', $cart_item['data']->wwp_data['wholesale_priced']);
                    }

                    if (isset($cart_item['data']->wwp_data['wholesale_role'])) {
                        wc_add_order_item_meta($item_id, '_wwp_wholesale_role', $cart_item['data']->wwp_data['wholesale_role']);
                    }
                }
        ...
        public function run()
        {
            ...
            if (version_compare($woocommerce_data['Version'], '3.0.0', '>=')) {
                ...
                add_action('woocommerce_checkout_create_order_line_item', array($this, 'add_order_item_meta_wc2_7_new'), 10, 4);
            } else {
