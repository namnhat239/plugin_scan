...
class Main
{
	 ...
	 * @var array
	 */
	public $loadExceptionsPageLevel = array( 'styles' => array(), 'scripts' => array() );

	/**
	 * @var array[]
	 */
	public $loadExceptionsPostType = array( 'styles' => array(), 'scripts' => array() );
...
SQL;
	...
	public function setVarsAfterAnyUpdate()
    {
		if ( ! is_admin() ) {
			...
			$this->loadExceptionsPageLevel      = $this->getLoadExceptionsPageLevel($type, $this->currentPostId);
			...
			if ($post && $type === 'post' && isset($post->post_type) && $post->post_type) {
				...
				if ( ! empty($loadExceptionsPostType) ) {
	...
	public function filterStyles()
	{
		...
		if ( empty($list) || ! is_array($list) ) {
			...
			* */
            ...
            $list = apply_filters('wpacu_filter_styles_list_load_exception', $list);
			/*
    ...
    public function filterScripts()
    {
	    ...
	    if ( empty($list) ) {
			...
			* */
            ...
            // These are PRO rules or rules added via custom coding
		    $list = apply_filters('wpacu_filter_scripts_list_load_exception', $list);
		    /*
			 * [END] Load Exception Check
			 * */
    ...
    public function getLoadExceptionsPageLevel($type = 'post', $postId = '')
    {
        ...
        if ($type === 'post' || ($homepageClass->data['show_on_front'] === 'page' && $postId) ) {
        ...
        */
        ...
        elseif ($isFrontEndEditView) {
	        ...
	        $data = array(
		        ...
		        'current_unloaded_page_level' => (array) json_decode( $this->getAssetsUnloadedPageLevel( $this->getCurrentPostId() ) )
	        );
    ...
    public function ajaxGetJsonListCallback()
    {
        ...
        $data['current_unloaded_page_level'] = (array) json_decode( $this->getAssetsUnloadedPageLevel( $postId ) );
     ...
     */
    public function getAssetsUnloadedPageLevel($postId = 0)
    {
