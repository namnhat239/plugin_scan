            ...
            <div style="padding: 10px; background: white; border: 1px solid #ccd0d4; box-shadow: 0 1px 1px rgba(0,0,0,.04);">
                ...
                if ( $hasPostsWithOptions ) {
                    foreach ($data['page_options_results']['posts'] as $results) {
                        ...
                        ?>
                        <tr <?php echo esc_html($rowStyle); ?>>
                            <td><?php echo get_the_title($results['post_id']); ?> / ID: <?php echo (int)$results['post_id']; ?>, Status: <?php echo esc_html($postStatusText); ?><br /><small><a target="_blank" href="<?php echo get_permalink($results['post_id']); ?>"><?php echo get_permalink($results['post_id']); ?></a></small></td>
                            <td>
                                <?php
                                $optionsForCurrentPage = array();
