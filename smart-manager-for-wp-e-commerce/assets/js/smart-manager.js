...
Smart_Manager.prototype.init = function() {
	...
	this.possibleOperators = {
			...
			'date': intOperators,
			'dropdown': {'is': _x('is', "select options - operator for 'dropdown' data type fields", 'smart-manager-for-wp-e-commerce'), 'is not': _x('is not', "select options - operator for 'dropdown' data type fields", 'smart-manager-for-wp-e-commerce')},
			'text': {'is': _x('is', "select options - operator for 'text' data type fields", 'smart-manager-for-wp-e-commerce'), 'like': _x('contains', "select options - operator for 'text' data type fields", 'smart-manager-for-wp-e-commerce'), 'is not': _x('is not', "select options - operator for 'text' data type fields", 'smart-manager-for-wp-e-commerce'), 'not like': _x('not contains', "select options - operator for 'text' data type fields", 'smart-manager-for-wp-e-commerce')}
	}
	...
	if( typeof this.sm_dashboards != 'undefined' && this.sm_dashboards != '' ) {
		...
		if(this.sm_beta_pro == 1 && (this.recentDashboardType == 'view' || defaultDashboardslug == '') && this.recentViews.length > 0){
...
Smart_Manager.prototype.loadNavBar = function() {
	...
	if( window.smart_manager.sm_beta_pro == 1 ) {
		...
		if(window.smart_manager.recentDashboards.length > 0){
			...
			window.smart_manager.dashboard_select_options += (options != '') ? '<optgroup label="'+_x('Common post types', 'dashboard option groups', 'smart-manager-for-wp-e-commerce')+'">'+options+'</optgroup>' : '';
		}
		...
		if(Object.keys(window.smart_manager.sm_dashboards).length > 0){
			window.smart_manager.dashboard_select_options += '<optgroup label="'+_x('All post types', 'dashboard option groups', 'smart-manager-for-wp-e-commerce')+'">';
...
Smart_Manager.prototype.event_handler = function() {
	...
	.off( 'click', '.sm_top_bar_action_btns #sm_beta_move_to_trash, .sm_top_bar_action_btns #sm_beta_delete_permanently').on( 'click', '.sm_top_bar_action_btns #sm_beta_move_to_trash, .sm_top_bar_action_btns #sm_beta_delete_permanently' ,function(){
		...
		if( deletePermanently && window.smart_manager.deletePermanently.disable ) {
			window.smart_manager.notification = {status:'error',message: window.smart_manager.deletePermanently.error_message,hideDelay: window.smart_manager.notificationHideDelayInMs}
			window.smart_manager.showNotification()
			return false;
		}
