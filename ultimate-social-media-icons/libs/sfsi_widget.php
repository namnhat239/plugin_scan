...
class Sfsi_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
		    ...
		    <input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo ($instance['title'])? $instance['title'] : 'Please follow & like us :)'; ?>" style="width:100%;" />
		</p>
		 <p>
   ...
   /* Access the saved settings in database  */
    ...
    if($sfsi_section3['sfsi_shuffle_icons']=="yes")
    {
       if($sfsi_section3['sfsi_shuffle_Firstload']=="yes" && $sfsi_section3['sfsi_shuffle_interval']=="yes"){
        ...
        {
            $jquery.="SFSI( document ).ready(function( $ ) {  SFSI('.sfsi_wDiv').each(function(){ new window.Manipulator( SFSI(this)); });  setTimeout(function(){  SFSI('#sfsi_wDiv').each(function(){ SFSI(this).click(); })},500); });";
        }    
    }    
