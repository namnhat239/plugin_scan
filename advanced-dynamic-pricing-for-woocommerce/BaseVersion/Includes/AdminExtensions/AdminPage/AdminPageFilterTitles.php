...
class AdminPageFilterTitles
{
    ...
    public function getIdsForFilterTitles($rules)
    {
        ...
        foreach (array_keys(Helpers::getCustomProductTaxonomies()) as $taxName) {
            $conditionsQtyCustomTaxonomyTypes["custom_taxonomy_$taxName"]           = $taxName;
            $conditionsAmountCustomTaxonomyTypes["amount_custom_taxonomy_$taxName"] = $taxName;
        }
        ...
        foreach ($rules as $rule) {
            ...
            if (isset($rule['conditions'])) {
                foreach ($rule['conditions'] as $condition) {
                    if ($condition['type'] === 'specific' && isset($condition['options'][2])) {
                    ...
                    } elseif ($condition['type'] === 'product_attributes' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        ...
                        $filtersByType['product_attributes'] = array_merge($filtersByType['product_attributes'],
                            (array)$value);
                    } elseif ($condition['type'] === 'product_custom_fields' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        $value                                  = $condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY];
                        $filtersByType['product_custom_fields'] = array_merge($filtersByType['product_custom_fields'],
                            (array)$value);
                    } elseif ($condition['type'] === 'product_sku' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        $value                                  = $condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY];
                        ...
                        $filtersByType['usermeta'] = array_merge($filtersByType['usermeta'],
                            (array)$value);
                    } elseif ($condition['type'] === 'product_categories' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        $value                               = $condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY];
                        $filtersByType['product_categories'] = array_merge($filtersByType['product_categories'],
                            (array)$value);
                    } elseif ($condition['type'] === 'product_category_slug' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        $value                                  = $condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY];
                        $filtersByType['product_category_slug'] = array_merge($filtersByType['product_category_slug'],
                            (array)$value);
                    } elseif ($condition['type'] === 'product_tags' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        $value                         = $condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY];
                        $filtersByType['product_tags'] = array_merge($filtersByType['product_tags'],
                            (array)$value);
                    } elseif ($condition['type'] === 'products' && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                        $value                     = $condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY];
                        ...
                        $filtersByType['subscriptions'] = array_merge($filtersByType['subscriptions'],
                            (array)$value);
                    } elseif (isset($conditionsQtyCustomTaxonomyTypes[$condition['type']]) && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
                    ...
                    } elseif (isset($conditionsAmountCustomTaxonomyTypes[$condition['type']]) && isset($condition['options'][ListComparisonCondition::COMPARISON_LIST_KEY])) {
