...
function vsel_metabox_callback( $post ) {
	...
	if ( $start_date > $end_date ) {
		$notice_date = '<span style="color:red;">'.sprintf( __( 'Error: %1$s must be equal to or greater than %2$s.', 'very-simple-event-list' ), __( 'End date', 'very-simple-event-list' ), __( 'Start date', 'very-simple-event-list' ) ).'</span>';
	}

	// error notice if start time is greater than end time
	$notice_time = '';
	if ( ($start_date == $end_date) && ($start_time > $end_time) ) {
		$notice_time = '<span style="color:red;">'.sprintf( __( 'Error: %1$s must be equal to or greater than %2$s.', 'very-simple-event-list' ), __( 'End time', 'very-simple-event-list' ), __( 'Start time', 'very-simple-event-list' ) ).'</span>';
	}

	// metabox fields
	if ( $one_date == 'yes' ) { ?>
		<p><label for="event-end-date"><?php esc_attr_e( 'Date', 'very-simple-event-list' ); ?></label>
		<input class="widefat" id="event-end-date" type="text" name="event-end-date" required maxlength="10" placeholder="<?php esc_attr_e( 'Use datepicker', 'very-simple-event-list' ); ?>" value="<?php echo wp_date( $date_format, esc_attr( $end_date_timestamp ), $utc_time_zone ); ?>" /></p>
	<?php } else { ?>
		<p><label for="event-start-date"><?php esc_attr_e( 'Start date', 'very-simple-event-list' ); ?></label>
		<input class="widefat" id="event-start-date" type="text" name="event-start-date" required maxlength="10" placeholder="<?php esc_attr_e( 'Use datepicker', 'very-simple-event-list' ); ?>" value="<?php echo wp_date( $date_format, esc_attr( $start_date_timestamp ), $utc_time_zone ); ?>" /></p>
		<p><label for="event-end-date"><?php esc_attr_e( 'End date', 'very-simple-event-list' ); ?> <?php echo esc_attr( $notice_date ); ?></label>
		<input class="widefat" id="event-end-date" type="text" name="event-end-date" required maxlength="10" placeholder="<?php esc_attr_e( 'Use datepicker', 'very-simple-event-list' ); ?>" value="<?php echo wp_date( $date_format, esc_attr( $end_date_timestamp ), $utc_time_zone ); ?>" /></p>
	<?php }
	if ( $one_time != 'yes' ) { ?>
		<fieldset><p><legend><?php esc_attr_e( 'Start time', 'very-simple-event-list' ); ?></legend>
			...
			<?php foreach ($hour_values as $hour_value_start) { ?>
				<option value='<?php echo esc_attr( $hour_value_start ); ?>'<?php echo ( ( wp_date( 'H', esc_attr( $start_date_timestamp ), $utc_time_zone ) ) == $hour_value_start )?' selected':''; ?>><?php echo esc_attr( $hour_value_start ); ?></option>
			<?php } ?>
			</select>
			...
			<?php foreach ($minute_values as $minute_value_start) { ?>
				<option value='<?php echo esc_attr( $minute_value_start ); ?>'<?php echo ( ( wp_date( 'i', esc_attr( $start_date_timestamp ), $utc_time_zone) ) == $minute_value_start )?' selected':''; ?>><?php echo esc_attr( $minute_value_start ); ?></option>
			<?php } ?>
			</select>
		</p></fieldset>
		<fieldset><p><legend><?php esc_attr_e( 'End time', 'very-simple-event-list' ); ?> <?php echo esc_attr( $notice_time ); ?></legend>
			<select id="event-end-time-hour" name="event-end-time-hour" style="width:100px;">
			<?php foreach ($hour_values as $hour_value_end) { ?>
				<option value='<?php echo esc_attr( $hour_value_end ); ?>'<?php echo ( ( wp_date( 'H', esc_attr( $end_date_timestamp ), $utc_time_zone ) ) == $hour_value_end)?' selected':''; ?>><?php echo esc_attr( $hour_value_end ); ?></option>
			<?php } ?>
			</select>
			...
			<?php foreach ($minute_values as $minute_value_end) { ?>
				<option value='<?php echo esc_attr( $minute_value_end ); ?>'<?php echo ( ( wp_date( 'i', esc_attr( $end_date_timestamp ), $utc_time_zone ) ) == $minute_value_end)?' selected':''; ?>><?php echo esc_attr( $minute_value_end ); ?></option>
			<?php } ?>
			</select>
	...
	<?php } else { ?>
		...
		<input class="widefat" id="event-time" type="text" name="event-time" placeholder="<?php esc_attr_e( 'Example: 16:00 - 18:00', 'very-simple-event-list' ); ?>" value="<?php echo esc_attr( $time ); ?>" /></p>
	<?php } ?>
	<p><label for="event-location"><?php esc_attr_e( 'Location', 'very-simple-event-list' ); ?></label>
	<input class="widefat" id="event-location" type="text" name="event-location" placeholder="<?php esc_attr_e( 'Example: Times Square', 'very-simple-event-list' ); ?>" value="<?php echo esc_attr( $location ); ?>" /></p>
	<p><label for="event-link"><?php esc_attr_e( 'Link to more info', 'very-simple-event-list' ); ?></label>
	<input class="widefat" id="event-link" type="text" name="event-link" placeholder="<?php esc_attr_e( 'Example: www.wordpress.org', 'very-simple-event-list' ); ?>" value="<?php echo esc_url( $link ); ?>" /></p>
	<p><label for="event-link-label"><?php esc_attr_e( 'Link label', 'very-simple-event-list' ); ?></label>
	<input class="widefat" id="event-link-label" type="text" name="event-link-label" placeholder="<?php esc_attr_e( 'Example: More info', 'very-simple-event-list' ); ?>" value="<?php echo esc_attr( $link_label ); ?>" /></p>
	...
	<label for="event-link-title"><?php esc_attr_e('Redirect event title to the more info link', 'very-simple-event-list'); ?></label></p>
...
function vsel_custom_columns_content( $column_name, $post_id ) {
	...
	if ( 'start_date_column' == $column_name ) {
		if(!empty( $start_date_timestamp ) ) {
			echo wp_date( get_option('date_format'), esc_attr($start_date_timestamp), $utc_time_zone );
		} else {
	...
	if ( 'end_date_column' == $column_name ) {
		if(!empty( $end_date_timestamp ) ) {
			echo wp_date( get_option('date_format'), esc_attr($end_date_timestamp), $utc_time_zone );
		} else {
			echo '<span aria-hidden="true">&mdash;</span>';
		}
	}
	...
	if ( 'time_column' == $column_name ) {
		if ( $one_time != 'yes' ) {
			if(!empty( $start_date_timestamp ) && !empty( $end_date_timestamp ) ) {
				echo wp_date( get_option('time_format'), esc_attr($start_date_timestamp), $utc_time_zone ).' - '.wp_date( get_option('time_format'), esc_attr($end_date_timestamp), $utc_time_zone );
			} else {
				echo '<span aria-hidden="true">&mdash;</span>';
			}
		} else {
