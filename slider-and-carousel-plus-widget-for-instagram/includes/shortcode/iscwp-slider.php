...
function iscw_slider( $atts, $content = null ) {
	...
	if( empty($insta_user_media) ) {
		$ajax_conf = compact('popup', 'loop_count', 'count', 'main_cls', 'offset_css', 'height_css', 'show_likes_count', 'show_comments_count', 'unique', 'instagram_link_main', 'instagram_link_text', 'show_caption' );
	}
	...
	<div class="iscwp-gallery-slider-wrp iscwp-clearfix iscwp-main-wrp ">
		<div id="iscwp-gallery-<?php echo $unique; ?>" class="iscwp-gallery-slider iscwp-gallery-main-wrap iscwp-result-wrap slidestoshow-<?php echo $slidestoshow; ?> <?php echo $popup_cls.' '.$image_fit_class; ?>" data-user="<?php echo $username; ?>" <?php if( empty($insta_user_media) ){ ?> data-ajax="true" data-ajax-conf="<?php echo htmlspecialchars(json_encode($ajax_conf)); ?>" <?php } ?>>
			<?php
			if( !empty($insta_user_media) ) { 
