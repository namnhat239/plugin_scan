 ...
 * @since 1.6
 */
function iscw_clean( $var ) {
	if ( is_array( $var ) ) {
		return array_map( 'iscw_clean', $var );
	} else {
 ...
 * @since 1.6
 */
function iscw_clean_number( $var, $fallback = null, $type = 'int' ) {
...
function iscw_insta_request( $api_url, $args = array() ) {
	...
	if( $api_url ) {
		...
		// Getting Data
		$response = wp_remote_get( esc_url_raw( $api_url ), $api_args );
...
function iscw_get_user_media( $username, $limit = 60, $cache = true ) {
	...
	if( $stored_transient === false || (!$cache) ) {
		...
		/* Try to fetch data by another api request */
		if( empty( $user_data ) ) {

			$api_url 		= "https://www.instagram.com/{$username}/?__a=1";
			$response_data 	= iscw_insta_request( $api_url );
			$page_data = json_decode($response_data['body'], true);

			if ( $page_data || !empty($page_data['graphql']['user']) ) {
				$user_data = $page_data['graphql']['user'];
			}
		}

		$result_data = iscw_user_data_store( $username, $user_data, $transient_key );
...
/**
 * Function to store instagram data
 * 
 * @package Slider and Carousel Plus Widget Instagram
 * @since 1.9
 */
function iscw_user_data_store( $username, $user_data, $transient_key ) {
	...
	if ( ! $user_data['is_private'] ) {
		...
		$cache_transient 			= ( !empty( $cache_transient ) && is_array( $cache_transient ) ) ? $cache_transient : array();
		$cache_transient[$username] = $transient_key;
		update_option( 'wp_iscwp_cache_transient', $cache_transient, false );
